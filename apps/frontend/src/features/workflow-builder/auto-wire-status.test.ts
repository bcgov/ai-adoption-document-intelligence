...
describe("computeNodeInputIssues", () => {
  ...
  it("does NOT flag an OPTIONAL locked-unbound port", () => {
    // azureClassify.poll's blobKey (kind DocumentRef, required: false) is
    // locked with no binding — a deliberate disconnect of an optional port
    // is not a problem. resultId and constructedClassifierName (both
    // required) are locked/bound so they don't also surface as problems.
    const cfg = makeConfig(
      {
        A: {
          id: "A",
          ...
          inputs: [
            ...
            { port: "constructedClassifierName", ctxKey: "m1" },
          ],
          metadata: {
            lockedInputPorts: [
              "resultId",
...
describe("computeNodeInputIssues — dyn.* nodes via the merged catalog", () => {
  ...
  it("flags a required unbound dyn input as unsatisfied", () => {
    const cfg = makeConfig({
      D: {
        id: "D",
        type: "activity",
    ...
    expect(computeNodeInputIssues(cfg, "D", [dynEntry])).toEqual({
      status: "unsatisfied",
      problemPorts: [
        {
          port: "document",
  ...
  it("reports 'ok' when the dyn input is pinned to a live workflow variable", () => {
    const cfg = makeConfig(
      {
        D: {
          id: "D",
  ...
  it("fails soft — a dyn type absent from the merged catalog reports 'ok'", () => {
    // A soft-deleted lineage (or an instance pinning a version of one) has
    // no merged-catalog entry. That absence must not paint problems: the
    // runtime still resolves pinned versions of soft-deleted lineages, and
    // the deleted state has its own dedicated surfaces.
    const cfg = makeConfig({
      D: {
        id: "D",
        type: "activity",
    ...
    expect(computeNodeInputIssues(cfg, "D", [])).toEqual({
      status: "ok",
      problemPorts: [],
    });
    expect(computeNodeInputIssues(cfg, "D")).toEqual({
