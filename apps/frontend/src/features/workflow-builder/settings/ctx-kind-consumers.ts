 ...
 * (`useActivityCatalog().entries` filtered to `dyn.*`) — extends the
 * accounting to dynamic nodes, exactly as it does for the drawer's
 * `autoWireIssuesToValidationErrors` pass; without it a dyn node's pinned
 * inputs are invisible here.
 *
