...
function CtxRow({
   ...
   */
  // Same single query instance the canvas subscribes to (one query key, no
  // extra fetch), filtered to dyn.* because that is all the mismatch walk
  // consults the list for — without it a dyn node's pinned inputs never
  // show up in this notice.
  ...
  const kindMismatches = useMemo(
    () => findKindMismatchedConsumers(config, ctxKey, dynamicEntries),
    [config, ctxKey, dynamicEntries],
  );
