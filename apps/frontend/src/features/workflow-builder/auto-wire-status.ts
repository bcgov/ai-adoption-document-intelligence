...
/**
 * `dynamicEntries` extends the resolution to the merged catalog: pass the
 * published `dyn.*` entries (in practice `useActivityCatalog().entries`)
 * and a dynamic node's required inputs get the same unbound/ambiguous
 * accounting a static activity gets. A `dyn.*` type absent from BOTH
 * catalogs — a soft-deleted lineage, or an instance pinning a version of
 * one — reports `ok` rather than an error: the runtime still resolves
 * pinned versions of soft-deleted lineages, and the deleted state has its
 * own dedicated surfaces (the red "Deleted" pill, the settings alert).
 */
export function computeNodeInputIssues(
  ...
  for (const port of entry.inputs) {
    ...
    // The cast bridges the merged catalog's serialised `kind?: string` to
    // the package's `KindRef` — sound because published dynamic signatures
    // are registry-checked (see `DynamicNodePortDescriptor`); static
    // entries already carry a `KindRef`.
    const result = resolveInputPort(config, nodeId, {
      name: port.name,
      kind: port.kind as KindRef | undefined,
    });
    const isProblem =
