    ...
    if (node.type === "activity") {
      const flowNode: ActivityFlowNode = {
        ...
        data: {
          ...
          portRows: computePortRows(config, node.id, wires, dynamicEntries),
      ...
      const peekPortLabel =
        (producerNode?.type === "activity" || producerNode?.type === "pollUntil"
          ? (
              ...
              dynamicEntries.find(
                (e) => e.activityType === producerNode.activityType,
              )
            )?.outputs.find((o) => o.name === wire.sourcePort)?.label
...
function buildStructuralFingerprint(
  ...
  // re-projection.
  //
  // Dynamic-node signatures participate too: `computePortRows` resolves
  // `dyn.*` types against the merged catalog, which arrives ASYNC of the
  // config — without this the fingerprint gate would hold the first
  // projection's row-less dyn cards on screen after the catalog loaded
  // (and a republished signature would leave stale rows). Ports only —
  // entry metadata the projection doesn't read (description, icon) stays
  // out so a cosmetic catalog edit doesn't re-project the canvas.
  const dynSignatures = dynamicEntries
    .map(
      (e) =>
        `${e.activityType}(${e.inputs
          .map(
            (p) =>
              `${p.name}:${p.label ?? ""}:${p.kind ?? ""}:${p.required ? 1 : 0}`,
          )
          .join(",")}→${e.outputs
          .map((p) => `${p.name}:${p.label ?? ""}:${p.kind ?? ""}`)
          .join(",")})`,
    )
    .sort();
...
function WorkflowEditorCanvasInner({
  ...
  // Merged-catalog access for the pure selectors (`computePortRows`,
  // `rendersPerPortHandle`, `resolveDataDropTarget`): the `dyn.*` slice of
  // the same TanStack query the node renderers and the palette already
  // subscribe to (one query key, no extra fetch). Filtered to dynamic
  // entries because that is all the selectors consult the list for — the
  // static catalog stays authoritative for everything it knows — and so
  // the fingerprint fold above only churns when a dynamic signature
  // actually changes. While the query is pending this is empty and dyn
  // cards render row-less, exactly the pre-fetch state.
  const activityCatalog = useActivityCatalog();
  const dynamicCatalogEntries = useMemo<readonly DynamicNodeCatalogEntry[]>(
    () =>
      activityCatalog.entries.filter((entry: DynamicNodeCatalogEntry) =>
  ...
  const dataFingerprint = useMemo(
    () =>
      buildStructuralFingerprint(config, simplifiedView, dynamicCatalogEntries),
    [config, simplifiedView, dynamicCatalogEntries],
  );
  ...
  useEffect(() => {
    ...
    const projected = simplifiedView
      ...
      : projectFlowWires(derivedWires, config, dynamicCatalogEntries);
  ...
  const handleConnect = useCallback(
    ...
    [config, onConfigChange, openConnectSummary, dynamicCatalogEntries],
  );
  ...
  const handleConnectEnd = useCallback<OnConnectEnd>(
    ...
    [config, openHoverExtendNow, dynamicCatalogEntries],
  );
