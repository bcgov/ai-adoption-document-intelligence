...
describe("computePortRows — dyn.* nodes resolve against the merged catalog", () => {
  ...
  it("renders rows — a required unbound input carries the amber-ring state", () => {
    const { inputs, outputs } = computePortRows(dynNode(), "D", [], [dynEntry]);

    expect(inputs).toHaveLength(2);
    expect(inputs[0]).toMatchObject({
      name: "document",
      label: "Document",
      handleId: "in-document",
      kind: "Document",
      required: true,
      bound: false,
      needsSource: true,
  ...
  it("reports a ctx-bound dyn input as clean, with the fromCtx chip", () => {
    const cfg = config({
      nodes: {
        D: node<ActivityNode>({
          id: "D",
          type: "activity",
          activityType: "dyn.sentiment-scorer",
          inputs: [{ port: "document", ctxKey: "docVar" }],
        }),
      },
      ctx: { docVar: { type: "string", isInput: true, kind: "Document" } },
    });

    const { inputs } = computePortRows(cfg, "D", [], [dynEntry]);
    expect(inputs.find((row) => row.name === "document")).toMatchObject({
      bound: true,
      needsSource: false,
      fromCtx: "docVar",
    });
  ...
  it("fails soft when the lineage is missing from the merged catalog", () => {
    // A soft-deleted lineage — or an instance pinning a version of one — is
    // absent from the merged list. The card renders row-less, exactly like
    // an unregistered static type: the runtime still resolves pinned
    // versions of soft-deleted lineages, and the deleted state has its own
    // dedicated "Deleted" pill, so this is not an error surface.
    const { inputs, outputs } = computePortRows(dynNode(), "D", [], []);
    expect(inputs).toEqual([]);
    expect(outputs).toEqual([]);
    expect(estimateNodeHeight(dynNode(), "D", [])).toBe(ACTIVITY_BASE_HEIGHT);
  });

  it("prefers the static catalog over a dynamic entry of the same type", () => {
    const shadow: DynamicNodeCatalogEntry = {
      activityType: "azureOcr.submit",
      inputs: [],
      outputs: [],
    };
    ...
    const { inputs } = computePortRows(cfg, "B", [], [shadow]);
    expect(inputs.map((row) => row.name)).toContain("fileData");
  });

  it("rendersPerPortHandle and estimateNodeHeight follow the dyn rows", () => {
    const cfg = dynNode();
    expect(
      rendersPerPortHandle(cfg, "D", "document", "input", [dynEntry]),
    ).toBe(true);
    expect(rendersPerPortHandle(cfg, "D", "score", "output", [dynEntry])).toBe(
      true,
    );
    // Without the merged entries the handles do not mount — the wire
    // projection must then anchor node-level, not at a dead handle id.
    expect(rendersPerPortHandle(cfg, "D", "document", "input")).toBe(false);
    expect(estimateNodeHeight(cfg, "D", [dynEntry])).toBe(
      ACTIVITY_BASE_HEIGHT + PORT_ROWS_TOP_MARGIN + 2 * PORT_ROW_HEIGHT,
    );
  });
});
