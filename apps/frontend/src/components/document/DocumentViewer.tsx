...
import { IconZoomIn, IconZoomOut, IconRotateClockwise, IconEye, IconEyeOff } from '@tabler/icons-react';
...
export function DocumentViewer({
  ...
  const renderKeyValueOverlays = () => {
    if (!showOverlays || !isImageLoaded || !imageRef.current || keyValuePairs.length === 0) {
      return null;
    }
    ...
    return keyValuePairs
      .filter(kvp => kvp.key?.boundingRegions?.some(br => br.pageNumber === pageNumber))
      .map((kvp, index) => {
        // Use the bounding region for this page
        const boundingRegion = kvp.key.boundingRegions.find(br => br.pageNumber === pageNumber);
        ...
        const confidenceColor = kvp.confidence >= 0.9
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Button
            ...
            leftSection={showOverlays ? <IconEye size={16} /> : <IconEyeOff size={16} />}
          >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '0.875rem', color: '#4b5563', minWidth: '60px', textAlign: 'center' }}>
            {Math.round(zoom * 100)}%
          </span>
      ...
      >
        ...
        >
          <img
            ...
            src={imageUrl}
            alt="Document page"
            style={{ display: 'block', maxWidth: '100%', height: 'auto', boxShadow: '0 10px 25px rgba(15, 23, 42, 0.15)' }}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <span>Image: {imageDimensions.width} × {imageDimensions.height}</span>
            <span>
