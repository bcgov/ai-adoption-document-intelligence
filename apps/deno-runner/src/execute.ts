...
/**
 * Per-stream cap, applied to stdout AND stderr. Overflowing stdout is a
 * structural failure (`stdoutTooLarge` — the output is unusable anyway) and
 * kills the subprocess; overflowing stderr is merely diagnostic noise, so
 * the reader keeps DRAINING past the cap while discarding, which bounds
 * runner memory without changing the subprocess's fate.
 */
const STREAM_CAP_BYTES = 5 * 1024 * 1024; // 5 MB
...
export async function handleExecute(req: Request): Promise<Response> {
  ...
  try {
    ...
    // would deadlock against a script that fills the ~64 KB stderr pipe
    // buffer while stdout is still open (and vice versa), leaving the
    // subprocess blocked on a full pipe until the timeout killed it — and
    // mis-reported as `timedOut`.
    const [stdoutResult, stderrResult] = await Promise.all([
      readStreamWithCap(child.stdout, STREAM_CAP_BYTES),
      readStreamDiscardingOverCap(child.stderr, STREAM_CAP_BYTES),
    ]);
...
/**
 * Read a diagnostic stream keeping at most `capBytes`; past the cap, keep
 * reading and DISCARD so the pipe drains and the subprocess never blocks
 * on it. No kill, no cancel: cancelling the pipe would SIGPIPE a script
 * that is otherwise behaving, and stderr overflow is not a failure — the
 * worker only keeps a short tail of it anyway. A truncation marker is
 * appended so the loss is visible.
 */
async function readStreamDiscardingOverCap(
  stream: ReadableStream<Uint8Array>,
  capBytes: number,
): Promise<string> {
  const reader = stream.getReader();
  ...
  text += decoder.decode();
  if (truncated) {
    text += `\n[runner] stderr truncated at ${capBytes} bytes`;
  }
  return text;
}
