...
describe("BlobStorageService", () => {
  ...
  describe("validateContainerSasUrl", () => {
    it("should validate SAS URL successfully with blobs", async () => {
      const mockBlobs = [
        { name: "blob1.txt" },
        { name: "blob2.txt" },
        { name: "blob3.txt" },
      ...
      expect(result.sampleBlobs).toEqual([
        "blob1.txt",
        "blob2.txt",
        "blob3.txt",
      ]);
    ...
    it("should validate SAS URL successfully with more than 5 blobs", async () => {
      const mockBlobs = Array.from({ length: 10 }, (_, i) => ({
        name: `blob${i + 1}.txt`,
      }));
      ...
      expect(result.sampleBlobs).toHaveLength(5);
    });

    it("should return error when SAS URL validation fails", async () => {
      const mockContainerClientInstance = {
      ...
      const result = await service.validateContainerSasUrl(
        "https://storage.blob.core.windows.net/container?invalid-sas",
      );

      expect(result.canList).toBe(false);
      expect(result.error).toBe("Access denied");
      expect(result.blobCount).toBeUndefined();
      expect(result.sampleBlobs).toBeUndefined();
    });
  });

  describe("listBlobs", () => {
    it("should list all blobs in container", async () => {
      const mockBlobs = [
        {
          name: "blob1.txt",
          properties: {
      ...
      expect(result[0].name).toBe("blob1.txt");
    ...
    it("should list blobs with prefix", async () => {
      ...
      const result = await service.listBlobs("test-container", "folder/");

      expect(result).toHaveLength(1);
      expect(result[0].name).toBe("folder/blob1.txt");
      expect(mockContainerClient.listBlobsFlat).toHaveBeenCalledWith({
        prefix: "folder/",
      });
    });
    ...
    it("should throw error when listing fails", async () => {
      ...
      await expect(service.listBlobs("test-container")).rejects.toThrow(
        "List failed",
      );
    });
  });
