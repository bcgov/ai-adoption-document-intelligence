...
export class QueueService {
  ...
  async publishDocumentUploaded(message: QueueMessage): Promise<boolean> {
    ...
    try {
      ...
      this.logger.debug("=== QueueService.publishDocumentUploaded completed ===");
      return true;
    } catch (error) {
      this.logger.error(`Failed to process OCR for document ${message.documentId}: ${error.message}`);
  ...
  private async processOcrForDocument(message: QueueMessage): Promise<void> {
    this.logger.debug(`=== Starting OCR processing for document ${message.documentId} ===`);

    try {
      // Step 1: Request OCR from Azure
      this.logger.debug(`Requesting OCR from Azure for document ${message.documentId}`);
      const ocrRequest = await this.ocrService.requestOcr(message.documentId);
      this.logger.debug(`OCR request sent. Status: ${ocrRequest.status}, APIM Request ID: ${ocrRequest.apimRequestId}`);
      ...
      this.logger.debug(`=== OCR processing completed for document ${message.documentId} ===`);
    } catch (error) {
      this.logger.error(`OCR processing failed for document ${message.documentId}: ${error.message}`);
   ...
   */
  private async waitForOcrCompletion(documentId: string, maxAttempts: number = 30, delayMs: number = 2000): Promise<void> {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        this.logger.debug(`Polling OCR results attempt ${attempt}/${maxAttempts} for document ${documentId}`);
        ...
        if (ocrResults && ocrResults.content) {
          this.logger.debug(`OCR results retrieved successfully for document ${documentId}`);
          ...
          this.logger.debug(`OCR results not ready yet for document ${documentId} (results: ${ocrResults}), will retry...`);
          await new Promise(resolve => setTimeout(resolve, delayMs));
          continue;
        }
      } catch (error) {
        if (error.message.includes('not yet been sent for OCR') || error.message.includes('Failed to retrieve OCR results')) {
          // OCR still processing, wait and try again
          this.logger.debug(`OCR still processing, waiting... (${error.message})`);
          await new Promise(resolve => setTimeout(resolve, delayMs));
          continue;
        } else {
