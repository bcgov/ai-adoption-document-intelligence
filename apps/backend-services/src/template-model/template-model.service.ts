...
export class TemplateModelService {
  ...
  async generateUniqueModelId(name: string): Promise<string> {
    ...
    const existing = await this.db.findTemplateModelByModelId(base);
    if (!existing) {
      return base;
    }
    ...
    while (suffix <= 1000) {
      ...
      const exists = await this.db.findTemplateModelByModelId(candidate);
      if (!exists) {
        return candidate;
      }
      suffix++;
  ...
  async getTemplateModels(groupIds?: string[]) {
    ...
    return this.db.findAllTemplateModels(groupIds);
  }

  async createTemplateModel(dto: CreateTemplateModelDto, userId: string) {
    this.logger.debug(`Creating template model: ${dto.name}`);
    const modelId = await this.generateUniqueModelId(dto.name);
    return this.db.createTemplateModel({
      name: dto.name,
      model_id: modelId,
  ...
  async getTemplateModel(id: string) {
    ...
    const templateModel = await this.db.findTemplateModel(id);
    if (!templateModel) {
  ...
  async updateTemplateModel(id: string, dto: UpdateTemplateModelDto) {
    ...
    const templateModel = await this.db.updateTemplateModel(id, {
      ...dto,
    });
    if (!templateModel) {
      throw new NotFoundException(`Template model with id ${id} not found`);
  ...
  async deleteTemplateModel(id: string) {
    ...
    const deleted = await this.db.deleteTemplateModel(id);
    if (!deleted) {
      throw new NotFoundException(`Template model with id ${id} not found`);
    }
  ...
  async getFieldSchema(templateModelId: string) {
    ...
    const templateModel = await this.db.findTemplateModel(templateModelId);
    if (!templateModel) {
  ...
  async addField(templateModelId: string, dto: CreateFieldDefinitionDto) {
    ...
    return this.db.createFieldDefinition(templateModelId, {
  ...
  async getTemplateModelDocuments(templateModelId: string) {
    ...
    return this.db.findLabeledDocuments(templateModelId);
  }
  ...
  async addDocumentToTemplateModel(
    ...
    return this.db.addDocumentToTemplateModel(
      templateModelId,
  ...
  async getTemplateModelDocument(
    ...
    const labeledDoc = await this.db.findLabeledDocument(
      templateModelId,
      documentId,
    );
    if (!labeledDoc) {
  ...
  async removeDocumentFromTemplateModel(
    ...
    const deleted = await this.db.removeDocumentFromTemplateModel(
      templateModelId,
      documentId,
    );
    if (!deleted) {
  ...
  async saveDocumentLabels(
    ...
    return this.db.findLabeledDocument(templateModelId, documentId);
  }
  ...
  async exportTemplateModel(templateModelId: string, options: ExportDto) {
    ...
    let documents = await this.db.findLabeledDocuments(templateModelId);
  ...
  async uploadLabelingDocument(
    ...
    const labeledDoc = await this.db.addDocumentToTemplateModel(
      templateModelId,
      labelingDocument.id,
    );
