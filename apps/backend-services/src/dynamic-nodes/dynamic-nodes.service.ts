...
export interface PublishInput {
  ...
  actorUserId?: string;
  /**
   * The acting identity's actor id (`resolvedIdentity.actorId`), recorded
   * on the publish audit event. Always present — `actorUserId` above is
   * null for API-key callers, which is exactly why it cannot carry the
   * audit trail alone.
   */
  actorId: string;
}
