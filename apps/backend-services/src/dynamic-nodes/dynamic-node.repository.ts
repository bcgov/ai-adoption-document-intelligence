...
export interface CreateWithFirstVersionInput {
  ...
  ownerUserId?: string;
  /**
   * The acting identity's actor id (from `resolvedIdentity.actorId`),
   * recorded on the audit event. Always present — unlike `ownerUserId`,
   * which is null for API-key callers with no linked user.
   */
  actorId: string;
}
...
export interface PublishNewVersionInput {
  ...
  /** The acting identity's actor id — see `CreateWithFirstVersionInput`. */
  actorId: string;
}
...
export class DynamicNodeRepository {
    ...
    try {
      return await this.prismaService.transaction(async (tx) => {
        ...
        if (existing !== null) {
          ...
          // Audit inside the SAME transaction (CLAUDE.md): publishing a
          // dynamic node ships executable, network-capable code, so the
          // event must not exist without the version row or vice versa.
          await this.auditService.recordEvent(
            {
              ...
              payload: {
                slug: input.slug,
                version: headVersion.versionNumber,
                restored: true,
              },
            },
            tx,
        ...
        // Audit inside the SAME transaction — see the restore branch.
        await this.auditService.recordEvent(
          {
            event_type: "dynamic_node_published",
            resource_type: "dynamic_node",
            resource_id: dynamicNode.id,
            actor_id: input.actorId,
            group_id: input.groupId,
            payload: {
              slug: input.slug,
  ...
  async publishNewVersion(
    ...
    return this.prismaService.transaction(async (tx) => {
      ...
      // Audit inside the SAME transaction (CLAUDE.md) — a new version of
      // executable code with no audit row would be an invisible mutation.
      await this.auditService.recordEvent(
        {
  ...
  async softDelete(
    ...
    return this.prismaService.transaction(async (tx) => {
      ...
      if (lineage.deletedAt !== null) {
        // Idempotent no-op — nothing mutated, so no audit event either.
        return lineage;
      }
      const deleted = await tx.dynamicNode.update({
        where: { id: lineage.id },
        data: { deletedAt: new Date() },
      });
      // Audit inside the SAME transaction as the delete (CLAUDE.md).
      await this.auditService.recordEvent(
        {
