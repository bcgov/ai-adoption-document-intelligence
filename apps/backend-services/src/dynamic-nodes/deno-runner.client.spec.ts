import {
  ...
  DENO_RUNNER_CHECK_TIMEOUT_MS,
  DENO_RUNNER_HEALTH_TIMEOUT_MS,
  DenoRunnerClient,
...
// ── Change J: fetch timeouts ────────────────────────────────────────

describe("fetch timeouts (Change J)", () => {
  function timeoutRejectingFetch(): jest.Mock<Promise<Response>> {
    return jest.fn(async () => {
  ...
  it("check passes an abort signal so a wedged runner cannot hang the publish", async () => {
    const fetchMock = makeFetchMock(200, { ok: true, errors: [] });
    const client = new DenoRunnerClient({
      baseUrl: "http://runner.example:9090",
      fetchImpl: fetchMock as unknown as typeof fetch,
    });
    await client.check("const x = 1;");
  ...
  it("check maps a timeout to DenoRunnerUnavailableError naming the ceiling", async () => {
    const client = new DenoRunnerClient({
      baseUrl: "http://runner.example:9090",
      fetchImpl: timeoutRejectingFetch() as unknown as typeof fetch,
    });
    let thrown: unknown;
    try {
      await client.check("const x = 1;");
    ...
    expect((thrown as DenoRunnerUnavailableError).details).toContain(
      `timed out after ${DENO_RUNNER_CHECK_TIMEOUT_MS / 1000}s`,
    );
  });

  it("health maps a timeout to DenoRunnerUnavailableError with its shorter ceiling", async () => {
    const client = new DenoRunnerClient({
      baseUrl: "http://runner.example:9090",
      fetchImpl: timeoutRejectingFetch() as unknown as typeof fetch,
    });
    ...
    expect(thrown).toBeInstanceOf(DenoRunnerUnavailableError);
    expect((thrown as DenoRunnerUnavailableError).details).toContain(
      `timed out after ${DENO_RUNNER_HEALTH_TIMEOUT_MS / 1000}s`,
    );
  });
});
