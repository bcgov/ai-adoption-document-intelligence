 ...
 * endpoints, and the internal token those self-calls authenticate with.
 */
export interface AgentToolContext {
  ...
  /**
   * Short-lived internal token (Change W) minted by `AgentService` from
   * the chat request's already-resolved identity — once per agent run,
   * with a TTL covering the run budget. Sent as `x-internal-token` on
   * every HTTP self-call, uniformly for JWT- and API-key-authenticated
   * chat callers; the caller's own credential is never forwarded.
   */
...
/**
 * Ceiling on any single agent self-call (and the sample-document upload).
 * The self-called endpoints answer in seconds — starting a run returns as
 * soon as Temporal accepts it, and status/preview reads are quick queries —
 * so a socket that is still open after 30s is a wedged connection, not a
 * slow answer. Without the bound, one hung fetch stalls the agent step (and
 * the user's stream) until Node's own socket timeout.
 */
 ...
 * preview-cache). Authenticates with the run's minted internal token
 * (Change W) so it works for every chat caller — JWT/IDIR users included —
 * without forwarding the caller's own credential.
 */
async function internalFetch<T>(
  ...
  try {
    ...
    if (isAbortLikeError(err)) {
      // Same unavailable/503 contract the tool consumers already handle
      // for a backend that cannot answer.
      return {
        ok: false,
        status: 503,
        body: {
          error: "self-call-timeout",
          message: `Backend self-call did not answer within ${INTERNAL_FETCH_TIMEOUT_MS / 1000}s.`,
        },
      };
    }
    throw err;
 ...
 * `TimeoutError` (or `AbortError` on some runtimes). Matched by name so no
 * DOM lib types are needed.
 */
          ...
          if (isAbortLikeError(err)) {
            return {
              ...
              error: {
                ...
                message: `Uploading the sample timed out after ${INTERNAL_FETCH_TIMEOUT_MS / 1000}s.`,
              },
            };
          }
          throw err;
