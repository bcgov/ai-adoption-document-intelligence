...
export class AgentController {
  ...
  @ApiBadRequestResponse({
    type: AgentErrorResponseDto,
    description:
      "The requested `model` is not one this backend serves (code `unknown-model`) — the allowlist is exactly what `GET /api/agent/models` returns.",
  })
  @ApiServiceUnavailableResponse({
    type: AgentErrorResponseDto,
    description:
  ...
  async chat(
    ...
    // Change W: the caller's credential (JWT or API key) is NOT threaded
    // into the agent run — `AgentService` mints a short-lived internal
    // token from the resolved identity for the tool loop's self-calls.
...
function resolveCallerOrThrow(req: Request): ResolvedCaller {
  ...
  if (groupIds === undefined) {
    ...
    return { actorId, groupId: requestedGroup };
  }
  ...
  return { actorId, groupId: groupIds[0] };
}
