...
/**
 * The turn named a `model` that is not on the allowlist `GET
 * /api/agent/models` serves — the single configured deployment per
 * provider. Without this check any ≤200-char string passed the DTO and was
 * forwarded to the provider on the server's own key, so a caller could
 * spend against models the operator never offered. 400 rather than 503:
 * the server is fine, the request named something that does not exist here.
 */
export class AgentUnknownModelException extends BadRequestException {
  constructor(requestedModel: string, availableModels: string[]) {
    super({
      statusCode: HttpStatus.BAD_REQUEST,
      code: "unknown-model",
      message:
        `Model '${requestedModel}' is not served by this backend. ` +
