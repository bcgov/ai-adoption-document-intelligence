...
const auditRecordEvent = jest.fn().mockResolvedValue(undefined);
...
function makeService(repo: Partial<ChatRepository>): AgentService {
  ...
  return new AgentService(
    ...
    dep,
    dep,
    dep, // internalTokenService — unused by the conversation queries
    { recordEvent: auditRecordEvent } as never,
    dep,
  );
}
...
describe("AgentService — conversation queries", () => {
  ...
  it("deleteConversationForCaller deletes when the conversation is owned by the caller", async () => {
    ...
    const service = makeService({
      findConversationByIdForUser: jest
        .fn()
        .mockResolvedValue({ id: "c1", groupId: "g1" }),
      deleteConversation,
    });
    await service.deleteConversationForCaller("c1", "user-1");
    expect(deleteConversation).toHaveBeenCalledWith("c1");
    // Change H: the delete is audited after the commit, naming the actor
    // and the conversation's group.
    expect(auditRecordEvent).toHaveBeenCalledWith({
  ...
  it("a refused delete (not owner) records no audit event", async () => {
    const service = makeService({
      findConversationByIdForUser: jest.fn().mockResolvedValue(null),
    });
    await expect(
      service.deleteConversationForCaller("c1", "someone-else"),
    ).rejects.toBeInstanceOf(NotFoundException);
