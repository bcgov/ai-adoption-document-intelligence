...
**Build order.** Create the workflow (it seeds a \`source.upload\` entry node), add activities in dependency order, connect them with edges (\`connectNodes\`), set the entry node. Two different binding rules — getting these right is the #1 thing that makes a workflow actually run:
- **Activity→activity hops auto-wire.** Connecting two activities with an edge auto-binds the downstream typed input to the upstream output on save. Don't set those bindings by hand.
- **The source→first-activity hop does NOT auto-wire — bind it yourself.** \`source.upload\` writes the uploaded file to a ctx key (default \`documentUrl\`; confirm via the node's \`ctxKey\` param). \`declareCtx\` that key, then bind the first activity's document/file input to it (e.g. \`file.prepare\`'s \`blobKey\` ← \`documentUrl\`) via \`connectNodes\`'s binding argument. Without this the graph validates but fails at runtime.
- Beyond that, use \`describeNode\` on each node to set required PARAMETERS (e.g. the status \`document.updateStatus\` writes) — parameters are values you set, not bindings. Don't \`declareCtx\` or hand-bind anything auto-wire already covers.
