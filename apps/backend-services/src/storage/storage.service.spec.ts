...
describe("StorageService", () => {
  ...
  describe("ensureStorageDirectory", () => {
    ...
    it("should throw and log error if mkdir fails", async () => {
      ...
      jest
        .spyOn(fs.promises, "mkdir")
        .mockRejectedValue(new Error("fail mkdir"));
  ...
  describe("saveFile", () => {
    ...
    it("should throw if writeFile fails", async () => {
      ...
      jest
        ...
        .mockRejectedValue(new Error("fail write"));
      const buffer = Buffer.from("test");
      await expect(service.saveFile("test.txt", buffer)).rejects.toThrow(
        "fail write",
      );
    });
  });
  ...
  describe("saveFilesBulk", () => {
    ...
    it("should throw if writeFile fails for any file", async () => {
      ...
      jest
        .spyOn(fs.promises, "writeFile")
        .mockRejectedValue(new Error("fail bulk"));
      const files = [{ originalname: "a.txt", buffer: Buffer.from("a") }];
      await expect(
        service.saveFilesBulk(files as any, "folder"),
      ).rejects.toThrow("fail bulk");
    });
  });
  ...
  describe("readFile", () => {
    ...
    it("should throw if file does not exist", async () => {
      jest
        .spyOn(fs.promises, "readFile")
        .mockRejectedValue(new Error("fail read"));
  ...
  describe("deleteFile", () => {
    ...
    it("should throw if unlink fails", async () => {
      jest
        ...
        .mockRejectedValue(new Error("fail unlink"));
  ...
  describe("deleteFolderRecursive", () => {
    it("should delete a folder recursively", async () => {
      ...
      const loggerLog = jest
        ...
        .mockImplementation();
      await expect(
        service.deleteFolderRecursive("folder"),
