  ...
  describe("getTrainingResult", () => {
   ...
   describe("getTrainingResult error branches", () => {
              it("should throw BadRequestException if name or group_id is missing", async () => {
                const req = { user: { sub: "user1" } };
                await expect(controller.getTrainingResult(req, { name: null, group_id: null })).rejects.toThrow();
              });
              it("should throw ForbiddenException if user not in group", async () => {
                databaseService.isUserInGroup.mockResolvedValue(false);
                const req = { user: { sub: "user1" } };
                await expect(controller.getTrainingResult(req, { name: "c1", group_id: "g1" })).rejects.toThrow(ForbiddenException);
              });
              it("should throw NotFoundException if classifier not found", async () => {
                databaseService.isUserInGroup.mockResolvedValue(true);
                databaseService.getClassifierModel.mockResolvedValue(null);
                const req = { user: { sub: "user1" } };
                await expect(controller.getTrainingResult(req, { name: "c1", group_id: "g1" })).rejects.toThrow(NotFoundException);
              });
              it("should throw error if operation_location missing", async () => {
                databaseService.isUserInGroup.mockResolvedValue(true);
                databaseService.getClassifierModel.mockResolvedValue({ operation_location: null });
                const req = { user: { sub: "user1" } };
                await expect(controller.getTrainingResult(req, { name: "c1", group_id: "g1" })).rejects.toThrow();
              });
              it("should throw error if pollOperationUntilResolved fails", async () => {
                databaseService.isUserInGroup.mockResolvedValue(true);
                databaseService.getClassifierModel.mockResolvedValue({ operation_location: "loc" });
                const req = { user: { sub: "user1" } };
                azureService.pollOperationUntilResolved.mockImplementation(
                  async (_loc, _onSuccess, onFailure) => onFailure({ error: { code: "fail", message: "fail" } })
                );
                await expect(controller.getTrainingResult(req, { name: "c1", group_id: "g1" })).rejects.toThrow(
                  "Could not retrieve status of classifier. Code: fail. Message: fail"
                );
          ...
          describe("deleteClassifierDocuments error handling", () => {
            it("should throw InternalServerErrorException if deleteFolderRecursive throws", async () => {
              ...
              storageService.deleteFolderRecursive.mockImplementation(() => { throw new Error("fail"); });
              const req = { user: { sub: "user1" } };
              const query = { name: "c1", group_id: "g1", folder: "f1" };
              await expect(controller.deleteClassifierDocuments(req, query)).rejects.toThrow(InternalServerErrorException);
            });
          });
        describe("deleteClassifierDocuments", () => {
          it("should delete a specific folder if folder param is provided", async () => {
            ...
            const req = { user: { sub: "user1" } };
            const query = { name: "c1", group_id: "g1", folder: "f1" };
            await expect(controller.deleteClassifierDocuments(req, query)).resolves.toBeUndefined();
            expect(storageService.getStoragePath).toHaveBeenCalledWith("g1", expect.anything(), "c1/f1");
            expect(storageService.deleteFolderRecursive).toHaveBeenCalledWith("/path/to/folder");
          });
        });
      describe("getClassifierDocuments", () => {
        it("should return documents if user in group and classifier exists", async () => {
          ...
          databaseService.getClassifierModel.mockResolvedValue({ id: "1" });
          storageService.getStoragePath.mockReturnValue("/path");
          storageService.listBlobsInFolder = jest.fn().mockResolvedValue(["doc1", "doc2"]);
          const req = { user: { sub: "user1" } };
          const query = { name: "c1", group_id: "g1" };
        ...
        it("should throw ForbiddenException if user not in group", async () => {
          ...
          const req = { user: { sub: "user1" } };
          const query = { name: "c1", group_id: "g1" };
          await expect(controller.getClassifierDocuments(req, query)).rejects.toThrow(ForbiddenException);
        });
        it("should throw NotFoundException if classifier does not exist", async () => {
          databaseService.isUserInGroup.mockResolvedValue(true);
          databaseService.getClassifierModel.mockResolvedValue(null);
          const req = { user: { sub: "user1" } };
          const query = { name: "c1", group_id: "g1" };
          await expect(controller.getClassifierDocuments(req, query)).rejects.toThrow(NotFoundException);
        });
      });
    describe("updateClassifier", () => {
      it("should update a classifier if user is in group and classifier exists", async () => {
        databaseService.isUserInGroup.mockResolvedValue(true);
        databaseService.getClassifierModel.mockResolvedValue({ id: "1" });
        databaseService.updateClassifierModel.mockResolvedValue({ id: "1", description: "new desc" });
        const req = { user: { sub: "user1" } };
      ...
      it("should throw ForbiddenException if user not in group", async () => {
        ...
        await expect(controller.updateClassifier(req, body)).rejects.toThrow(ForbiddenException);
      });
      it("should throw NotFoundException if classifier does not exist", async () => {
        databaseService.isUserInGroup.mockResolvedValue(true);
        databaseService.getClassifierModel.mockResolvedValue(null);
        const req = { user: { sub: "user1" } };
        const body = { name: "c1", group_id: "g1", description: "desc" };
        await expect(controller.updateClassifier(req, body)).rejects.toThrow(NotFoundException);
      });
    });
});
