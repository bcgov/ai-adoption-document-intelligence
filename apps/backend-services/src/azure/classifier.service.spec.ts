...
const mockBlobService = {
    getContainerClient: jest.fn().mockReturnValue({ listBlobsByHierarchy: jest.fn() }),
...
describe('ClassifierService', () => {
    ...
    describe('requestClassifierTraining', () => {
        ...
        it('should call updateClassifierModel if training request accepted', async () => {
            ...
            (databaseService.getClassifierModel as jest.Mock).mockResolvedValue({ description: 'desc' });
            (blobService.getContainerClient as jest.Mock).mockReturnValue({
                listBlobsByHierarchy: jest.fn().mockReturnValue([]),
            });
            (databaseService.updateClassifierModel as jest.Mock).mockResolvedValue({ status: ClassifierStatus.TRAINING });
        ...
        it('should throw an error if response is missing headers', async () => {
            ...
            await expect(service.requestClassifierTraining('c', 'g', 'u')).rejects.toThrow();
        });
    });
    ...
    describe('createLayoutJson', () => {
        it('should skip non-image files', async () => {
            ...
            (blobService.getContainerClient as jest.Mock).mockReturnValue({ getBlockBlobClient: jest.fn() });
            blobService.getBlobSasUrl = mockGetBlobSasUrl;
            (service as any).client = { path: jest.fn() };
            await expect(service.createLayoutJson(['file.txt'])).resolves.toBeUndefined();
            expect(mockGetBlobSasUrl).not.toHaveBeenCalled();
        });

        xit('should process image files (jpg)', async () => {
            const mockGetBlobSasUrl = jest.fn().mockReturnValue('https://mockbloburl/file.jpg');
            const mockGetBlockBlobClient = jest.fn().mockReturnValue({});
            (blobService.getContainerClient as jest.Mock).mockReturnValue({ getBlockBlobClient: mockGetBlockBlobClient });
            blobService.getBlobSasUrl = mockGetBlobSasUrl;
            ...
            await expect(service.createLayoutJson(['file.jpg'])).resolves.toBeUndefined();
            expect(mockGetBlobSasUrl).toHaveBeenCalledWith('classification', 'file.jpg');
    ...
    describe('uploadDocumentsForTraining', () => {
        it('should upload all files and return results', async () => {
            (blobService.ensureContainerExists as jest.Mock).mockResolvedValue(undefined);
            (storageService.getStoragePath as jest.Mock).mockReturnValue('/mock/path');
            (storageService.getAllFileNamesAndPaths as jest.Mock).mockResolvedValue([
                { path: '/mock/path/file1.pdf', name: 'file1.pdf' },
            ]);
            (storageService.readFile as jest.Mock).mockResolvedValue(Buffer.from('test'));
            (blobService.uploadFile as jest.Mock).mockResolvedValue(undefined);
    ...
    describe('generateTrainingConfig', () => {
        it('should generate config with docTypes', async () => {
            ...
            const config = await service.generateTrainingConfig('gid', 'cid', 'desc', 'url');
    ...
    describe('requestClassificationFromFile', () => {
        it('should return operation location on 202', async () => {
            ...
            const result = await service.requestClassificationFromFile(file, 'cid', 'gid');
            expect(result.status).toBe('202');
            expect(result.content).toBe('loc');
        });
        it('should return error on non-202', async () => {
