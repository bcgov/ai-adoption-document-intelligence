...
describe("AzureService", () => {
    ...
    beforeEach(async () => {
        configService = {
            get: jest.fn((key: string) => {
                if (key === "AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT") return "https://test.com";
    ...
    describe("checkOperationStatus", () => {
        it("should call fetch with correct headers", async () => {
            const mockFetch = jest.fn().mockResolvedValue({ json: jest.fn().mockResolvedValue({ status: "succeeded" }) });
            // @ts-ignore
            global.fetch = mockFetch;
            const url = "https://test.com/operation";
            await service.checkOperationStatus(url);
            expect(mockFetch).toHaveBeenCalledWith(url, { headers: { "api-key": "secret-key" } });
        });
    });
    ...
    describe("pollOperationUntilResolved", () => {
        it("should call onSuccess when status is succeeded", async () => {
            const url = "https://test.com/operation";
            const pollResp = { json: jest.fn().mockResolvedValue({ status: "succeeded" }) };
            const mockFetch = jest.fn().mockResolvedValue(pollResp);
            // @ts-ignore
            service.checkOperationStatus = mockFetch;
            const onSuccess = jest.fn();
            await service.pollOperationUntilResolved(url, onSuccess, undefined, { maxRetries: 2, intervalMs: 1 });
            expect(onSuccess).toHaveBeenCalledWith({ status: "succeeded" });
        });

        it("should call onFailure when status is failed", async () => {
            const url = "https://test.com/operation";
            const pollResp = { json: jest.fn().mockResolvedValue({ status: "failed" }) };
            const mockFetch = jest.fn().mockResolvedValue(pollResp);
            // @ts-ignore
            service.checkOperationStatus = mockFetch;
            const onFailure = jest.fn();
            await service.pollOperationUntilResolved(url, jest.fn(), onFailure, { maxRetries: 2, intervalMs: 1 });
        ...
        it("should retry until maxRetries if status is not succeeded or failed", async () => {
            const url = "https://test.com/operation";
            const pollResp = { json: jest.fn().mockResolvedValue({ status: "notStarted" }) };
            const mockFetch = jest.fn().mockResolvedValue(pollResp);
            // @ts-ignore
            service.checkOperationStatus = mockFetch;
            const onSuccess = jest.fn();
            const onFailure = jest.fn();
            await service.pollOperationUntilResolved(url, onSuccess, onFailure, { maxRetries: 2, intervalMs: 1 });
