import { GroupRole, type $Enums } from "@generated/client";
...
describe("GroupDbService", () => {
  ...
  describe("findGroup", () => {
    it("returns a group (no tx)", async () => {
      const g = makeGroup(); mockPrisma.group.findUnique.mockResolvedValue(g);
      expect(await service.findGroup("g-1")).toEqual(g);
    });
    it("uses tx client", async () => {
      const g = makeGroup();
      const txGroup = { findUnique: jest.fn().mockResolvedValue(g) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.findGroup>[1];
  ...
  describe("findActiveGroup", () => {
    it("returns active group (no tx)", async () => {
      ...
      expect(mockPrisma.group.findUnique).toHaveBeenCalledWith({ where: { id: "g-1", deleted_at: null } });
    });
    it("uses tx client", async () => {
      const txGroup = { findUnique: jest.fn().mockResolvedValue(null) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.findActiveGroup>[1];
  ...
  describe("findGroupByName", () => {
    ...
    it("uses tx client", async () => {
      const txGroup = { findUnique: jest.fn().mockResolvedValue(null) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.findGroupByName>[1];
  ...
  describe("findActiveGroupByNameExcluding", () => {
    it("queries with exclusion (no tx)", async () => {
      const g = makeGroup(); mockPrisma.group.findFirst.mockResolvedValue(g);
      expect(await service.findActiveGroupByNameExcluding("G", "other")).toEqual(g);
    ...
    it("uses tx client", async () => {
      ...
      const tx = { group: txGroup } as unknown as Parameters<typeof service.findActiveGroupByNameExcluding>[2];
  ...
  describe("findAllGroups", () => {
    ...
    it("uses tx client", async () => {
      ...
      const tx = { group: txGroup } as unknown as Parameters<typeof service.findAllGroups>[0];
  ...
  describe("createGroup", () => {
    it("creates without description (no tx)", async () => {
      ...
      expect(mockPrisma.group.create).toHaveBeenCalledWith({ data: { name: "N" }, select: { id: true, name: true, description: true } });
    });
    it("creates with description", async () => {
      mockPrisma.group.create.mockResolvedValue({ id: "g-1", name: "N", description: "d" });
      await service.createGroup("N", "d");
      expect(mockPrisma.group.create).toHaveBeenCalledWith({ data: { name: "N", description: "d" }, select: { id: true, name: true, description: true } });
    });
    it("uses tx client", async () => {
      const txGroup = { create: jest.fn().mockResolvedValue({ id: "g-1", name: "X", description: null }) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.createGroup>[2];
  ...
  describe("updateGroupData", () => {
    it("updates group (no tx)", async () => {
      ...
      expect(await service.updateGroupData("g-1", { name: "U", description: null, updated_by: "u" })).toEqual(g);
    });
    it("uses tx client", async () => {
      const txGroup = { update: jest.fn().mockResolvedValue({ id: "g-1", name: "U", description: null }) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.updateGroupData>[2];
      await service.updateGroupData("g-1", { name: "U", description: null, updated_by: "u" }, tx);
  ...
  describe("softDeleteGroup", () => {
    it("soft deletes group (no tx)", async () => {
      ...
      expect(mockPrisma.group.update).toHaveBeenCalledWith({ where: { id: "g-1" }, data: expect.objectContaining({ deleted_by: "user-1" }) });
    });
    it("uses tx client", async () => {
      const txGroup = { update: jest.fn().mockResolvedValue({}) };
      const tx = { group: txGroup } as unknown as Parameters<typeof service.softDeleteGroup>[2];
  ...
  describe("findUsersGroups", () => {
    it("returns user groups (no tx)", async () => {
      const ug = [makeUserGroup()]; mockPrisma.userGroup.findMany.mockResolvedValue(ug);
      expect(await service.findUsersGroups("user-1")).toEqual(ug);
    });
    it("uses tx client", async () => {
      const txUG = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findUsersGroups>[1];
  ...
  describe("findUserAdminMemberships", () => {
    it("returns ADMIN memberships (no tx)", async () => {
      const ug = [{ ...makeUserGroup(), role: GroupRole.ADMIN }]; mockPrisma.userGroup.findMany.mockResolvedValue(ug);
      expect(await service.findUserAdminMemberships("user-1")).toEqual(ug);
    });
    it("uses tx client", async () => {
      const txUG = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findUserAdminMemberships>[1];
  ...
  describe("findUserGroupsWithGroup", () => {
    it("returns user groups with group data (no tx)", async () => {
      const records = [{ ...makeUserGroup(), group: makeGroup() }]; mockPrisma.userGroup.findMany.mockResolvedValue(records);
      expect(await service.findUserGroupsWithGroup("user-1")).toEqual(records);
    });
    it("uses tx client", async () => {
      const txUG = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findUserGroupsWithGroup>[1];
  ...
  describe("findUserGroupsInGroups", () => {
    it("returns filtered groups (no tx)", async () => {
      const records = [{ ...makeUserGroup(), group: makeGroup() }]; mockPrisma.userGroup.findMany.mockResolvedValue(records);
      expect(await service.findUserGroupsInGroups("user-1", ["g-1"])).toEqual(records);
    });
    it("uses tx client", async () => {
      const txUG = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findUserGroupsInGroups>[2];
  ...
  describe("isUserInGroup", () => {
    ...
    it("uses tx client", async () => {
      ...
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.isUserInGroup>[2];
  ...
  describe("findUserGroupMembership", () => {
    it("returns UserGroup record (no tx)", async () => {
      const ug = makeUserGroup(); mockPrisma.userGroup.findUnique.mockResolvedValue(ug);
      expect(await service.findUserGroupMembership("user-1", "g-1")).toEqual(ug);
    });
    it("uses tx client", async () => {
      const txUG = { findUnique: jest.fn().mockResolvedValue(null) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findUserGroupMembership>[2];
  ...
  describe("upsertUserGroup", () => {
    it("upserts record (no tx)", async () => {
      ...
      expect(mockPrisma.userGroup.upsert).toHaveBeenCalledWith(expect.objectContaining({ where: { user_id_group_id: { user_id: "user-1", group_id: "g-1" } } }));
    });
    it("uses tx client", async () => {
      const txUG = { upsert: jest.fn().mockResolvedValue(makeUserGroup()) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.upsertUserGroup>[2];
  ...
  describe("deleteUserGroup", () => {
    it("deletes record (no tx)", async () => {
      ...
      expect(mockPrisma.userGroup.delete).toHaveBeenCalledWith({ where: { user_id_group_id: { user_id: "user-1", group_id: "g-1" } } });
    });
    it("uses tx client", async () => {
      const txUG = { delete: jest.fn().mockResolvedValue(makeUserGroup()) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.deleteUserGroup>[2];
  ...
  describe("findGroupMembersWithUser", () => {
    it("returns members with user data (no tx)", async () => {
      const records = [{ ...makeUserGroup(), user: { id: "user-1" } }]; mockPrisma.userGroup.findMany.mockResolvedValue(records);
      expect(await service.findGroupMembersWithUser("g-1")).toEqual(records);
    });
    it("uses tx client", async () => {
      const txUG = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { userGroup: txUG } as unknown as Parameters<typeof service.findGroupMembersWithUser>[1];
  ...
  describe("isUserSystemAdmin", () => {
    ...
    it("uses tx client", async () => {
      const txUser = { findUnique: jest.fn().mockResolvedValue({ is_system_admin: true }) };
      const tx = { user: txUser } as unknown as Parameters<typeof service.isUserSystemAdmin>[1];
  ...
  describe("findMembershipRequest", () => {
    it("returns request (no tx)", async () => {
      const req = makeRequest(); mockPrisma.groupMembershipRequest.findUnique.mockResolvedValue(req);
      expect(await service.findMembershipRequest("req-1")).toEqual(req);
    });
    it("uses tx client", async () => {
      const txReq = { findUnique: jest.fn().mockResolvedValue(null) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.findMembershipRequest>[1];
      await service.findMembershipRequest("req-1", tx);
      expect(txReq.findUnique).toHaveBeenCalled();
      expect(mockPrisma.groupMembershipRequest.findUnique).not.toHaveBeenCalled();
    });
  });

  describe("findPendingMembershipRequest", () => {
    it("returns pending request (no tx)", async () => {
      const req = makeRequest(); mockPrisma.groupMembershipRequest.findFirst.mockResolvedValue(req);
      expect(await service.findPendingMembershipRequest("user-1", "g-1")).toEqual(req);
    });
    it("uses tx client", async () => {
      const txReq = { findFirst: jest.fn().mockResolvedValue(null) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.findPendingMembershipRequest>[2];
      await service.findPendingMembershipRequest("user-1", "g-1", tx);
      expect(txReq.findFirst).toHaveBeenCalled();
      expect(mockPrisma.groupMembershipRequest.findFirst).not.toHaveBeenCalled();
    });
  });

  describe("createMembershipRequest", () => {
    it("creates PENDING request (no tx)", async () => {
      const req = makeRequest(); mockPrisma.groupMembershipRequest.create.mockResolvedValue(req);
      expect(await service.createMembershipRequest("user-1", "g-1")).toEqual(req);
      expect(mockPrisma.groupMembershipRequest.create).toHaveBeenCalledWith({ data: expect.objectContaining({ status: "PENDING" }) });
    });
    it("uses tx client", async () => {
      const txReq = { create: jest.fn().mockResolvedValue(makeRequest()) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.createMembershipRequest>[2];
  ...
  describe("updateMembershipRequest", () => {
    it("updates request (no tx)", async () => {
      ...
      await service.updateMembershipRequest("req-1", { status: "APPROVED" as $Enums.GroupMembershipRequestStatus });
      expect(mockPrisma.groupMembershipRequest.update).toHaveBeenCalledWith({ where: { id: "req-1" }, data: { status: "APPROVED" } });
    });
    it("uses tx client", async () => {
      const txReq = { update: jest.fn().mockResolvedValue(makeRequest()) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.updateMembershipRequest>[2];
  ...
  describe("approveRequestTransaction", () => {
    it("uses $transaction when no tx", async () => {
      ...
      await service.approveRequestTransaction("user-1", "g-1", "req-1", { status: "APPROVED" as $Enums.GroupMembershipRequestStatus });
    ...
    it("uses tx client directly", async () => {
      ...
      const tx = { userGroup: txUG, groupMembershipRequest: txReq } as unknown as Parameters<typeof service.approveRequestTransaction>[4];
  ...
  describe("findGroupMembershipRequests", () => {
    it("returns all requests without status filter (no tx)", async () => {
      const reqs = [makeRequest()]; mockPrisma.groupMembershipRequest.findMany.mockResolvedValue(reqs);
      expect(await service.findGroupMembershipRequests("g-1")).toEqual(reqs);
      expect(mockPrisma.groupMembershipRequest.findMany).toHaveBeenCalledWith({ where: { group_id: "g-1" }, include: { user: true } });
    });
    it("applies status filter when provided", async () => {
      mockPrisma.groupMembershipRequest.findMany.mockResolvedValue([]);
      await service.findGroupMembershipRequests("g-1", "PENDING" as $Enums.GroupMembershipRequestStatus);
      expect(mockPrisma.groupMembershipRequest.findMany).toHaveBeenCalledWith({ where: { group_id: "g-1", status: "PENDING" }, include: { user: true } });
    });
    it("uses tx client", async () => {
      const txReq = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.findGroupMembershipRequests>[2];
  ...
  describe("findUserMembershipRequests", () => {
    it("returns all requests without status filter (no tx)", async () => {
      ...
      expect(mockPrisma.groupMembershipRequest.findMany).toHaveBeenCalledWith({ where: { user_id: "user-1" }, include: { group: { select: { name: true } } } });
    });
    it("applies status filter when provided", async () => {
      mockPrisma.groupMembershipRequest.findMany.mockResolvedValue([]);
      await service.findUserMembershipRequests("user-1", "PENDING" as $Enums.GroupMembershipRequestStatus);
      expect(mockPrisma.groupMembershipRequest.findMany).toHaveBeenCalledWith({ where: { user_id: "user-1", status: "PENDING" }, include: { group: { select: { name: true } } } });
    });
    it("uses tx client", async () => {
      const txReq = { findMany: jest.fn().mockResolvedValue([]) };
      const tx = { groupMembershipRequest: txReq } as unknown as Parameters<typeof service.findUserMembershipRequests>[2];
