 ...
 * Validates step configurations before saving workflows
 * This mirrors the validation logic in the temporal package
 */

import { VALID_WORKFLOW_STEP_IDS } from "../temporal/workflow-constants";
...
export function validateWorkflowConfig(config: WorkflowStepsConfig): {
  valid: boolean;
  ...
  for (const stepId of Object.keys(config)) {
    if (
      !validStepIds.includes(stepId as (typeof VALID_WORKFLOW_STEP_IDS)[number])
    ) {
      errors.push({
        stepId,
    ...
    if (stepConfig.parameters) {
      if (stepId === "pollOCRResults") {
        const params = stepConfig.parameters as {
          maxRetries?: number;
          waitBeforeFirstPoll?: number;
          waitBetweenPolls?: number;
        };
        if (
          params.maxRetries !== undefined &&
          (params.maxRetries < 1 || params.maxRetries > 100)
        ) {
          errors.push({
            stepId,
            field: "maxRetries",
        ...
        if (
          params.waitBeforeFirstPoll !== undefined &&
          params.waitBeforeFirstPoll < 0
        ) {
          errors.push({
            stepId,
        ...
        if (
          params.waitBetweenPolls !== undefined &&
          params.waitBetweenPolls < 0
        ) {
          errors.push({
            stepId,
      ...
      if (stepId === "checkOcrConfidence") {
        const params = stepConfig.parameters as { threshold?: number };
        if (
          params.threshold !== undefined &&
          (params.threshold < 0 || params.threshold > 1)
        ) {
          errors.push({
            stepId,
            field: "threshold",
      ...
      if (stepId === "humanReview") {
        const params = stepConfig.parameters as { timeout?: number };
