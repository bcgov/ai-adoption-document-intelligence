...
describe("executeCachedActivity (US-132)", () => {
  ...
  it("Change N — a failed initial findFresh is treated as a MISS, never failing the node (best-effort read)", async () => {
    const { deps, findFresh, upsert } = makeDeps();
    // A cache-DB outage that outlasts the proxy's own retries surfaces
    // here as a rejection. The contract says the cache is best-effort on
    // BOTH sides: the read failure must degrade to a miss, the activity
    // must run, and the write is still attempted.
    findFresh.mockRejectedValue(new Error("cache DB unreachable"));
    upsert.mockResolvedValue(undefined);

    const node = makeCacheableNode();
    const ctx: Record<string, unknown> = {};
    const rawExecute = jest.fn().mockResolvedValue({ out: "computed" });
    ...
    expect(result.cacheHit).toBe(false);
    expect(rawExecute).toHaveBeenCalledTimes(1);
    expect(ctx.out).toBe("computed");
    // The miss path still tries to publish the fresh result.
    expect(upsert).toHaveBeenCalledTimes(1);
  });
