...
// All tests pre-populate the LRU cache, so the script lookup never touches
// prisma. The stub carries the `internalToken` delegate the Change-W mint
// uses, so the token flow is observable without a real DB (the token itself
// is never validated by the runner — only the backend does that).
const internalTokenCreate = jest
  .fn()
  .mockImplementation(async () => ({ id: "tok-int-1" }));
const internalTokenDelete = jest.fn().mockImplementation(async () => ({}));
const stubPrisma = {
  internalToken: { create: internalTokenCreate, delete: internalTokenDelete },
...
`;
    ...
    // Item 4 / Change W (security): AI_DI_API_KEY is a short-lived internal
    // token the activity mints per invocation — never part of the activity
    // input (which would put it in Temporal's durable history), and never a
    // shared platform key.
    internalTokenCreate.mockClear();
    internalTokenDelete.mockClear();
    const result = await dynRun(
      {
        slug: "test-node",
        versionId: "v-env",
    ...
    // The RAW minted token reached the sandbox (the script read it back)...
    const rawToken = env.AI_DI_API_KEY;
    expect(rawToken).toMatch(/^[A-Za-z0-9_-]{43}$/);
    // ...only its SHA-256 hash was stored, in a row scoped to the invoking
    // group with purpose "dyn-run" and no user binding...
    ...
    expect(createArg.data.groupId).toBe("g-int");
    expect(createArg.data.userId).toBeNull();
    expect(createArg.data.purpose).toBe("dyn-run");
    // ...and the row was deleted once the invocation finished.
