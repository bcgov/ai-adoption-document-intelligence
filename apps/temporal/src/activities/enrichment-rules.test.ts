...
function kvp(key: string, value: string, confidence: number): KeyValuePair {
  return {
    ...
    value: { content: value, boundingRegions: [emptyRegion], spans: [emptySpan] },
    confidence,
  };
}
...
describe('applyRulesToValue', () => {
  it('applies trim and date normalization for date type', () => {
    ...
    const { value, changes } = applyRulesToValue('Date', '  01/15/2024  ', fieldMap);
    expect(value).toBe('2024-01-15');
    expect(changes.length).toBeGreaterThanOrEqual(1);
  });

  it('applies trim and number normalization for number type', () => {
    const fieldMap = buildFieldMap([{ field_key: 'Amount', field_type: 'number' }]);
    const { value, changes } = applyRulesToValue('Amount', '  $ 100  ', fieldMap);
    expect(value).toBe('100');
    expect(changes.length).toBeGreaterThanOrEqual(1);
  });
});
...
describe('applyRules', () => {
  it('applies rules to keyValuePairs and returns changes', () => {
    ...
    const { ocrResult: result, changes, rulesApplied } = applyRules(ocrResult, fieldMap);
...
describe('mergeKeyValuePairs', () => {
  it('overlays overlay onto base by key', () => {
    ...
    const byKey = Object.fromEntries(merged.map((p) => [p.key.content, p.value?.content]));
