...
(mockedAxios as { isAxiosError: (p: unknown) => boolean }).isAxiosError = (
  p: unknown,
) =>
  typeof p === "object" &&
  p !== null &&
...
describe("redactPiiInText", () => {
  ...
  it("leaves other text unchanged", () => {
    expect(redactPiiInText("Applicant Print Name")).toBe(
...
describe("callAzureOpenAI", () => {
  ...
  it("throws when response missing content", async () => {
    ...
    await expect(
      callAzureOpenAI({ extractedText: "", fields: [] }, "gpt-4o", {
