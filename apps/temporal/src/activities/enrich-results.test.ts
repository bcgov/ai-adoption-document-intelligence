...
import { enrichResults, type EnrichResultsParams } from './enrich-results';
    ...
    keyValuePairs: [
      {
        ...
        value: { content: '2O24-0l-15', boundingRegions: [{ pageNumber: 1, polygon: [] }], spans: [{ offset: 0, length: 0 }] },
        confidence: 0.9,
      },
      {
        key: { content: 'Amount', boundingRegions: [], spans: [] },
        value: { content: '$ 1,234.56', boundingRegions: [{ pageNumber: 1, polygon: [] }], spans: [{ offset: 0, length: 0 }] },
        confidence: 0.8,
      },
    ],
    sections: [],
...
function projectWithSchema(fieldSchema: Array<{ field_key: string; field_type: string; field_format?: string | null }>) {
  return {
    id: 'project-1',
...
describe('enrichResults activity', () => {
  ...
  describe('when enableLlmEnrichment is true', () => {
    it('does not call LLM when no Azure env vars are set', async () => {
      ...
      if (origDeployment !== undefined) process.env.AZURE_OPENAI_DEPLOYMENT = origDeployment;
      callSpy.mockRestore();
    });

    it('calls LLM when Azure env set and merges corrected values into result', async () => {
      const callSpy = jest.spyOn(enrichmentLlm, 'callAzureOpenAI').mockResolvedValue({
        correctedValues: { Date: '2024-01-15' },
        summary: 'Corrected date.',
        changes: [
          { fieldKey: 'Date', originalValue: '2O24-0l-15', correctedValue: '2024-01-15', reason: 'OCR fix' },
        ],
      });
      const origEndpoint = process.env.AZURE_OPENAI_ENDPOINT;
      const origKey = process.env.AZURE_OPENAI_API_KEY;
    ...
    it('on LLM error returns rule result only and does not throw', async () => {
      jest.spyOn(enrichmentLlm, 'callAzureOpenAI').mockRejectedValue(new Error('API error'));
      process.env.AZURE_OPENAI_ENDPOINT = 'https://example.openai.azure.com';
      process.env.AZURE_OPENAI_API_KEY = 'key';
      process.env.AZURE_OPENAI_DEPLOYMENT = 'gpt-4o';
  ...
  describe('error handling', () => {
    it('on database error returns original ocrResult and summary null', async () => {
      prismaMock.labelingProject.findUnique.mockRejectedValue(new Error('Database connection failed'));
      const ocrResult = minimalOcrResult();
      const params: EnrichResultsParams = {
        documentId: 'doc-1',
        ocrResult,
