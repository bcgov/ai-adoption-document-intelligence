 ...
 *
 * Also seeds:
 *   - A `BenchmarkProject` ("Extraction Experiments") that all 5 experiment
 *     branches add their `BenchmarkDefinition` rows to.
 *   - A `Split` per local dataset version containing every sample, type=test,
 *     so each experiment's `BenchmarkDefinition` can reference it.
 *
...
function localDatasetSplitId(folder: string, visibility: string): string {
  return `seed-local-${folder}-${visibility}-split`;
}
...
async function seedLocalDatasets() {
  ...
  for (const entry of parsed) {
    ...
    // Read the manifest to extract sample IDs for the Split row.
    // The local manifest format is the user-friendly one (with possible
    // metadata.autoGenerated marker); samples[].id is the field-key-value JSON
    // sample ID we need.
    const manifestAbsPath = path.resolve(repoRoot, entry.manifestPath);
    let sampleIds: string[] = [];
    try {
      const manifestContent = JSON.parse(
        fs.readFileSync(manifestAbsPath, "utf-8"),
      );
      if (Array.isArray(manifestContent.samples)) {
        sampleIds = manifestContent.samples
          .map((s: { id?: unknown }) => (typeof s.id === "string" ? s.id : null))
          .filter((id: string | null): id is string => id !== null);
      }
    } catch (err) {
      console.warn(
        `  ⚠ Could not read manifest at ${manifestAbsPath} for split: ${(err as Error).message}`,
      );
    }

    const splitId = localDatasetSplitId(entry.folder, entry.visibility);
    await prisma.split.upsert({
      where: { id: splitId },
      update: {
        name: "test",
    ...
    console.log(
      `  ✓ ${entry.datasetName} / ${entry.visibility} (${entry.sampleCount} samples, split ${splitId})`,
    );
  }
