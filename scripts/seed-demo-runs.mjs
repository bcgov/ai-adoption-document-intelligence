 ...
 * what every later `/tries` call on that document replays.
 */
...
/**
 * `POST /:id/tries` — a canvas Try (`RunTrigger = "try"`); cancels the
 * lineage's previous still-running Try. Every seeded run is a Try: the
 * output cache and the per-run endpoints are Try-scoped, and the run
 * history the demo guide points at shows Tries. `workflowVersionId` is
 * optional and defaults to the head version.
 */
async function startTry(workflowId, initialCtx, workflowVersionId) {
  const body = { initialCtx };
  if (workflowVersionId) body.workflowVersionId = workflowVersionId;
  const res = await api("POST", `/api/workflows/${workflowId}/tries`, body);
 ...
 * with the SAME ctx is what produces the cache hit — the cache only serves
 * Try runs, `file.prepare` is a cacheable activity, and the second Try's
 * `(configHash, inputHash)` matches the row the first one wrote, so the
 * node comes back `skipped`.
 */
async function seedTryPreviewRuns(wf) {
  ...
  const cachedRunId = await startTry(wf.id, ctx);
  ...
  const failedRunId = await startTry(wf.id, {
...
async function seedBranchErrorRuns(wf) {
  ...
  const errorRunId = await startTry(wf.id, {
    [target.ctxKey]: MISSING_BLOB_KEY,
    documentId: upload.documentId,
  });
 ...
 *
 * `POST /:id/tries` takes an explicit `workflowVersionId`, so no version
 * juggling is needed — the demo is seeded with both versions and the Try is
...
async function seedReplayVersionRun(wf, ctx) {
  ...
  const runId = await startTry(wf.id, ctx, v1.id);
