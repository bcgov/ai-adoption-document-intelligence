...
Run: `cd <repo-root> && node scripts/seed-feature-demos.mjs`
...
```bash
cd <repo-root>/packages/graph-workflow && npm test
cd <repo-root>/apps/frontend && npx vitest run src/features/workflow-builder
cd <repo-root>/apps/temporal && npx jest ocr-payload-ref build-workflow-result graph-workflow
```
