# US-031: Provide Standard OCR and SDPR Workflow Templates

**As a** workflow author,
**I want to** have pre-built workflow templates for the standard OCR workflow and the SDPR multi-page workflow,
...
- [ ] **Scenario 2**: SDPR multi-page workflow template is available
    - **Given** the SDPR workflow example from Section 4.5
    - **When** a user wants to create an SDPR workflow
    - **Then** a template JSON file or seeded database record provides the complete `GraphWorkflowConfig` for the SDPR pipeline (update status, split document, map segments, OCR child workflows, classify, join, aggregate, store results)
...
- Per Section 17.2 step 5: "Create a Standard OCR Workflow template and optionally an SDPR Multi-Page template"
...
- The SDPR template is the graph from Section 4.5
