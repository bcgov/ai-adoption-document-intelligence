...
Stress parameter matrix and execution order: [docs-md/benchmarking/LOAD_TEST_STRESS_RUN_SHEET.md](./LOAD_TEST_STRESS_RUN_SHEET.md).
...
To deploy an extra isolated instance (separate Postgres / Temporal) in **`fd34fb-test`** with images built from your branch, follow **[MANUAL_LOAD_TEST_INSTANCE.md](../operations/MANUAL_LOAD_TEST_INSTANCE.md)** (`scripts/oc-build-push.sh` + `scripts/oc-deploy-instance.sh`).

For blob-heavy scenarios (`npm run load-test:k6:upload-ocr`, `:blob-storage`, `:payload-sizes`) you can keep all object I/O **inside the cluster** by passing **`--blob-storage-provider minio`** to the deploy script. The instance then includes a per-instance `<instance>-minio` Deployment + PVC and routes backend / worker writes to it instead of the shared Azure container — see the **Mock blob storage with in-cluster MinIO** section of [MANUAL_LOAD_TEST_INSTANCE.md](../operations/MANUAL_LOAD_TEST_INSTANCE.md).
...
2. **Benchmark OCR cache** — `__benchmarkOcrCache` with `ocrResponse` replays prior results (see [OCR_IMPROVEMENT_PIPELINE.md](../extraction/OCR_IMPROVEMENT_PIPELINE.md)).
...
| Backend blob storage | Object storage via `BLOB_STORAGE_PROVIDER` — in-cluster MinIO (its own PVC) or Azure Blob; no shared backend filesystem PVC (the former `backend-services-storage` RWX PVC was removed) | [backend-services/deployment.yml](../../deployments/openshift/kustomize/base/backend-services/deployment.yml) (`BLOB_STORAGE_PROVIDER`, `MINIO_*`, `AZURE_STORAGE_*`), [components/minio/pvc.yml](../../deployments/openshift/kustomize/components/minio/pvc.yml) |
| Backend log volume | Per-pod `emptyDir` mounted at `/var/log/app`, tailed by the promtail sidecar and truncated past 10MB by the `log-rotator` sidecar; Loki holds the durable copy, so no PVC | [backend-services/deployment.yml](../../deployments/openshift/kustomize/base/backend-services/deployment.yml) (`logs` volume, `log-rotator`) |
