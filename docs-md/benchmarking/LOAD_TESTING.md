...
| Backend log volume | `ReadWriteMany` PVC (`netapp-file-standard`, `backend-services-logs`) mounted at `/var/log/app`, tailed by the promtail sidecar | [backend-services/pvc-logs.yml](../../deployments/openshift/kustomize/base/backend-services/pvc-logs.yml) |
