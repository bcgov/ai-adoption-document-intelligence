...
Dynamic-node **run** tests additionally need the deno-runner container from
the docker infra. No credential is configured for them: the worker mints a
short-lived internal token per invocation over its own DB connection — see
