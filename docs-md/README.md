...
| [workflows/](workflows/) | DAG workflow engine, graph types, adding nodes/activities/OCR providers (incl. Mistral OCR), workflow builder guide and design brief, node catalog, lineage/versions, config overrides, Temporal worker concurrency, Temporal payload footprint (gzip codec + OCR payload refs), [workflow linter](workflows/WORKFLOW_LINTER.md) (validity + shape coverage), [workflow agent setup](workflows/AGENT_SETUP.md) (which model service the chat assistant uses and how to get access), [templates/](workflows/templates/) example configs |
