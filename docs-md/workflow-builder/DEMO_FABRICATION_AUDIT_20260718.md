...
**OPEN — catalog gap (needs a decision, NOT fixed):** `ocr.normalizeFields` declares its input kind as **`OcrFields`**, but **no activity produces `OcrFields`** (verified: feeding it an `OcrResult` is rejected — `OcrResult not assignable to OcrFields`). So the field-normalization step cannot be wired into any auto-validated chain. To keep the two templates that used it (`standard-ocr-workflow-normalize`, `-with-corrections`) load-clean, the `normalizeFields` node was **removed** from both (chains rewired to bypass it; `-with-corrections` still demonstrates corrections via `characterConfusion` + `spellcheck`). **`-normalize` now no longer normalizes anything** — its distinguishing step is gone. Decision needed: either (a) make `ocr.normalizeFields` accept `OcrResult` (backend catalog change), or (b) add a real `OcrFields` producer, after which `normalizeFields` can be re-added; or (c) retire/repurpose the `-normalize` template. Until then it is effectively a duplicate of the base standard-ocr template.
