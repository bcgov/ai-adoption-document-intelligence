...
1. **Try-in-place previews + incremental cache (`@infra`, 9.4–9.6, 9.9).** The core value prop; only "a run starts" is guarded (`tier3-try-infra`). Asserting status-badge progression, a rendered preview widget, and cache-hit-on-param-tweak needs a run that **completes successfully** — i.e. real activity execution (Azure OCR = external + non-deterministic + cost) or a dynamic-node-only workflow (which needs the worker's `PLATFORM_API_KEY` provisioned, see the note below). It is also blocked on the open `node-statuses` 500-on-FAILED-run gap for negative cases. Build deliberately with a deterministic completing workflow + a configured worker, not as a quick batch.
