...
- [ ] **4.1 Switch — route each item down one edge.** *A switch tests its cases top-to-bottom; the first match sends the item down that case's **edge**, else the default edge. Key model: a case does **not** point at a target node directly — it points at a **conditional edge** that already runs from this switch to a target. So you wire the edge on the canvas first, then bind a case's condition to it.*
  1. Click **Branch by condition** (the yellow diamond) → its settings open on the right. It already has two cases plus a default, and each is bound to one conditional edge leaving the diamond (to **Sub-workflow**, to **Wait until condition**, and — as default — to **Wait for approval**). All three of this switch's conditional edges are therefore already claimed.
  2. **Add a new route (do the edge first).** On the canvas, drag a new **conditional edge** from the switch's output handle to the target node you want the new case to reach. *(Edges drawn out of a switch are conditional; a fresh target gives you an unclaimed edge to bind.)* If you skip this and just click **Add Case**, the case's **Edge** dropdown will be **empty** — because there is no unclaimed conditional edge to attach it to. That empty dropdown is expected, not a bug; it's the signal to go draw the edge.
  3. Click **Add Case** → a new empty case row appears. Click its **condition** → build a test (e.g. `currentDoc.type` **equals** `"invoice"`). *(`currentDoc` is an untyped loop item, so type the `.type` path by hand — see the untyped-data note above.)*
  4. Open the case's **Edge** dropdown. **What this dropdown is:** the list of edges this case is allowed to fire. It shows **only conditional edges that (a) start at *this* switch and (b) aren't already used by another case** — normal edges, error edges, and edges leaving *other* switches are all excluded, because a switch can only route down its own conditional branches. Pick the edge you drew in step 2.
  5. Optionally set a **Default edge** (taken when no case matches).
  **Pass:** with a spare conditional edge present, the Edge dropdown lists exactly this switch's unclaimed conditional edge(s); **Save + reload** keeps every case and its edge.
