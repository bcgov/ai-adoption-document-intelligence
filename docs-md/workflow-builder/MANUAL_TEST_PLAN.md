...
- ✅ **Control-flow authoring (Part 4).** `tier2-control-flow` (6 tests): all six control-flow settings forms render their saved values; the **join** source-map NodePicker lists **only** `type:"map"` nodes and the **switch** case EdgePicker offers only that switch's `conditional` edges (the scoping constraints); the recursive **condition editor** deserializes a 3-level `AND(OR(EQ,GTE),NOT(IS-NULL))` expression (asserted by the nested `…-operand-N-editor-body-*` testids); switching humanGate **onTimeout→Fallback** reveals the fallback-edge picker; and a `maxAttempts` edit **round-trips** through Save + reload (the serialize/deserialize path unit tests miss). Deterministic, default CI. Two notes surfaced while building this: (a) `JoinNode.strategy` is fixed to `"all"` in the schema — the plan's "all/any" (4.3) is stale, there is no strategy control; (b) HumanGate's fallback EdgePicker passes **no** `edgeTypes`, so it lists all edges out of the node, not only `error` edges. Residual manual: 4.2/4.4/4.5 deeper form editing.
