...
- [ ] **9.10 Cache-evicted preview.** Replay a run whose cache row was deleted **for a node that SUCCEEDED** (`DELETE FROM activity_output_cache WHERE node_id='<id>'`). **Pass:** red alert “Preview unavailable — cache evicted. Re-run to repopulate.” + **Re-run** button fetches the original `initialCtx` and starts a fresh Try. This must remain distinct from 9.10a — eviction is the ONLY no-output state that offers a Re-run.
