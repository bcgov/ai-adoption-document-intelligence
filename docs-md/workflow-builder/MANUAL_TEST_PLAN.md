...
1. **Dynamic-node security tier as `@infra` e2e (14.11–14.13).** Highest risk, and currently only unit-level. Add end-to-end specs that publish + run a node attempting network egress / remote import / env read and assert the run fails with the typed permission error. Drives the real deno-runner + worker path the unit tests mock.
2. **Try-in-place previews + incremental cache (`@infra`, 9.4–9.6, 9.9).** The core value prop; only "a run starts" is guarded. Assert status-badge progression, at least one preview widget renders, and a param tweak yields upstream cache-hits (violet) + downstream re-exec.
3. **Validation surfacing in the UI (5.4) + save-time binding-walk error anchoring (7.6 via UI).** Logic is unit-tested; the UI surfacing (red badges + validation drawer keyed by node/port) is not.
