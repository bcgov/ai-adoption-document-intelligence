...
2. **Validation surfacing in the UI (5.4) + save-time binding-walk error anchoring (7.6 via UI).** Logic is unit-tested; the UI surfacing (red badges + validation drawer keyed by node/port) is not.
3. **Workflow-as-API happy path (11.3/11.4) + versioning revert & run-a-version (12.2/12.4).** Deterministic, no LLM; guards the run/versioning contract end-to-end.
4. **Agent depth beyond the surface (15.6–15.10).** File-drop→source.upload resolution, conversation persistence/switcher, cost ceiling, and injection guard — extend `tier3-agent-stubbed` with recorded streams where possible; the guards themselves stay unit-tested.
