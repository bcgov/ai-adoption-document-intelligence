...
- [ ] **7.2 Row tooltip.** Hover a port row (or its handle). **Pass:** tooltip reads `<name>: <Kind> — <description>` (e.g. `document.split`'s output: `segments: Segment[] — List of produced segments — each with segmentIndex, pageRange, blobKey, and pageCount.`). A **required input with no bound source** shows an **amber ring** around its handle. ⚠️ The ring is broader than the problems badge: required base-`Artifact` identifier ports (e.g. `documentId`) wear the ring but do **not** count in the badge/drawer or block Save (reconciliation is Phase 3 — see PORT_WIRING_DESIGN §15).
