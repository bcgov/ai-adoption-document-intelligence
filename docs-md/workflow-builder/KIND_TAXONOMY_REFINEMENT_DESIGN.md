...
- **Segment drill-down scope (follow-up shipped 2026-07-18):** segment fields are a `TypedSegment` **map item** (`currentSegment`), not a scalar producer output — every real Segment producer port is an array kind (`DocumentSegment[]`). The map item's element kind was already resolvable via `resolveProducerKindFor`'s map-item unwrap, but `buildVariableOptions` never listed the item key (it lives on the map node, not on ctx or an activity output), so neither the variable picker nor the condition editor offered it. Fixed by surfacing in-scope loop variables (map `itemCtxKey`/`indexCtxKey`) as a **"Loop variables"** group for nodes inside the map body (body membership via upstream reachability between `bodyEntryNodeId` and `bodyExitNodeId`). Now both surfaces drill it — e.g. the multi-page-report demo's `segmentRouter` switch can pick `currentSegment.segmentType` from the list. Tested in `graph-widgets/variable-picker-scope`.
