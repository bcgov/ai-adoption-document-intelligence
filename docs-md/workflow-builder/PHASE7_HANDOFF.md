...
1. **Azure GPT-4.1 tool-use blocked by APIM strictness.** The Azure SDK sends `content: null` on assistant tool-call messages (standard OpenAI shape). The APIM at `test.aihub.gov.bc.ca/sdpr-invoice-automation` rejects this with `Invalid value for 'content': expected a string, got null`. Anthropic works end-to-end. Two fixes possible: (a) deploy a newer model behind the APIM that accepts the latest schema, (b) add a content-normalization middleware on the backend that replaces null with empty-string before sending. Code path is in place; switching is one dropdown click once the upstream accepts the schema.
