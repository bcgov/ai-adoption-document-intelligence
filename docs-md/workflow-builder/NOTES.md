...
Captured from the original verbal stream of consciousness, paraphrased and lightly organised.

### 1.1 Typed connections between nodes

The workflow system is driven by nodes that connect through standard types. There is a base `document` type with a `document source` (for example SharePoint or API input); a document can be single-page or multi-page; a splitter turns a multi-page document into single-page documents; segmenting a document produces document fragments. Nodes are the units that do work, and the type system is permissive through clear class structures — multi-page and single-page documents are both documents.
...
A workflow should be deployed and interacted with through its own API — a workflow that *has* an API — rather than talking to the app directly.
...
It should be impossible to try a workflow without deploying it — you launch it and try it out in place, interacting with it as it runs (for example an upload node that feeds a document straight through the workflow interface).
...
Per-node preview windows displayed inside the workflow editor — for example seeing the splitter's output, paging through documents, or inspecting what a preprocessing step does. (Inspired by ComfyUI.)
...
Support dynamic nodes — nodes defined at runtime, in the style of Windmill.
...
Instruct an AI agent to build these workflows on the fly — porting scripts into Windmill and turning them into nodes and activities. Have it work in a feedback loop: it sets up the pipeline and tests it, and if something isn't working it tweaks the code and re-runs until it delivers what the user asked for. The agent can run inside Claude Code — built by modifying an instruction file, then run as a sub-agent from the current conversation.
...
Curiosity about segmentation — how Azure Content Understanding and similar services do it, what products exist, and how we might implement it ourselves (for example, extract a segment from a document and pass it to a particular OCR or VLM).

See §4 below for research findings.

### 1.9 Taxonomy curiosity

Determine the full taxonomy of building blocks needed — whether this collection has a formal name in workflow systems, and what existing models we can borrow from.
...
- **Drop generic "Activity" node with a dropdown.** Expose distinct activity types directly in the palette/menu, with search. The term "activity" reads as too abstract for end users — a concrete label such as "extraction" communicates the step better; "activity" is an internal concept users shouldn't need to know. → Decision in [IMPLEMENTATION_PLAN.md §3.2](IMPLEMENTATION_PLAN.md#32-one-distinct-palette-entry-per-activity-type).
...
- **Config in a popup on click**, not always-on (ComfyUI-style). The target user is assumed to be non-technical, so the interface should keep visual noise low.
- **Allow node-type swap** (change a node's type in place, preserving overlapping config). Without it, changing an activity's type forces deleting the node, recreating it, and re-linking everything.
- **User-friendly labels over engineering terms.** The engineering primitives `loop / map / switch / wait-retry` map to user-facing names like "Document upload / OCR processing / AI extraction / Conditional human reviews / Export". Decision: keep engineering primitives in the schema; surface user-friendly display names in the palette + on the node.
- **Simplify config field labels.** Some field labels aren't self-explanatory (for example "retry count"). → either rename, or add inline help text. The schema-driven renderer uses `.describe()` text as help.
- **Typed multi-handle ports (ComfyUI-style) — pushed back on.** Position: prefer single-purpose nodes (one input, one output, doing one thing) over multi-typed connectors. Multiple wires into one node is fine; multiple *typed* connectors is what to avoid. The "noodles problem" observation reinforces. Net effect: aligns with the I/O model decision doc (Model A); typed-artifact handles are deferred and treated as a *UI hint*, not engine semantics.
...
- **AI-built workflows confirmed as the long-term primary creation path.** The long-term vision is that the user describes the workflow in chat and the AI assembles it. The visual editor is for inspection and manual edits, not the primary creation path.
...
The research question: how does Azure Content Understanding do segmentation, and how could we implement it ourselves — for example, extracting a segment from a document and passing it to a particular OCR or VLM.
