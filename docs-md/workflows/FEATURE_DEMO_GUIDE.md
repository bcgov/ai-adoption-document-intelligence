...
1. ⚠️ *Executing* this node in a run additionally needs the deno-runner up alongside the Temporal worker (14.9) — the script's platform credential is a per-invocation internal token the worker mints itself, so there is nothing to configure.
