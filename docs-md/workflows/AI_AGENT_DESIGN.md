...
| GET  | `/api/agent/models` | — | `{ items: [{ provider, model, label, isDefault }] }` — the (provider, model) pairs this backend is configured for; the picker renders exactly this |
...
**Self-call auth.** Most tools call the Nest services directly, but the Phase 4 controller-only endpoints (run start, node-statuses, preview-cache, source upload) are reached over HTTP. Those self-calls authenticate with a short-lived internal token that `AgentService` mints once per agent run from the already-resolved identity (group + actor, purpose `agent-self-call`), sent as the `x-internal-token` header on every self-call — uniformly for JWT/IDIR-authenticated and API-key-authenticated chat callers. The caller's own credential is never forwarded, so there is no cookie forwarding, no CSRF dance, and no mid-run staleness when a JWT refresh rotates.
...
1. **Azure provider is live; the model list comes from the backend.** `AgentEnv` reads `AGENT_DEFAULT_PROVIDER` (`anthropic` | `azure`), plus the shared `AZURE_OPENAI_API_KEY` / `AZURE_OPENAI_ENDPOINT` / `AZURE_OPENAI_DEPLOYMENT` / `AZURE_OPENAI_API_VERSION`. `ProviderResolver.buildModel` constructs an Azure model via `createAzure(...).chat(deployment)` (legacy chat/completions, which APIM proxies forward; the Responses API is avoided). The per-request `{ provider, model }` from the frontend picker overrides the default — `model` is used verbatim as the Azure **deployment name**. NB: `AZURE_OPENAI_*` is **shared** with `enrich-results`, `format-suggestion`, and `ai-recommendation` — repoint the endpoint only knowing all four move together.

   **The picker is served, not hardcoded** (2026-08-08, item 23). `GET /api/agent/models` returns `{ items: [{ provider, model, label, isDefault }] }` from `configured-models.ts#listConfiguredModels`, one entry per provider `AgentEnv.hasProvider` says is configured, carrying that provider's single configured model (`AZURE_OPENAI_DEPLOYMENT` / `AGENT_ANTHROPIC_MODEL`) — there is no multi-deployment list variable. The drawer renders exactly that list (`useAgentModels.ts`), so it can never offer a model this backend cannot serve; with no selection yet, a turn omits `provider`/`model` and the backend applies its own default. This replaced `AGENT_MODEL_OPTIONS`, a hardcoded six-entry array in `store.ts` whose first element (`gpt-5.4`) was sent on every turn regardless of configuration.

   **A blank credential is not a credential.** `AgentEnv` trims every setting and treats empty/whitespace-only as absent. The repo-root `.env` holds `ANTHROPIC_API_KEY=""`, which previously made `hasProvider("anthropic")` true, handed the SDK an empty key, and produced a mid-stream HTTP 401 instead of the typed `provider-not-configured` refusal. Anthropic remains fully supported in the code; it is simply not configured here, so it is not offered.
