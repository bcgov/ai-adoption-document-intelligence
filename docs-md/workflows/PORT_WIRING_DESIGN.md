...
- **The settings panel populates from the same set (2026-08-03).** `isEditableInputPort` used to exclude optional base-`Artifact` ports outright, so the card rendered a row and an `in-<port>` handle for `file.prepare`'s `fileType` — advertising its kind and its description — while the only surface that could accept an answer pretended it did not exist. What separates the two populations now is **prominence, not existence**: an optional base-`Artifact` port with nothing bound to it is returned flagged `optional` and folded behind a collapsed *"N optional inputs"* disclosure in `InputsSection`. Anything holding a wire, a pin or a constant is never `optional`, so giving a port a source moves its row up into the main list. Kind-less ports are still excluded. The behaviour is opt-in per caller (`resolveWireableInputRows(..., { includeOptionalIdentifierPorts: true })`) — see §6.5 for why the connect-summary popover opts out.
...
Nodes get taller (worst current case: Azure OCR extract, 5 inputs). Mitigations, in order of application: only wireable ports get rows (§4.2 removes the config-param rows that dominate most nodes); the existing Simplified view collapses groups as today. No preemptive virtualization — revisit after the first prototype if 15+-node graphs feel heavy.

The planned `+N more` collapse for optional ports **on the card** was never built and is not the shipped answer. The card keeps a row per wireable port — that is what makes drag-to-bind possible for them — and the folding happens in the **settings panel** instead (§4.2, §6.5), where reading is the job and the port count is the density problem. Card density is currently acceptable at the worst case, so nothing on the canvas folds.
...
**The pinned key must be the key the producer actually writes (2026-07-25, G-104).** `pinPortBinding` computed it as `synthesiseCtxKey(producerNodeId, producerPort)` unconditionally. That is correct for an `activity`/`pollUntil`, whose output ctx key really is `__auto.<node>.<port>` unless an `outputs[]` row says otherwise — and wrong for **every** producer G-007 made bindable, which write ctx through a dedicated field the author names: a `map`'s `itemCtxKey`, a `join`'s `resultsCtxKey`, a `humanGate`'s `<nodeId>Payload`, a `childWorkflow`'s `outputMappings`, a `source`'s emitted key. Pinning one of those persisted a key no executor ever writes **and** stamped a matching `outputs[]` row on the producer, which made the dead key decode as healthy (`resolveCtxKeySource` treats a live node still declaring the port as the source) — so the broken binding reported as a clean "Pinned" row on every surface. The pin path now consults `producerCtxKeyForPort` first (as `resolveBindings` already did), falls back to the producer's `outputs[]` row and only then synthesises, and adds an `outputs[]` row **only** when the producer genuinely binds through `outputs[]`. A hand-authored `outputs[]` row on a control-flow node is inert — the executor reads the field — so the field wins over it.
...
### 6.3a Reconnect clears reconnectable locks ("connect again = wire again")

A `locked-unbound` port (§6.3) is skipped by the resolver forever until the lock is cleared — so after a user deletes a data wire *and* fully detaches (removing the surviving control edge too), re-drawing the same node-level execution edge would NOT auto-wire, inconsistent with the first connect. Fix: when the user draws a **fresh node-level execution edge** into a node, `clearReconnectableLocks(config, targetNodeId)` (in `canvas/wire-mutations.ts`) clears the `locked-unbound` lock on that node's port(s) the new edge makes auto-bindable. For each locked port that is *unbound* (no `inputs[]` row, or a row with a falsy `ctxKey`), it builds a trial config with just that lock removed (via `revertPortToAutomatic`) and runs `resolveInputPort` with the port's catalog `kind`; the lock is dropped **only** when the trial reports `auto-bound` (dropping the whole `lockedInputPorts` metadata field when the list empties). Ports that would still be `unsatisfied`/`ambiguous` keep their lock, so the in-place-delete behaviour is preserved — only an explicit connect that actually re-satisfies the port clears it. It runs against the config that already *includes* the new edge (so the source counts as upstream) and only *decides* which locks to drop; the host's `resolveBindings` pass writes the binding. Excluded: port-to-port pins (explicit — §6.1, returns early), duplicate/no-op edges (nothing added), and `locked-bound` "pinned" ports (a source the user explicitly chose).
...
When a node-level connect causes the target to auto-bind an input to the source — so the gray sequence edge the user just drew is visually superseded by a blue data wire — a toast fires: *"Auto-wired — data now flows from &lt;source&gt;."* Strictly gated: `handleConnect` re-derives data wires before vs after the new edge (`deriveWires` over `resolveBindings` of the lock-cleared config from §6.3a) and shows the toast **only** when a NEW `auto` data wire `source→target` appears — so a reconnect that clears a `locked-unbound` lock and re-binds the port narrates it just like a first connect. It does NOT fire on port-to-port pins (explicit; the wire is its own feedback), on connects that produce no new binding, or on node drops. Complements the §6.4 summary popover — the popover lists all input states; the toast explains the specific wire-swap that just happened on canvas.

### 6.5 Typing a value on an input row (constants)

**Shipped 2026-08-03.** Some inputs want a value, not a wire — but only sometimes. `file.prepare`'s `fileType`, `fileName` and `contentType` are optional and auto-derived from the blob key; the default path is to leave them empty. This is about **override**, and about the card and the panel telling the same story (§4.2).

An unbound row inside the *"N optional inputs"* disclosure (and any `unsatisfied` / `ambiguous` / `locked-unbound` row elsewhere in the panel) carries an inline value field, placeholdered with the port's own catalog description. Committing on blur or <kbd>Enter</kbd> — never per keystroke, so one value is one undo step — does three things in one config write:

1. mints a ctx declaration `{ type: "string", defaultValue: <text> }`,
2. replaces the port's `inputs[]` entry with `{ port, ctxKey }`,
3. adds the port to `metadata.lockedInputPorts`.

The lock is not decoration: `resolveBindings` rewrites unlocked ports on every config change, so an unlocked constant would be overwritten by the first upstream producer that could satisfy the port. Clearing the field reverses all three — except it keeps the declaration if another port still binds the same key, which would otherwise manufacture a dangling binding.

**No engine change.** `CtxDeclaration.defaultValue` already existed, `initializeContext` already seeds ctx from it, the upload/run API already overlays per-run values on top, and `deriveInputSchema` already skips anything without `isInput`. A constant is therefore a **default a run can override**, which is exactly the semantics the feature wants.

**Why the key carries no dots.** The key is `__const_{nodeId}_{port}` with every character outside `[A-Za-z0-9_]` folded to `_` (and a numeric suffix if that sanitisation collides with an existing key). Recognition is by prefix alone — `isConstCtxKey`.

The dots are not a style choice. A **declared** default and a **runtime** write take different paths through ctx:

| | writer | reader |
|---|---|---|
| declared `defaultValue` | `initializeContext` — `ctx[key] = declaration.defaultValue`, **flat** ([apps/temporal/src/graph-engine/context-utils.ts](../../apps/temporal/src/graph-engine/context-utils.ts)) | `resolveCtxBinding` — `resolvedKey.split(".")`, then walks each segment, **nested** ([packages/graph-workflow/src/validator/context-utils.ts](../../packages/graph-workflow/src/validator/context-utils.ts)) |
| `__auto.{node}.{port}` | `writeToCtx` at runtime, which **nests** each dot segment (`ctx.__auto` → `{ node: { port: … } }`) | the same nested read — so the two agree |

A dotted constant key would be seeded at `ctx["__const.n.p"]` and read at `ctx.__const.n.p` — never the same slot, and silently empty at run time with nothing to see on any surface. `__auto.*` survives its dots only because the engine *writes* those through the nesting helper; a declared default never goes through it. So the constant key carries no dots at all. `port-constants.test.ts` asserts the failure mode directly ("a DOTTED constant key would be seeded flat and read as nothing") rather than trusting the comment.

**Promotion.** *"Make this a workflow input"* on a constant row renames the hidden key to a name the author picks (`renameCtxKeyInConfig`, which rewrites the port binding, map/join keys, childWorkflow mappings and `ValueRef` refs) and sets `isInput: true`. The typed value survives as the declaration's `defaultValue`, so the promoted key reaches the run-spec as an **optional** property carrying a default. The Workflow-settings ctx editor also gained a **Default value** field — the surface for values worth naming and sharing, deliberately not the way in.

**Rejected alternative:** a `{ port, value }` variant on `PortBinding`. More direct, but a schema change across engine, validator, resolver, canvas, drawer and run-spec, plus a third answer to "where does this input come from". Still open if ctx proves insufficient.

**Counts are unmoved by design.** An optional identifier port with no constant is counted by nobody — `computeNodeInputIssues` skips it, so no badge, no drawer entry, no top-bar count — and typing a constant adds no warning and removes none. A constant on a *required* port does clear that port's warning. `ConnectSummaryPopover` calls `resolveWireableInputRows` **without** `includeOptionalIdentifierPorts`: that surface narrates what a connection just did, and it would otherwise render "⚠ needs a source" for a port the badge and the drawer both deliberately decline to count. The shared resolver is still one module; the two callers differ by one explicit option rather than by drift.
...
- **Interactive producer rows (item 6X):** an `InputsSection` row that resolves to a REAL producer node is click/hover-interactive — **click** the row to select that producer on the canvas and pan/center it into view (`WorkflowEditorV2Page.handleJumpToProducer` → `selectNodeSticky` + the live instance's `setCenter`); **hover** the row to highlight the producer node (page holds `highlightedNodeId`, passed to `WorkflowEditorCanvas`, which stamps the `wb-node-highlight` outline class on that node — any node type, node-only, no wire highlight). A row is interactive only when a real producer source exists: `auto-bound` (→ `producerNodeId`) or `locked` bound to an `__auto.*` key (→ `decodeAutoProducerNodeId`, guarded to a live node). `unsatisfied` / `ambiguous` / `locked-unbound` / `ctx-bound` / `locked`-to-a-hand-authored-ctx-var rows are inert (no pointer, no jump, no highlight). Producer id is derived by `producerNodeIdForRow` in `settings/InputsSection.tsx`; callbacks thread `InputsSection → NodeSettingsPanel → WorkflowEditorV2Page`.
...
2. **Per-port handles + derived wires, render-only** (§4, §5, §7 tooltips) — the canvas starts telling the truth; gestures still node-level. Biggest e2e/docs churn lands here.
   *Status: complete 2026-07-13.* Landed:
   ...
   - Auto-arrange uses measured per-node sizes instead of the old fixed box (`ACTIVITY_BASE_HEIGHT` + 22px/row + 6px margin; control-flow 180; dagre node width widened to 482 to fit port-row cards). *(The intercepts were re-measured 2026-08-08 when the node-card preview became a fixed-height strip — `ACTIVITY_BASE_HEIGHT` 177 → 88 (`ACTIVITY_CHROME_HEIGHT` 58 + the 30px strip), source 165 → 96 (`SOURCE_CHROME_HEIGHT` 66 + strip). The 22px/row slope and the control-flow 180 are unchanged. See [TRY_IN_PLACE_DESIGN.md §4.7](TRY_IN_PLACE_DESIGN.md).)*
   ...
   **Known limitations of this slice, documented rather than silently deferred:**
   ...
   - ~~Map-body container sizing still assumes fixed footprints, so a wide member card can protrude past the container border (pre-existing).~~ **Resolved** — `computeGroupBounds` encloses each member's full footprint using per-node `estimateNodeWidth`/`estimateNodeHeight` plus a 40px pad, so a wide port-row card no longer spills out. Since 2026-08-03 the same function sizes authored group containers too (one component, `GroupContainerNode`, for both).
   - ~~The activity height estimate calibrates to the preview-skeleton state — never-run cards render ~120px shorter, ready-state previews up to ~80px taller than estimated.~~ **Resolved 2026-08-08 (UX walkthrough item 9).** The card's preview is now a fixed-height result strip (`PREVIEW_STRIP_TOTAL_HEIGHT_PX` = 30) with the scrollable widget in a popover, so a card's rendered height no longer depends on run state and the estimate is a fact rather than a mid-point. The constants were re-measured against real `offsetHeight` on `standard-ocr`: `ACTIVITY_CHROME_HEIGHT` 58 + strip = `ACTIVITY_BASE_HEIGHT` 88, `SOURCE_CHROME_HEIGHT` 66 + strip = `SOURCE_NODE_HEIGHT` 96, `CONTROL_FLOW_NODE_HEIGHT` unchanged at 180 (control-flow cards draw no strip). Drift measured across a full Try: 0px on 14 of 15 cards, 5px on the one that failed (its failure chip, not the strip). See [TRY_IN_PLACE_DESIGN.md §4.7](TRY_IN_PLACE_DESIGN.md).
   ...
   - ~~Map-body item data flow renders as a plain sequence wire — the resolver feeds the loop item through `itemCtxKey` without materializing a map-node output binding, so no map-item wire derives until §4.4's `item (T)` port lands (the `via: "map-item"` tooltip branch is forward-looking).~~ **Resolved 2026-07-25 (G-104).** No map-node output binding was needed after all — what blocked the wire was a naming disagreement. The resolver reported a map's item producer as `producerPort = itemCtxKey` (the author's variable name) while the shared write enumeration `nodeTypeCtxWrites` records it under the fixed port name `"item"`, so a derived wire would carry a `sourcePort` its own provenance lookup could never match and `derive-wires.ts` excluded `map` from the producer index outright. The resolver now reports `"item"` — the same stable-identifier shape every other control-flow producer uses (join `"results"`, humanGate `"payload"`, childWorkflow `mapping.port`) — `map` indexes like any of them, and the `via: "map-item"` branch is live rather than forward-looking. The map still materializes **no** `outputs[]` row: `producerCtxKeyForPort` maps the port name back to `itemCtxKey`, which is what both the resolver and the pin path bind to. See §6.1 for the pin-path half of the same fix.
3. **Gestures** (§6) + **port-aware popover** (§9) — drag-to-bind, connect-time validation, delete semantics, connect summary.
   *Status: complete 2026-07-14.* Landed:
   ...
   - Connect summary popover (§6.4): a node-level connect opens a transient popover narrating what auto-wire did — ✓ rows for auto-bound/pinned/ctx-bound ports, ⚠ rows for needs-a-source/ambiguous/disconnected ports with **Fix** deep-links — auto-dismissing after ~8s. Row resolution is shared with the settings panel via `settings/input-row-resolution.ts` so the two surfaces can't drift. *(Since 2026-08-03 they populate from the same resolver but not the same set: the panel opts in to optional identifier ports and the popover does not — one named option, not divergence. §6.5.)*
