...
Open the disclosure and each row offers a value field, with the port's own description as its placeholder (*"Auto-detected from the extension if omitted"*). Three states, and the common one costs nothing:
...
3. Click "Group selected". A new entry appears in `nodeGroups` with an auto-numbered label (e.g., `Group 1`, then `Group 2`, …) and the selected node ids, and a container box is drawn around the members. A toast confirms how many steps were grouped and names the header strip as the way to move them.
