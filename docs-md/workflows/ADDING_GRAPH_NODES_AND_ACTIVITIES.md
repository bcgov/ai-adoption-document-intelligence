...
References:
- `docs-md/workflows/DAG_WORKFLOW_ENGINE.md`
- `docs-md/workflows/GRAPH_TYPES.md`
...
A programmatic manifest of the registered correction tools and their parameters is available via `apps/temporal/src/correction-tool-registry.ts` (mirrored for the API in `apps/backend-services/src/hitl/tool-manifest.service.ts` — keep the two aligned). Of the registered tools, only **`ocr.characterConfusion`** and **`ocr.spellcheck`** are AI-recommendable; **`ocr.normalizeFields`** and **`ocr.recoverNumericZerosFromCheckboxes`** are registered but excluded from AI recommendation via `getAiRecommendableTools()` (they require deterministic or per-form structural config the model can't author reliably). The AI recommendation pipeline (Feature 008 Step 3) uses the manifest for tool IDs and parameter schemas; **node placement** for candidate workflows is fixed: **split the first normal edge after structured OCR** (`azureOcr.extract`, `mistralOcr.process`, … — `OCR_CORRECTION_AFTER_ACTIVITY_TYPES` in `@ai-di/graph-insertion-slots`; see `docs-md/extraction/OCR_IMPROVEMENT_PIPELINE.md`), not per-entry “safe insertion” metadata in the registry.

See also: `docs-md/extraction/OCR_CONFUSION_MATRICES.md` for confusion matrix documentation.
