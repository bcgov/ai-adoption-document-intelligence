...
- **Phase 4.0 (this milestone):**
  ...
  - Sidecar Postgres K/V activity-output cache with TTL + hourly GC — **editor Try runs only**: a production (`api`-triggered) run never reads or writes the cache
  - Opt-out caching (every Try-run activity cached unless its catalog entry sets `nonCacheable: true`)
  ...
  - `GET /api/workflows/:id/runs` run-history endpoint with cursor pagination + status filter + date range — lists editor Try runs only
...
**Each Try spawns a fresh Temporal workflow execution.** Every run carries its `RunTrigger` (`"try"` | `"api"`, stamped by the route) into the workflow input, and the cache layer (§2) engages only when `trigger === "try"` — production runs execute every activity for real, every time. Within the editor, the cache short-circuits already-computed activities so subsequent Trys are still fast. This keeps Phase 4 cleanly layered: workflow code stays unchanged, the cache is a worker-level decorator, and the canvas talks to Temporal via existing query/start primitives.
...
**Try has its own endpoint (D-17).** `POST /api/workflows/:id/tries` and `POST /api/workflows/:id/runs` share one implementation and differ in the `RunTrigger` they stamp — `"try"` vs `"api"` — and in that only a Try cancels the workflow's in-flight Tries. A run started through `/runs` cancels nothing, so production traffic never interrupts someone testing the same workflow in the editor. The trigger is assigned by the *route*, never by a request field, so an API caller cannot opt a production run into the cancel set that editor activity sweeps. This matters because the cancel is narrow by design (G-021): only `"try"`-stamped runs are eligible, so that feeding 240 documents through `/runs` does not have document #2 cancel document #1. Until D-17 the editor posted its previews to `/runs`, every Try was stamped `"api"`, and the cancel below always swept an empty set.
...
A Prisma model in `apps/shared/prisma/schema.prisma` (written to `apps/backend-services/src/` and `apps/temporal/src/` via the `npm run db:generate` helper), mapped to a snake_case table like every other model in the schema:
...
model ActivityOutputCache {
  ...
  workflowLineageId String   @map("workflow_lineage_id")
  /// Node id within the workflow config. Stable across versions if the user doesn't recreate the node.
  nodeId            String   @map("node_id")
  /// sha256 hex of the canonical node identity: type + subtype + parameters (§2.3).
...
**Multi-tenancy.** Cache rows are scoped to `workflowLineageId`, which is per-org. There's no cross-org cache pollution. Within a lineage, multiple editors' Try runs share cache rows — lineage-level scope is the natural boundary. Production (`api`-triggered) runs never touch the cache at all (§0), and the GDPR review needed before extending the cache to cross-lineage shared content stays deferred to Phase 4.x.
...
The endpoint (like its sibling `GET /:id/runs/:runId/input-ctx`) answers **editor Try runs only**: the run's `RunTrigger` search attribute is read via `describe()`, and anything other than `"try"` — including an absent attribute — gets the same 404 as an unknown run, fail closed, so a production run's inputs and statuses never surface through the editor endpoints. Returns 404 if the Temporal handle can't be found (run is too old / never existed); 410 Gone if the run has been retention-cleaned by Temporal. The canvas treats 410 as "use the cache row's `outputCtx.endedAt`" — the run is finished, statuses are frozen.
...
**Option C, ruled 2026-08-08** (the alternatives — reserving 200px per node in the resting layout, or letting the pane grow out of flow and occlude what is below — are written up in `feature-docs/20260806-ux-review-batch-four/DECISIONS/09-try-reflow.md`). The card's height becomes *state-independent*:
...
The Temporal visibility query is scoped to `RunTrigger = "try"`: the drawer lists editor Try runs only, so a production run's `initialCtx` (document ids, filenames) never renders as a run-history chip. Production runs are monitored in the Processing queue. The same scope applies to the per-version run counts (§6.5).
...
16. **GC sweep** — `apps/backend-services/src/cache/activity-output-cache-gc.service.ts`, an hourly `@nestjs/schedule` cron (§2.7).
