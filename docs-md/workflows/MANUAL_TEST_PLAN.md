...
- [ ] **6.7a Auto-arrange in Simplified view (2026-08-03).** On a workflow with 2+ groups, turn **Simplified view** ON and drag the chips into a deliberate mess (or just note where they are), then click **Auto-arrange**. **Pass:** the **chips** move — they are laid out left-to-right, non-overlapping, and the viewport re-fits. *(Previously this looked like it did nothing: dagre laid out the member-level graph and each chip sits at its members' centroid, so the visible nodes barely moved.)*
  - Now turn Simplified view **OFF**. **Pass:** each group's members are in the **same internal arrangement** relative to each other as before the arrange — the group moved rigidly, it was not re-laid-out internally. Ungrouped nodes are where the arrange put them.
  - ⚠️ **Known limit, not a bug:** a simplified arrange reserves a *chip-sized* slot per group, so a large group can overlap its neighbours once expanded. Re-run **Auto-arrange** in expanded view to tidy it. Reserving the members' bounding box instead would push visible chips apart by the size of things nobody can see.
...
- [ ] **8.15 Optional inputs fold; a typed value overrides them (2026-08-03).** Drop a **Prepare File** node and click it. In the right rail's **Inputs** section, **Pass:** the optional identifier ports (`fileType`, `fileName`, `contentType`) are **not** in the main list — they sit behind a collapsed **"3 optional inputs"** disclosure (`optional-inputs-toggle`). Expand it (`optional-inputs-list`). *(Before this, the card rendered a row and a handle for each of these while the panel excluded them entirely — the canvas advertised a port no surface would accept an answer for.)*
  - Type `image` into the `fileType` row's value field (`input-constant-fileType`; its placeholder is the port's own description) and press <kbd>Enter</kbd> or click away. **Pass:** the row moves **out** of the disclosure into the main list, wearing a blue **Value** badge, and the count on the disclosure drops to *"2 optional inputs"*. Typing does **not** commit per keystroke — only blur/Enter does, so it is one undo step.
