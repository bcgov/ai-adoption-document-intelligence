...
- [ ] **8.15 A map's loop item draws a wire (G-104).** Build: a `T[]` producer (e.g. `document.split`, `Segment[]`) → a **Run for each item** (map) node → **draw an edge from the map to a body node** whose input takes the element type (e.g. `document.classify`'s `segment`, `Segment`). Set the map's **Item variable** to a name of your choosing (e.g. `currentSegment`).
  ...
  - ⚠️ **Known limitation (G-106):** if the map reaches its body **only** through its *Body entry* setting with no edge drawn from the map, auto-wire cannot see the map at all — the item binding must be typed by hand, and a hand-typed key loads as **Pinned**. Both maps shipped in the product have this shape, so the wire will draw there but the provenance will read *"Pinned by you"* rather than *"item from the loop"*.
