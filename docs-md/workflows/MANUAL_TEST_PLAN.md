...
- [ ] **3.6a Draft save — saving and running are different things (2026-08-02).** Create a new workflow and add a single **Submit OCR** node (its required `Prepared file data` input has no source) → **Save**. **Pass:** the save **succeeds** — an **amber** toast reads *"Created — N issues remain"* and names the unbound port; reload round-trips the incomplete workflow exactly as drawn. **Try**/**Run** stay disabled with the reason in their tooltip, and `POST /api/workflows/:id/runs` answers **400** with the same findings. Wire the input (add **Prepare File** upstream) → Save again → **green** toast, Run enables.
