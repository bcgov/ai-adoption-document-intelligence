...
Sixteen workflow templates live in `docs-md/workflows/templates/` and are loaded via the **New from template** picker on the `/workflows` list page (not auto-seeded into the DB). `multi-page-report-workflow.json` is the 16-node “everything” template (switch with 4 branches, map/join, validateFields, 5 groups) — use it as the master exemplar for many tests below.
...
|---|---|---|
| 3.3, 8.4, 8.8 (settings panel; advanced bindings; inline ctx-key create round-trip) | `tier1-node-config` | 1 (default local) |
| 3.4 | `tier2-canvas-drag` | 2 (default local) |
| 3.7, 5.1 (render only) | `tier2-canvas-render` | 2 (default local) |
| 5.4 (validation surfacing), 7.6 (node-anchored, warning path) | `tier2-validation` | 2 (default local) |
| 11.3 (run-spec contract), 12.2 (version revert) | `tier2-workflow-api` | 2 (default local) |
| 3.7, 6.7 (auto-layout on load) | `tier1-editor-load` | 1 (default local) |
| 7.1, 7.2, 7.3 (port rows, row tooltip, derived wire rendering incl. a hover regression guard) | `tier2-typed-io` (4 tests) | 2 (default local) |
| 8.1, 8.2, 8.3, 8.5, 8.6 (auto-bind, states, override/revert, unified problems badge + click-to-picker deep-link, locked) | `tier2-autowire` | 2 (default local) |
| 8.9, 8.10 (partial), 8.11 (partial), 8.13 (drag-to-bind pins a port-to-port binding + reload; incompatible-**kind** port-to-port drop rejected with no wire + a "can't be used here" notice; data-wire delete → `needs-source` → **Revert to automatic** restores an auto wire; node-to-node **connect-summary** popover with a needs-a-source row + Fix deep-link into the source picker) | `tier2-port-wiring` | 2 (default local) |
| 16.7 (kind-**filtered** hover-extend popover: `Show all` escape present, an incompatible-kind activity filtered OUT, an exact-kind activity offered, pick → new node lands **pre-wired** with a `pinned` data wire) | `tier2-port-wiring` | 2 (default local) |
| 4.8, 4.9 (condition **step-picker** shown by default in Ref mode with the resolved "Node → Port" caption; pick a step output → resolved caption; Save+reload persists **both** the condition ref and the producer's materialised `outputs[]` binding) | `tier2-condition-step-ref` | 2 (default local) |
| 10.1 | `tier1-library` | 1 (default local) |
| 12.1, 12.3 | `tier1-versioning` | 1 (default local) |
| 13.3 | `tier1-sources` | 1 (default local) |
| 14.7 (list + editor render), 14.14 (delete→republish restores under same slug; live collision still 409s) | `tier1-dynamic-node` | 1 (default local); 14.14 `@infra` |
| 14.11, 14.12, 14.13 (publish allowlist gate + runtime net/env denial) | `tier3-dynamic-node-security` | 3 `@infra` (opt-in) |
| 15.1, 15.2, 15.3 (chat surface: streamed text, tool-call chips, model picker + abort); 15.4 (file attach → composer chip, queued client-side); 15.5/15.6 (conversation switcher lists prior conversations + select-to-activate) | `tier3-agent-stubbed` | 3 (default local) |
| 15.3 (real build) | `tier3-agent-live` | 3 `@llm` (opt-in) |
| 9.3 (run starts) | `tier3-try-infra` | 3 `@infra` (opt-in) |
| 9.4, 9.5, 9.13 (partial) (run completes: status badges → succeeded + the node's result strip reaches `data-state="ready"` and its popover renders the preview widget; **wire data peek** reaches `data-state="ready"` during the live run + the "View data" wire-menu item is present once a run has happened) | `tier3-try-preview` | 3 `@infra` (opt-in) |
| 9.6, 9.9 (incremental cache-hit: re-run → node `skipped`) | `tier3-try-cache` | 3 `@infra` (opt-in) |
| 14.9, 14.10 (dyn-node run path: node succeeds / throws→failed) | `tier3-dynamic-node-run` | 3 `@infra` (opt-in) |
| 4.1–4.7 (control-flow forms render; map full-form values + maxConcurrency round-trip; join source-map + switch case-edge picker constraints; recursive condition editor deep render; humanGate fallback reveal; **4.4** childWorkflow input/output mapping list editors + port round-trip; **4.5** pollUntil interval round-trip + invalid-duration inline error; pollUntil maxAttempts round-trip) | `tier2-control-flow` | 2 (default local) |
| 6.3, 6.4, 6.6, 6.7 (simplified-view group chip; exposed-param prune on member removal; activity node-type swap + control-flow swap blocked; auto-arrange spreads a stacked graph + persists) | `tier2-node-swap-grouping` | 2 (default local) |
| 13.2, 13.3, 13.4, 13.7 (SourceNodeSettings UI + maxFileSizeMB round-trip; upload endpoint validation matrix; single-source rule + isInput warning) | `tier2-sources` | 2 (default local) |
| 11.1, 11.2, 13.6 (Run drawer renders trigger URL / declared input schema / sample curl / auth notes; upload-source workflow shows the dropzone, no API tabs) | `tier2-run-drawer` | 2 (default local) |

**🔬 unit / integration-backstopped** (not e2e): **3.15/3.16** (find-a-node + ctx references — `node-search.test.ts`, `NodeSearchBox.test.tsx`, `packages/graph-workflow/src/auto-wire/ctx-references.test.ts`, and the drawer wiring in `WorkflowSettingsDrawer.test.tsx`; the *pan* half of 3.15 is jsdom-invisible and stays ✍️ manual), **5.7** (inline child-graph validation — `packages/graph-workflow/src/validator/validator-inline-child.test.ts` for the descent + anchor shape, `useGraphValidation.inline.test.ts` for the top-bar/drawer surfacing, `anchor-target.test.ts` for the parent-node navigation, `ChildWorkflowNodeSettings.test.tsx` for the panel's problems list), **7.9** (pollUntil port rows + catalog fallback — `port-rows.test.ts`, `WorkflowEditorCanvas.test.tsx` "G-016"), **3.8–3.14** (undo/redo + the unsaved-changes guard — `use-config-history.test.ts`, `use-undo-redo-hotkeys.test.ts`, `use-unsaved-guard.test.tsx`, plus the page/canvas wiring in `WorkflowEditorV2Page.test.tsx` and `WorkflowEditorCanvas.test.tsx`; the *browser-native* halves — the real `beforeunload` dialog and native text undo inside an input — are ✍️ manual by nature, jsdom can only assert the event was prevented), 5.5 (graph validator), 7.6 (`dynamic-node-binding-walk.spec` + workflow validator), 11.3/11.4 (`build-run-spec.spec`), 14.1/14.2 (`dynamic-nodes.service/controller.spec`), 14.11–14.13 (`dynamic-nodes.service.spec` + deno-runner + `dyn-run.activity` sandbox-escape specs), 15.7/15.9/15.10 (`agent.service.spec`, `tools.spec`, `abort-flag-map.spec`), and the **2026-08-03 batch**: **3.12a** undo-does-not-refit, **3.17/3.18** canvas menu close + pane menu (`WorkflowEditorCanvas.test.tsx`, `PaneContextMenu.test.tsx`), **6.2a/6.2c** header-vs-member drag and header click (`group-drag-cohesion.test.ts`, `GroupContainerNode.test.tsx`), **6.2b** cancel-preserves-wires + one-undo (`WorkflowEditorCanvas.test.tsx`), **6.7a** simplified auto-arrange (`auto-layout.test.ts`), **8.15/8.15a** port constants (`port-constants.test.ts`, `InputsSection.test.tsx`, `input-row-resolution.test.ts`), **16.1/16.1a** top bar (`WorkflowEditorV2Page.test.tsx`, `WorkflowTitleField.test.tsx`). The *gestures* in these — a real header drag, a real right-click, a real camera — are jsdom-invisible and stay a manual walk.
...
- ✅ **Typed-I/O + auto-wire design-time specs (Parts 7 & 8 core).** `tier2-typed-io` (rewritten 2026-07-13 for the port-wiring Phase 2 render-only slice — 4 tests: per-port row handles + kind colors, row tooltip, derived data/sequence wire rendering with provenance, and a real hover-tooltip regression guard) + `tier2-autowire` (5 tests: auto-bind, unsatisfied/ambiguous/locked states, override→locked→revert, locked-binding preservation). Deterministic, in the default local tier. Residual manual: 7.4/7.5/7.7/7.8 and 8.7.

- ✅ **Dynamic-node security tier as `@infra` e2e (14.11–14.13).** `tier3-dynamic-node-security` (4 tests): publish-time **allowlist gate** rejects a `@allowNet` host outside `DYNAMIC_NODE_ALLOW_NET` (`stage:"allowlist"`, `rejectedHost`); runtime **network egress** to a non-allowlisted host is denied by the Deno sandbox (`Requires net access`); **granting** the host lifts the denial (an A/B on one script and one host — a closed loopback port, so the granted half's clock is a ~40ms connection refusal rather than DNS; see 14.11 for why); **env isolation** denies reading `PATH` (`Requires env access`). Drives the real publish pipeline + deno-runner sandbox over their HTTP surfaces. The complementary file/write/subprocess/ffi/remote-import denial matrix stays covered against the live runner by `dyn-run.activity.integration.test.ts` (Item 5). See the two robustness notes below surfaced while building this.
...
> **Note — dynamic-node run prerequisite:** a full publish→run of a `dyn.*` node needs the Temporal worker and the deno-runner both up. The script's platform credential is not an operator-provisioned secret: for every invocation `dyn.run` mints a short-lived internal token (120 s TTL, purpose `dyn-run`, scoped to the group that owns the running workflow), stores only its SHA-256 hash in the `internal_token` table over the worker's existing DB connection, and injects the raw value into the sandbox as `AI_DI_API_KEY`; the backend accepts it on the `x-internal-token` header and the row is deleted best-effort when the invocation ends. The worker also needs `AI_DI_API_BASE_URL` pointing at the backend (defaults to `http://localhost:3002`) so the sandbox can reach the platform API. If the mint fails (worker cannot reach the database), `dyn.run` fails with an explicit mint-failure message before the sandbox is ever invoked — and transport-level failures of the runner call itself are retried up to 3 times, while script errors fail terminally. Temporal still reports activity failures to node-statuses as a bare `Activity task failed`, so the spec carries the prerequisite in its assertion message. This is also why `tier3-dynamic-node-security` isolates the permission gate via the publish pipeline + deno-runner fast-path rather than a full run.
...
- [ ] **3.6a Draft save — saving and running are different things (2026-08-02).** Create a new workflow, add **Prepare File**, then add a **Branch by condition** (switch) from Flow Control and leave it unwired → **Save**. **Pass:** the save **succeeds** — an **amber** toast titled *"Created as a draft"* reads *"Workflow "…" saved. 1 issue to fix before it can run."* and carries a **Review issues** action (`saved-toast-review-issues`); clicking it opens the Validation drawer, which names the finding (`nodes.switch_1.defaultEdge — Switch node "switch_1" must have a defaultEdge`). The toast itself does **not** recite validator paths. Reload round-trips the incomplete workflow exactly as drawn. **Save stays enabled and blue** while **Run…** is greyed out, hovering it shows *"Fix 1 validation error first — this graph cannot run as it stands"*, and `POST /api/workflows/:id/runs` answers **400** with the same finding. Wire the switch's default edge → Save again → **green** toast, Run enables.
...
- [ ] **3.12a Undo after a delete must not move the camera (2026-08-03).** Load the master template (3.7), **Auto-arrange**, then pan and zoom to a corner you can recognise. Select one node and press <kbd>Delete</kbd>, then <kbd>Ctrl</kbd>+<kbd>Z</kbd>. **Pass:** the node comes back and **the viewport does not move** — no zoom onto the restored card, no pan. Compare with adding a node from the palette, which **must** still fit to the new node (3.1). *(The auto-fit effect used to diff the node-id set and read a history restore as an authored add, so undoing a single-node delete hard-zoomed onto that one card.)*
  - Repeat with a **multi-node** delete and with **Redo**. **Pass:** neither moves the camera.
- [x] **3.13 Undo must not hijack typing.** Click the top-bar workflow **title** (`workflow-title`) to open its rename field (`workflow-title-input`), type `abcdef`, then press <kbd>Ctrl</kbd>+<kbd>Z</kbd> a few times. **Pass:** the *text* undoes character-group by character-group, exactly as in any browser input; the canvas does **not** change and no node reappears. Repeat inside a settings-panel text field. Then click empty canvas and press <kbd>Ctrl</kbd>+<kbd>Z</kbd> → now the workflow edit undoes.
...
- [x] **3.15 Find a node.** Load the master template (16 nodes, 3.7) and pan somewhere the node you want is off-screen. In the top bar's centre zone, left of the **Simplified** switch, type into **Find a node…** (`node-search-input`).
...
### 3.17–3.18 Canvas context menus (2026-08-03)

- [ ] **3.17 A right-click menu closes when you click the canvas.** Right-click a **node** → the node menu opens (`node-context-menu`). Now **left-click empty canvas**. **Pass:** the menu closes.
  - Repeat and instead **pan** (drag the canvas) or **zoom** (scroll) → **Pass:** the menu closes.
  - Repeat and instead **left-click another node** → **Pass:** the menu closes.
  - <kbd>Esc</kbd> still closes it, and so does clicking outside the canvas (the palette, the right rail).
  - Repeat every step above on a **wire** menu (right-click a data wire → `wire-context-menu`) and on the **pane** menu (3.18). **Pass:** all three behave the same. *(They didn't: React Flow's pan/zoom layer calls `stopImmediatePropagation` on pane mousedown, so the click-away listener never fired and the menu closed everywhere **except** the canvas — which is where you always click.)*
- [ ] **3.17a Right-click acts on the whole selection (2026-08-03).** Select **three** nodes — **Ctrl-click** each, or **shift-drag** a marquee across them; plain shift-*click* does not multi-select (see 6.2) — then right-click **one of them**. **Pass:** the menu is headed *"3 steps selected"* and offers **Group these 3 steps**, **Delete 3 steps**, and **Ungroup** only if they are in a group. **Change activity type** and **Edit script** are **absent** — they mean nothing for a set.
  - Click **Delete 3 steps** → **Pass:** all three go, and a **single** <kbd>Ctrl</kbd>+<kbd>Z</kbd> brings all three back. *(It used to show the single-node menu and delete only the node you happened to right-click.)*
  - Re-select three and click **Group these 3 steps** → **Pass:** same result as the top bar's **Group selected** — a box around them, the group panel in the right rail, and the toast.
  - Now select three and right-click a **fourth** node **outside** the selection → **Pass:** the selection collapses to that one node (the other three visibly deselect) and you get the ordinary single-node menu.
- [ ] **3.18 Right-click empty canvas (pane menu).** Right-click a blank part of the canvas. **Pass:** the browser's own context menu does **not** appear; a `pane-context-menu` opens with **Add node here**, a divider, **Auto-arrange**, **Fit view**, **Select all**. There is no **Paste** — nothing in the builder copies yet.
  - On an **empty** graph → **Pass:** only **Add node here** is enabled; the other three are disabled.
  - **Add node here** → the activity picker opens; pick an activity. **Pass:** the new node lands **at the point you right-clicked**, not at the viewport centre — pan somewhere else before picking and confirm it still lands on the original spot. On an empty graph the new node also becomes the entry node.
  - **Fit view** → the whole graph fits. **Auto-arrange** → same result as the top-bar button (6.7).
  - **Select all** → every step is selected. **Pass:** group **container boxes are not** selected (they are not selectable, so they can never be swept into a Delete).
...
- [x] **6.2 Create a group.** Select 2+ nodes, then **More ▸ Group selected**. **Pass:** `nodeGroups[<id>]` is created and persists through Save/reload; a node can be in only one group (moving it prunes empty old groups). The group's settings (label, description, icon, colour, member list, exposed-params editor) are reached by clicking the container's **header strip** in expanded view, or its **chip** in Simplified view — see 6.2c and 6.3.
  ...
  - ~~⚠️ Known gap: creating the group produces no immediate feedback.~~ ~~**Fixed 2026-08-02:** grouping now flips the canvas straight to Simplified view…~~ **Superseded 2026-08-03:** grouping draws a **container box** around the members and stays in the view you were in. **Pass:** the canvas does **not** flip to Simplified view; a `group-container-<groupId>` box appears with a header strip carrying the group's icon, colour and label, and a green toast reads *"N steps grouped. Drag the box's header to move them together."*
  - Select only Map-body members and click **Group selected** → **Pass:** a yellow toast refuses (*"Need 2+ selectable nodes. Map body members are grouped automatically."*) and no group is made.

- [ ] **6.2a Header drag moves the group; member drag moves one step (2026-08-03).** With Simplified view **OFF**, drag the group's **header strip** (`group-container-header-<groupId>`). **Pass:** every member moves by the same delta, keeping the same relative spacing; nodes outside the group don't move. Release, then **Save + reload** → the whole group is where you left it. **Undo once** → the entire move reverses in one step (not one node at a time).
  - Now drag a **member** card instead. **Pass:** only that member moves, and the container box **re-fits around it live** while you drag — it grows or shrinks to keep every member inside. Undo once → that one node returns. *(This reverses the 2026-08-02 rule where dragging any member carried its siblings; the header is the surface that made cohesive drag aimable.)*
  - Drag an **ungrouped** node into the box's area and release. **Pass:** it joins nothing — the group's member list is unchanged and the box does **not** grow to include it. Membership is explicit config, never geometry.
  ...
  - Drag a node inside a **Map body** box (the auto-drawn green container, not a group you made) → **Pass:** only that node moves, and the map-body box has no draggable header. That grouping is derived from the Map node, not authored.

- [ ] **6.2b Delete a group as a unit from its chip (2026-08-02, wires fixed 2026-08-03).** Turn Simplified view **ON**, select a group's chip and press <kbd>Delete</kbd>. **Pass:** a confirm asks *"Delete «name» and its N steps?"* with the real step count.
  - **Cancel** → nothing is removed and the chip is still on the canvas (it must not vanish while the question is open). **Crucially, the wires are also untouched:** turn Simplified view off and confirm every edge into and out of the group's members is still drawn, and that **Undo is not enabled** by the cancelled gesture. *(Before 2026-08-03 the chip's incident edges were cut the moment you pressed Delete — before the modal was answered, and irrecoverably on Cancel.)*
  - **Confirm** → the group and all N steps are gone; a toast names any ctx variables that lost their source, with **Undo**. **One** <kbd>Ctrl</kbd>+<kbd>Z</kbd> restores the whole group *and* its wires — count the presses; two used to be needed because the edge cut and the node removal were separate history entries.
  - Contrast with **Ungroup** (right-click a member, or the right rail's *Ungroup (steps stay)*) → grouping gone, every step stays.
- [ ] **6.2c Open group settings from the container header (2026-08-03).** With Simplified view **OFF**, click empty canvas to clear any selection, then click a group's **header strip**. **Pass:** the right rail switches to **Group settings** for that group — the same panel the chip opens. *(Expanded view had no route to this panel at all before.)*
  - ⚠️ Node selection still wins over the group panel: if a step is selected, the rail stays on the step. Deselect first.
  - Drag the header (6.2a) and release → **Pass:** the settings panel does **not** open. A drag is not a click.
  - Click a **Map body** container's header → **Pass:** the owning Map node is selected and its settings open. Map bodies have no group panel of their own.
- [x] **6.3 Simplified view.** Flip the **Simplified** switch in the top bar's centre zone (`simplified-view-toggle`) on a grouped workflow — it is a visible control now, not a row inside the **More** menu. **Pass:** each group collapses to a single chip (master template → 5 chips); clicking a chip opens **Group settings** in the right rail (label, description, icon, colour, `Members (n)` with per-member remove, **Ungroup (steps stay)**, and the **Exposed parameters** editor — that panel is 6.2's and 6.4's real entry point); toggling back reveals the nodes; round-trips.
...
- [x] **6.7 Auto-arrange.** Click **Auto-arrange** (top bar, `topbar-menu-auto-arrange`) on a stacked layout. **Pass:** dagre lays nodes left-to-right + re-fits; the button is disabled with zero nodes. Right-click empty canvas → **Auto-arrange** does the same thing (3.18).
- [ ] **6.7c Chips can be dragged (2026-08-03).** Turn **Simplified view** ON and drag a group chip somewhere deliberate. **Pass:** it goes where you put it and its wires follow. Save and reload → it is still there. Turn Simplified **OFF** → **Pass:** the expanded canvas is untouched, every member exactly where it was. Drop a chip on the spot it already occupies → **Pass:** nothing is written, so <kbd>Ctrl</kbd>+<kbd>Z</kbd> does not consume an empty step. *(Chips were non-draggable until the two views got separate arrangements: a chip's position was re-derived from its members' centroid on every projection, so a drop had nowhere to land.)*
- [ ] **6.7b Group boxes fit their steps (2026-08-03).** Open a workflow with several groups (e.g. **Standard OCR Workflow (SDPR)**, 5 groups) in **expanded** view. **Pass:** every group box hugs its members — a roughly even margin on all four sides, a little more at the top where the header strip sits — and **no two boxes overlap**. Drag a member out to the right: the box grows to follow it, still tightly. *(The boxes used to be sized from worst-case catalog estimates — a flat 522px width for every activity card and a height including a preview block most cards never render — so the slack piled up on the right and bottom and adjacent boxes collided. They are sized from the cards as measured now.)*
- [ ] **6.7a Auto-arrange in Simplified view (2026-08-03).** On a workflow with 2+ groups, turn **Simplified view** ON and drag the chips into a deliberate mess (or just note where they are), then click **Auto-arrange**. **Pass:** the **chips** move — they are laid out left-to-right, non-overlapping, and the viewport re-fits. *(Previously this looked like it did nothing: dagre laid out the member-level graph and each chip sits at its members' centroid, so the visible nodes barely moved.)*
  - Now turn Simplified view **OFF**. **Pass:** the expanded canvas is **exactly as you left it** — every step at the coordinates it had before you ever switched views. Nothing moved and nothing overlaps.
  - **The two views arrange independently (2026-08-03).** Arrange in simplified view, switch to expanded, arrange again, switch back. **Pass:** each view keeps its own layout; neither arrange disturbs the other. A step that appears in both (an ungrouped one) therefore has a position in each — nudging it in one view does **not** move it in the other. That is the design, not a sync bug. *(This replaces the old known limit: a simplified arrange used to reserve a chip-sized slot per group and then drag every member into it, so expanding showed the whole graph bunched up — permanently, because the result is saved.)*
  - Repeat via the pane menu's **Auto-arrange** and via a demo's arrange-on-load → **Pass:** all three entry points behave identically.
...
- [x] **7.6 Binding-walk validator.** Build a real cross-kind binding (a `Document` producer’s ctx key read by a `Segment`-typed input) → Save. **Pass:** the save persists (draft-save) with an amber *"Saved as a draft"* toast; behind **Review issues**, the error is anchored to the **consumer node + port**, naming producer/consumer kinds + ctx key + “not assignable”. Cardinality strict (`Document` → `Document[]` rejected). **Run…** stays disabled until fixed. Fix → re-save → green toast.
...
- [ ] **8.15 Optional inputs fold; a typed value overrides them (2026-08-03).** Drop a **Prepare File** node and click it. In the right rail's **Inputs** section, **Pass:** the optional identifier ports (`fileType`, `fileName`, `contentType`) are **not** in the main list — they sit behind a collapsed **"3 optional inputs"** disclosure (`optional-inputs-toggle`). Expand it (`optional-inputs-list`). *(Before this, the card rendered a row and a handle for each of these while the panel excluded them entirely — the canvas advertised a port no surface would accept an answer for.)*
  - **The field reads as a labelled field (2026-08-03).** **Pass:** each row's value field sits on its own line **spanning the panel's full width**, indented under the port's label and ruled off from it so the pair reads as one control, with the port's description as wrapped **helper text under the field** — not as a truncated placeholder. The placeholder is just *"Type a value"*. *(It used to render into the middle "source" column of the row grid, which capped it at under half the panel width and left it beside nothing.)*
  - Type `image` into the `fileType` row's value field (`input-constant-fileType`) and press <kbd>Enter</kbd> or click away. **Pass:** the row moves **out** of the disclosure into the main list, wearing a blue **Value** badge, and the count on the disclosure drops to *"2 optional inputs"*. The field keeps the same full-width treatment up there — the move must not change the layout. Typing does **not** commit per keystroke — only blur/Enter does, so it is one undo step.
  - **Save + reload.** **Pass:** the value is still there. Check **More ▸ Workflow settings**: the declaration count did **not** go up — a typed value is a hidden entry (`__const_<nodeId>_<port>`), deliberately kept out of the named-variable table.
  - **Run the workflow** (Part 11) and confirm the value reaches the activity: the run's input ctx carries the key, and the activity behaves as if `image` were passed. Supplying the same key in the run payload **overrides** it — it is a default, not a lock.
  - Clear the field. **Pass:** the row folds back into the disclosure, the hidden declaration is gone, and the port is auto-wireable again.
  - ⚠️ **The problem counts must not move.** An optional identifier port with no value is counted by nobody (no badge, no drawer entry, no top-bar count), and typing a value adds no warning and removes none. On a **required** port, a typed value *does* clear that port's warning.
- [ ] **8.15a Make this a workflow input (2026-08-03).** With a value typed on a port row (8.15), open the row's **⋯** menu (`input-row-menu-<port>`) → **Make this a workflow input**. **Pass:** a modal asks for an **Input name**, prefilled with the port name; it rejects names that aren't `letters/digits/underscore starting with a letter` and rejects a name already declared, quoting it back.
  - Confirm with a fresh name. **Pass:** the hidden entry is **renamed** to that name and flagged as an input — it now appears in **More ▸ Workflow settings ▸ Context declarations** (with your typed text as its **Default value**), in the **Run drawer**'s schema table as an **optional** property carrying that default, and in `GET /api/workflows/:id/run-spec`. The port stays pinned to it.
  - Post a run with a different value for that key. **Pass:** the run value wins over the default.
...
> **⚠️ 9.1 and 9.2 cannot be run on this Part's demo.** The Part 9 demo is **`source.upload`-only**, and such a workflow has **no Try-on-canvas tab** — there is nothing to type into a try, so the Run drawer shows its upload dropzone instead of the two tabs, and the source node's **Upload & Try** is the way in (9.3). Pre-selection rule (`WorkflowEditorV2Page.tsx`, `runDrawerOpenMode`): the drawer opens on **Try on canvas** when there is a `source.api` node **or** an `isInput`-flagged ctx key **or** no `source.upload` at all, and on **Call from outside** when upload is the *sole* input path. Asserted directly in `WorkflowEditorV2Page.test.tsx` ("a source.upload-only workflow keeps the button and opens on \"run\"").
>
> *Changed by batch-four item 8 (2026-08-08).* The top bar used to carry **two** buttons — **Try** and **Run this workflow** — that opened the **same** drawer on different tabs, and **Try** was hidden for upload-only workflows. There is now one **Run…** button, shown for every workflow, and the tab it lands on is derived from the workflow's inputs.
...
- [ ] **9.1 Try on canvas.** On the **saved source.api** workflow above → top-bar **Run…** → the Run drawer opens **pre-selected on the “Try on canvas” tab** → paste JSON → **Try**. **Pass:** drawer closes; canvas polling starts; badges/edges/previews animate. The tab also states in place that *“A try is disposable — starting another run cancels a try that is still going.”* In create mode **Run…** is **disabled** (“Save the workflow first”). **Fail-state to confirm the check works:** open the Part 9 demo instead — **Run…** is still there, but the drawer shows the upload dropzone with **no tabs at all**.
- [ ] **9.2 “Try on canvas” vs “Call from outside”.** Same workflow — switch to the **Call from outside** tab and submit from its **Start a run** box. **Pass:** it keeps Phase-2 behavior (shows `workflowId` inline, drawer stays open, no canvas takeover), while **Try on canvas** takes over the canvas. The two tabs are the only place the choice is offered — there is **one** top-bar button, not two.
- [x] **9.3 Upload & Try.** On a saved source.upload workflow → the source node’s **Upload & Try** → drop a PDF. **Pass:** file commits to blob, a run starts, the source node’s result strip switches from **Not run yet** to the document’s kind + first line, clicking it opens the DocumentPreview of the doc, canvas animates.
...
- [ ] **9.5 Preview widgets.** *(generic-view + no-empty-card clauses verified 2026-07-27; the four kind-specific widgets still need a graph that produces them)* **Click the node's result strip** to open its preview popover (since 2026-08-08 the widget is not inline on the card — see 9.5d). **Pass:** Document → thumbnail strip; Segment[] → polygon overlays; OcrResult → K/V table + “View raw”; Classification → label pill + confidence bar. A kind with **no** dedicated widget (`ValidationResult`, `Reference`, bare `Artifact`, `Document[]`, scalar `Segment`, …) must render the **generic view**: a dimmed caption naming the kind (“ValidationResult — no dedicated preview, showing the raw value”) above a JSON snippet with **View raw**. **No node may render an empty card.** (G-011)
  - The strip itself must already say something useful *without* opening the popover: the kind, then a one-line summary of the value, truncated with an ellipsis (bounded at 72 characters). For an OCR step that summary must be the **content**, not the blob path (it follows the pointer into the same `blobExcerpts` the popover uses).
  - API: `GET /api/workflows/<WF>/preview-cache?nodeId=<NODE>`.
- [x] **9.5a Multi-output preview.** Use a node with more than one output port (`ocr.spellcheck` — `correctedResult` / `corrections` / `metadata`; or `document.classify`, `azureOcr.poll`). After a Try, open that node’s preview popover from its strip. **Pass:** a row of small port chips appears above the preview, labelled with the catalog port labels; the first port is selected and its value is shown; clicking another chip switches the pane to **that port’s** value, styled by that port’s own kind. **The strip follows the chip** — its `<label> · <Kind>` prefix and its one-line summary switch to the port you picked, so the card and the panel can never disagree. A single-output node shows **no** chip row, and its strip shows the kind alone (unchanged). (G-011)
- [ ] **9.5b Bound-but-empty and unbound outputs.** *(unbound clause verified verbatim 2026-07-27; bound-but-empty not yet)* On a node whose output binding points at a ctx key the run never wrote: **Pass:** “No value was recorded for this output (expected `<Kind>`).” On a node with no output binding at all: **Pass:** the strip reads **“Not bound to a value”** (`data-state="empty"`) and its popover reads “This step’s output isn’t bound to a workflow value yet, so there’s nothing to read.” Neither may be a blank card. (G-011)
- [ ] **9.5c OCR previews show values, not a blob pointer.** *(payload-values + truncation clauses verified 2026-07-27; the blob-deleted clause not yet)* On an OCR step (`azureOcr.extract`, `mistral-ocr.process`, any `ocr.*` correction), open the preview popover from the strip. **Pass:** the K/V table shows the **payload’s** keys (e.g. `fileName`, `status`, `extractedText`, `pages`, `documents`) — **not** `blobPath` / `storage` / `byteLength`. For a custom-model run, drilling into `documents → fields` via **View raw** shows the extracted field values. If the payload was bounded, a dimmed line reads “Truncated preview — pages: showing the first 5 of N items; …”, naming every omission. If the blob is gone (delete it from MinIO and re-open the preview), **Pass:** the pointer is shown **with** “The full OCR payload is no longer in storage — showing the reference only.” — never a bare blob key with no explanation. (G-022)
...
- [ ] **9.5d Pressing Try must not move the graph (item 9, 2026-08-08).** Open a **multi-node** workflow — `multi-page-report-workflow` (used by 9.4a) is a good one because it mixes activity, source and control-flow nodes. **Before** running, note two things: every card that can produce output already carries a one-line result strip reading **“Not run yet”**, and control-flow nodes (switch / map / join / humanGate / childWorkflow / pollUntil) carry **no strip at all**. Take a screenshot, or note the vertical gap between two vertically-adjacent cards. Now **Run… → Try on canvas** (or **Upload & Try**) and watch the whole run through to the end.
  - **Pass:** **no card changes height and no card overlaps a neighbour**, at any moment — not while the strips show *Waiting* / *Running…*, not when output lands, not after the run finishes. The strips change what they *say*; the layout does not move.
  - This is the whole point of the fix. Until 2026-08-08 the full preview widget rendered **inline in the card body**, so a producing card grew by up to 200px into dagre’s 60px node separation — and grew **twice**, once for a 120px loading skeleton and again when real content replaced it — leaving cards sitting on top of the ones below. Alex, on the shared screen: *“when you hit try, it also resized the boxes and they started to overlap in a strange way.”*
  - **To measure it rather than eyeball it:** in devtools, `[...document.querySelectorAll('.react-flow__node')].map(n => [n.dataset.id, n.offsetHeight])` before the Try and again after. `offsetHeight` ignores the canvas zoom transform, so the numbers are directly comparable. Expect **0px** drift on every node; the one legitimate exception is a node that **failed**, which grows ~5px for its failure chip.
  - ⚠️ If a card *does* change height, the cause is almost always a disagreement between the strip’s rendered height (`PREVIEW_STRIP_HEIGHT_PX` + `PREVIEW_STRIP_MARGIN_TOP_PX` in `preview/strip-metrics.ts`) and what `estimateNodeHeight` (`canvas/port-rows.ts`) reserved for it — dagre laid the graph out from the estimate and cannot be re-run mid-Try.
...
- [x] **9.8 Run history.** **More ▸ Run history** (⚠️ in More menu). **Pass:** infinite-scroll rows with status badge, version pin (`v3 — head`), timestamp, input summary chip, Replay button; filters (status / date range / version) work. The list holds editor **Try** runs only — production (`api`-triggered) runs are monitored in the Processing queue, not here.
  - API: `GET /api/workflows/<WF>/runs?status=succeeded&limit=50`.
- [x] **9.9 Replay.** Run history → **Replay** a row. **Pass:** canvas shows that run’s frozen badges/edges/previews (from cache, no polling); a **banner across the top of the canvas** (between the top bar and the graph, not in the bar) reads **“Replay mode — you are looking at v{n}, the graph this run used”**, says the canvas is read-only and that edits/Undo/Redo do nothing here, and carries a **Leave replay** button. *(Until 2026-08-08 this was a chip parked beside Undo — see 9.9d.)*
- [x] **9.9a Replay shows the graph that RAN (G-004).** Run a workflow, then **change it** — add a node, delete one, save (creating a new version). Now **More ▸ Run history ▸ Replay** the earlier run. **Pass:** the canvas renders the **older version's** graph: the node you added is **not** on it, and the node you deleted **is** (wearing its result). The replay banner names that version (`v2`, not head). *(Before G-004 statuses were matched by id and painted onto whatever config was on screen — you read yesterday's results on today's diagram.)* Click **Leave replay**: the canvas returns to the live graph, including the added node.
- [x] **9.9b Replay never risks unsaved work (G-004).** Make an **unsaved** edit (rename a node — do NOT save). Enter replay per 9.9a, try to edit something on the historical graph, **press Undo while still in replay**, then **Leave replay**. **Pass:** your unsaved rename is still there, the replayed version's content did **not** leak into the editing config, the leave-page guard still warns, Undo/Redo are **disabled** while replaying, and once you leave replay **Undo** steps back through your own edits only (replay adds no history entries).
- [x] **9.9c Version that can't be loaded.** Replay a run whose version was removed (or stop the backend mid-replay). **Pass:** the replay banner turns orange, leads with **“Replay mode — v{n} could not be loaded, so this is the current graph”**, and spells the consequence out underneath: the workflow *as it stands today* is on screen, it *may differ from what actually ran*, nodes may have been added/removed/reconfigured since. It never silently pretends the current graph is the one that ran. *(This is the state the old chip could not carry — it had one line the width of a word.)*
- [ ] **9.9d Replay banner placement + the version it can't name (item 13, 2026-08-08).** Enter replay per 9.9a. **Pass:** the banner is **below** the top bar and above the canvas — there is **no chip next to Undo** any more — and the top bar does **not** overflow at any width from 1920 down to 1024 (the item-14 fix still holds with the banner up). The canvas simply gets shorter by the banner's height; the graph does not jump or re-fit. Leaving replay gives the height back. Then replay a run that recorded **no version** (an older run started without the version memo, `versionNumber: 0`): the banner says the version is **unknown** — it never prints **v0**, which is not a version that exists.
- [x] **9.10 Cache-evicted preview.** Replay a run whose cache row was deleted **for a node that SUCCEEDED** (`DELETE FROM activity_output_cache WHERE node_id='<id>'`). **Pass:** the node’s result strip reads **“Preview expired”** and its popover holds the red alert “Preview unavailable — cache evicted. Re-run **v{n}** (the version you are viewing) to repopulate.” + a **Re-run v{n}** button that fetches the original `initialCtx` and starts a fresh Try. This must remain distinct from 9.10a — eviction is the ONLY no-output state that offers a Re-run.
  - ⚠️ **It must be impossible to see this during a LIVE Try (item 9, 2026-08-08).** Watch a first, non-replay Try closely as each node goes green. **Pass:** in the ~250ms between a node reporting `succeeded` and its cache row landing, the strip reads **“Output pending”** (`data-state="awaiting-cache"`, popover copy *“This step finished — its output preview is still on its way.”*) and offers **no** Re-run. **Fail:** “cached output has expired · Re-run” on a run that started seconds ago — that blames a TTL which has not expired and offers a Re-run that would cancel the run in flight. `evicted` is reachable only once the run has finished.
- [x] **9.10b Re-run targets the version being viewed (G-024).** With 9.9a's setup (a run on an older version, plus newer edits saved as head), trigger the Re-run from 9.10 while replaying. **Pass:** the POST body carries `workflowVersionId` for the **replayed** version, and the resulting run's history row pins that version — not head. Outside replay (no version pin) the button reads plain **Re-run** and targets head, which is correct there. *(Before G-024 the offered remedy silently ran a different graph and filed the result as if it were the same thing.)*
  - API: watch `POST /api/workflows/<WF>/tries` in devtools → `{ "initialCtx": {…}, "workflowVersionId": "<the replayed version>" }`.
- [x] **9.10c Retention (G-024).** Intermediate values are kept **14 days** by default, not 24 hours — "the run happened yesterday" is the most common debugging situation there is and used to land exactly on the old boundary. **Pass:** `SELECT created_at, expires_at FROM activity_output_cache ORDER BY created_at DESC LIMIT 5;` shows `expires_at ≈ created_at + 14 days`, and replaying a run from **two days ago** still shows its previews rather than the cache-evicted alert. Set `ACTIVITY_OUTPUT_CACHE_TTL_MS=3600000` in the worker/backend env and restart: freshly written rows now expire in an hour, proving the window is tunable without a code change. ⚠️ The GC sweep is an hourly backend-services cron (`ActivityOutputCacheGcService`, `deleteExpired()`), so expired rows are collected without any operator action; expiry is enforced lazily on read regardless, so a not-yet-swept row past `expires_at` still reads as evicted.
- [ ] **9.10a Distinct no-output states.** *(control-flow clause verified 2026-07-27 on 3 nodes; the other states not yet)* Each situation below must show its **own** copy, and none may offer a Re-run button. Each has a short **strip label** on the card and a full sentence in the popover behind it — check both. (G-012)
  - **Mid-Try, node not reached yet:** strip **“Waiting”** → “Waiting — the run hasn’t reached this step yet.” (Previously this was a blank card — the live run showed nothing at all.)
  - **Mid-Try, node executing:** strip **“Running…”** → “Running now — output appears when this step finishes.”
  - **Mid-Try, node just went green:** strip **“Output pending”** → “This step finished — its output preview is still on its way.” Must never be the cache-evicted copy (see 9.10).
  - **Replay, branch not taken:** on a switch-heavy workflow, replay a run and look at a node on the untaken branch → strip **“Not reached”** → “This step was never reached — the run took a different branch.”
  - **Replay, node failed:** strip **“Failed”** → “This step failed — no output was produced to preview.”
  - **Node whose activity never caches** (a catalog `nonCacheable` activity such as `document.updateStatus`, or a dynamic node whose script is *not* tagged `@deterministic true`): strip **“Not cached”** → “This step ran, but this activity never caches its output…”, and for a `dyn.*` node the variant that names the `@deterministic true` tag. No Re-run — nothing expired.
  - **No run selected:** the strip reads **“Not run yet”** and its popover — like a clicked data wire — reads “Run this workflow to see what this step produces.” No query is fired at rest, so a *previous* run’s output is never shown as if it were current. *(Until 2026-08-08 the card showed nothing at all here; the strip is drawn at rest because that reserved space is what stops the graph reflowing when you press Try — 9.5d.)*
  - **Control-flow node (switch/map/join/humanGate/childWorkflow/pollUntil):** draws **no strip at all** (it never writes an output row) — no copy, no band, and never a cache-evicted alert.
  - Inspect via devtools: both the result strip (`node-result-strip-<id>`) and the preview wrapper (`preview-widget-<id>`) carry `data-state="not-started" | "running" | "branch-not-taken" | "failed" | "cancelled" | "awaiting-cache" | "evicted" | "not-cached" | "not-previewable" | "no-run"` — one state per situation, and the same names on the wire-peek popover.
- [x] **9.11 Version run-count badge.** **More ▸ History** after some runs. **Pass:** each version row shows a `{n} runs` badge.
- [x] **9.12 A Try never runs a graph you are not looking at.** Try on a workflow with unsaved changes, and on one with a validation error. **Pass:** the top-bar **Run…** button is **disabled** and explains itself — *"Save your changes first — a run always executes the saved graph, not the canvas"* / *"Fix N validation errors first — this graph cannot run as it stands"* — and **no** Temporal resource is used.
...
- [ ] **11.2 Run drawer.** Top-bar **Run…** (disabled in create mode) → **Call from outside** tab. **Pass:** the tab shows Trigger URL (+copy), input schema field list, sample curl (+copy), auth notes, and a **Start a run** box (Paste-JSON + Run) — the heading reads *Start a run*, not *Test run*: a run started here is not disposable.
- [x] **11.3 run-spec API.**
  ```bash
...
- [x] **14.8 In-canvas custom node.** Palette **Custom** section → **New custom node** modal → publish → node auto-drops as `dyn.<slug>` with a grape **DYN** badge. Right-click a `dyn.*` node → **Edit script**. **Pass:** deleted-lineage node shows a red **Deleted** badge, settings Alert, **Run…** disabled.

### Execute + security
- [x] **14.9 Execute (Try).** Build `source.api → dyn.uppercase-url`, wire `document`, Save → **Run… ▸ Try on canvas** with `{"documentUrl":"https://example.com/foo.pdf"}`. **Pass:** node goes blue→green; preview shows the uppercased URL. Publish v2 (reverse) → try again → cache miss → preview shows reversed URL.
...
- [x] **14.11 🔒 Network egress blocked.** Publish/run a node doing `await fetch("https://blocked.example.com")` with that host **not** in `DYNAMIC_NODE_ALLOW_NET`. Fast path — hit the runner directly:
  ...
  ```
  **Pass:** `exitCode != 0`, stderr mentions Deno net permission denied.

  Then prove the allowlist is what caused it, by granting the host and re-running the same script. **Use a closed loopback port for this half, not a public hostname** — `"script":"export default async function(){ try { await fetch(\"http://127.0.0.1:9/\"); return {reached:true}; } catch(e) { return {reached:false, error:String(e)}; } }"` with `"allowNet":["127.0.0.1:9"]`. **Pass:** `exitCode: 0`, `timedOut: false`, and the returned `error` is an ordinary connection error — *not* `NotCapable: Requires net access`. Granting a non-existent public host instead makes DNS the clock: on a corporate network the sandbox's resolver takes >8s to conclude NXDOMAIN, the runner's own 5s timeout fires first, and you get `timedOut: true` / `exitCode: -1` — a failure that says nothing about permissions. (This is what made the automated `tier3-dynamic-node-security` version of this step fail until 2026-08-09.)

  ⚠️ Locally the container still has NAT internet — you’re verifying the per-script Deno permission gate, not container isolation (true isolation only in OpenShift).
...
- [x] **15.8 Conversation persistence + switcher.** Close/reopen drawer → history reloads. Expand switcher → prior conversations (title/timestamp/model) → switch → trash deletes (204) → reset icon starts a new one. **Pass:** list sorted by `lastMessageAt` desc, scoped to caller+group; cross-user access → 404. Seeded demos are the one exception: they list for every member of their group, badged **demo replay**, with no delete control.
- [ ] **15.9 Cost ceiling.** Set `AGENT_MAX_CONVERSATION_TOKENS=1000`, send 2–3 turns. **Pass:** next turn refused with the in-conversation alert “This conversation has spent its budget” / “Conversation token budget exceeded (X / Y)…” (cause `conversation-budget-exceeded`). New conversation clears it.
...
- [ ] **16.1 Top bar — one row, four groups (rebuilt 2026-08-03).** Confirm `topbar-zone-left` / `topbar-zone-center` / `topbar-zone-right` (the last containing `topbar-group-state` and `topbar-group-actions`) render on **one row, one baseline**, without overlap at narrow widths:
  - **Left:** the workflow **name as a click-to-edit title** (`workflow-title` → `workflow-title-input`) · a **chevron** beside it that opens the workflow switcher (`workflow-switcher-button`) · the node/edge counter. **Pass:** the name is the leftmost thing in the bar, there is no labelled **Switch** button, and there is no **Name** text input and no **Description** field in the top bar at all.
  - **Narrow windows (fixed 2026-08-08).** Resize from 1920px down to 1280px. **Pass:** no control ever overlaps another — in particular the disabled **Undo** button never lands on the **Simplified** switch — and the bar never scrolls sideways. The title and the node/edge counter truncate as the window narrows (counter first, then the title); every control stays present and clickable. Below ~1150px the left zone is fully collapsed and overlap returns; collapsing the app sidebar buys that width back.
  - **Centre:** **Find a node…** (`node-search-input`) · **Simplified** switch (`simplified-view-toggle`) · **Auto-arrange** (`topbar-menu-auto-arrange`) · **Fit view** (`topbar-fit-view`). **Pass:** the last two are disabled with zero nodes; **Fit view** fits the whole graph.
  - **Right:** Undo/Redo · the validity chip (*Valid* / *N warnings* / *N errors*, opens the Validation drawer) │ **Save** · **Run…** · **More**. **Pass:** there is exactly **one** run entry point — the separate **Try** button was removed by batch-four item 8 (it opened the same drawer, on the other tab).
  - **More** menu items, and only these: History, Run history, Save as library, Group selected, Workflow settings, Form preview. **Pass:** **Simplified view** and **Auto-arrange** are *not* in the menu — they left it for the visible centre-zone controls.
  - **Description** → **More ▸ Workflow settings** (`workflow-settings-description`). **Pass:** it is a multi-line box that wraps; it no longer truncates mid-word.
- [ ] **16.1a Click-to-edit title.** Click the title. **Pass:** it becomes a text field with the text selected. <kbd>Enter</kbd> or clicking away commits; <kbd>Esc</kbd> reverts without committing; clearing it and committing restores the old name (empty is not a rename).
- [x] **16.2 Group containers / map-body grouping.** Build a map node with body nodes → toggle **Simplified view**. **Pass:** expanded view draws a container box behind the map body (`group-container-<groupId>`, here `group-container-__map_body_<mapNodeId>`) with a header strip (`group-container-header-<groupId>`); simplified view collapses it to a group chip. An **authored** group renders the same component in violet, with an icon in its header (`group-container-icon-<groupId>`) — see 6.2. *(Testids renamed 2026-08-03: the box used to be `map-body-container-<groupId>` and served map bodies only.)*
...
- [ ] **16.7a Extending BACKWARDS pins only when it is sure (2026-08-03).** Drop a **Poll Classify** (`azureClassify.poll`) node, hover its **Result ID** *input* handle, and pick **Read Blob** from the popover. **Pass:** Read Blob lands to the left, wired into Poll Classify — the **flow edge** is drawn, so execution order is what you asked for — but **Result ID is not bound to anything**. It still reads *"needs a source"*. *(It used to pin Read Blob's `base64` output onto Result ID. `resultId` is typed with the root `Artifact` kind, which every output in the catalog satisfies, so the type check proved nothing and Read Blob's only output won by default.)*
  - Now do the same from Result ID and pick **Submit Classify** (`azureClassify.submit`) instead. **Pass:** this one **does** pin — `resultId → resultId`, matched by name. Its `constructedClassifierName` output, the same kind, is correctly ignored.
  - Sanity-check the forward direction is unchanged: hover a typed **output** (16.7) and pick a consuming activity → **Pass:** still lands pre-wired.
...
POST   /api/workflows/:id/runs           (production trigger — not listed in Run history)
POST   /api/workflows/:id/tries          (editor Try — cached, cancelled by the next Try)
...
GET    /api/workflows/:id/runs[?status=&startedAfter=&startedBefore=&workflowVersionId=&cursor=&limit=]   (Try runs only)
GET    /api/workflows/:id/runs/:runId/node-statuses   (Try runs only; a production runId → 404)
GET    /api/workflows/:id/runs/:runId/input-ctx       (Try runs only; a production runId → 404)
