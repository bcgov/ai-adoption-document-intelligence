...
> **Note — dynamic-node run prerequisite:** a full publish→run of a `dyn.*` node needs the worker's `PLATFORM_API_KEY` provisioned; without it `dyn.run` fails fast with a config error (`PLATFORM_API_KEY is not configured on the worker`) *before* reaching the sandbox. Locally, start the worker with any non-empty value — `PLATFORM_API_KEY=local-dev npm run dev` (from `apps/temporal`), or persist it in the worker's own env so a restart keeps it: `$DI_SECRETS_DIR/temporal.env` (default `~/.config/bcgov-di/temporal.env`), which `apps/temporal/src/env-loader.ts` reads first, ahead of the repo-root `.env`. Restart the worker after setting it — then `tier3-dynamic-node-run` passes. Note the failure gives you no clue on its own: Temporal reports the cause to node-statuses as a bare `Activity task failed`, so the spec carries the prerequisite in its assertion message. In CI it's a worker secret. This is also why `tier3-dynamic-node-security` isolates the permission gate via the publish pipeline + deno-runner fast-path rather than a full run.
