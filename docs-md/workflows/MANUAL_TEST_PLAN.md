...
- [ ] **3.4 Connect nodes.** Add two activities — e.g. **Prepare File** then **Submit OCR** — and drag from the first node’s **right-edge handle** (the round execution handle on the node’s right border) to the second node’s **left-edge handle**. **Pass:** the two nodes are connected, and **what you see depends on whether data rides the edge**:
  - This pair **auto-binds** (Prepare's `preparedData` satisfies Submit OCR's `fileData`), so you get a **kind-coloured data wire** whose hover tooltip reads *"Connected automatically — nearest Prepared file data producer"*, plus an *"Auto-wired…"* toast and the connect-summary popover (8.13). The consumer's **Inputs** row flips to `← Prepare File` with a green **AUTO** badge.
...
- [ ] **5.4a Clicking a problem takes you to it (G-010).** Open the Validation drawer (top-bar issues button) on a workflow with several kinds of problem and click each row. **Pass:** every row that names something concrete is clickable, shows what it will do before you click, and gets you there:
...
- [ ] **6.2 Create a group.** Select 2+ nodes, then **More ▸ Group selected**. **Pass:** `nodeGroups[<id>]` is created and persists through Save/reload; a node can be in only one group (moving it prunes empty old groups). The group's settings (label, description, icon, colour, member list, exposed-params editor) are reached by turning on **Simplified view** and clicking the group's chip — see 6.3.
  ...
  - ⚠️ **Known gap:** creating the group produces **no immediate feedback** — no toast, no canvas change in normal view, and the settings panel does not open even though `handleGroupSelected` sets the active group id for exactly that purpose. The group *is* created correctly; you just can't tell until you switch to Simplified view.
- [ ] **6.3 Simplified view.** **More ▸ Simplified view** on a grouped workflow — note this is a **Switch inside the menu row** (`simplified-view-toggle`), so click the toggle itself, not the row. **Pass:** each group collapses to a single chip (master template → 5 chips); clicking a chip opens **Group settings** in the right rail (label, description, icon, colour, `Members (n)` with per-member remove, Delete group, and the **Exposed parameters** editor — that panel is 6.2's and 6.4's real entry point); toggling back reveals the nodes; round-trips.
...
- [ ] **9.1 Try button.** On the **saved source.api** workflow above → top-bar **Try** → Run drawer opens on the **Try tab** → paste JSON → Try. **Pass:** drawer closes; canvas polling starts; badges/edges/previews animate. In create mode Try is **disabled** (“Save the workflow first”). **Fail-state to confirm the check works:** open the Part 9 demo instead — the Try button must be **absent** (not merely disabled).
- [ ] **9.2 Run vs Try tabs.** Same workflow — switch to the **Run** tab and submit. **Pass:** Run keeps Phase-2 behavior (shows `workflowId` inline, drawer stays open, no canvas takeover); Try takes over the canvas.
...
- [ ] **9.9 Replay.** Run history → **Replay** a row. **Pass:** canvas shows that run’s frozen badges/edges/previews (from cache, no polling); top bar shows **Replay mode — v{n} (read-only)** + Clear.
- [ ] **9.9a Replay shows the graph that RAN (G-004).** Run a workflow, then **change it** — add a node, delete one, save (creating a new version). Now **More ▸ Run history ▸ Replay** the earlier run. **Pass:** the canvas renders the **older version's** graph: the node you added is **not** on it, and the node you deleted **is** (wearing its result). The replay chip names that version (`v2`, not head). *(Before G-004 statuses were matched by id and painted onto whatever config was on screen — you read yesterday's results on today's diagram.)* Click **Clear**: the canvas returns to the live graph, including the added node.
- [ ] **9.9b Replay never risks unsaved work (G-004).** Make an **unsaved** edit (rename a node — do NOT save). Enter replay per 9.9a, try to edit something on the historical graph, then **Clear**. **Pass:** your unsaved rename is still there, the replayed version's content did **not** leak into the editing config, the leave-page guard still warns, and **Undo** still steps back through your own edits only (replay adds no history entries).
- [ ] **9.9c Version that can't be loaded.** Replay a run whose version was removed (or stop the backend mid-replay). **Pass:** the replay chip turns orange and reads **"v{n} unavailable, showing current graph"** — it never silently pretends the current graph is the one that ran.
