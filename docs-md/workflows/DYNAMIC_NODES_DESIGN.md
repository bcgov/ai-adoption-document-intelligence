...
Distinct from Q3's deferred user-supplied secrets, the `dyn.run` activity injects four ambient env vars into every Deno subprocess. These are server-controlled (the credential is minted by the worker per invocation, never read from workflow input — see "Key provenance" below) and let scripts call back into the running backend.

| Env var | Value | Purpose |
|---|---|---|
| `AI_DI_API_BASE_URL` | Backend's base URL (e.g. `http://localhost:3002`) | What URL to call back at |
| `AI_DI_API_KEY` | A short-lived internal token the `dyn.run` activity mints for this one invocation, scoped to the group that owns the running workflow | Authentication for callback requests (sent as the `x-internal-token` header) |
...
**Key provenance.** `AI_DI_API_KEY` is a per-invocation internal token minted SERVER-SIDE by the `dyn.run` activity — it is NOT the caller's `x-api-key`, NOT a shared service-account key, and NOT threaded through workflow/activity input (Temporal persists workflow + activity inputs in durable history, so forwarding a caller credential would store it in cleartext in the history store — security item #4). Before each invocation the worker writes an `internal_token` row over its existing shared-database connection: only the SHA-256 hash of the raw token is stored, alongside `group_id` (the group that owns the running workflow), `purpose = "dyn-run"` and a 120 s `expires_at` (2× the runner's 60 s script wall-clock clamp). The raw value goes into the sandbox env, the backend's auth guard resolves the `x-internal-token` header against the hashed row, and the row is deleted best-effort when the invocation ends — expiry covers the crash path. No secret is provisioned or rotated for this: worker and backend already share the database. If the mint fails (the worker cannot reach the database), `dyn.run` fails with an explicit mint-failure message before the sandbox is invoked.
...
**Credential scope (security item #4):** `AI_DI_API_KEY` is a per-invocation
internal token minted by the `dyn.run` activity, scoped to the group that owns
the running workflow, with a 120 s TTL (§3, "Key provenance"). The sandbox
never sees a long-lived or shared credential: a script can only ever act as
its own group, for roughly the lifetime of its own invocation. This is what
lets any group member publish dynamic nodes without an elevated role — the
blast radius of a malicious or buggy script's platform callbacks is bounded by
the group's own permissions, and there is nothing to steal that outlives the
run.
...
**Retry classification** (`DYN_RUN_ACTIVITY_OPTIONS`): the activity distinguishes transport failures from script outcomes. TRANSPORT failures — the deno-runner unreachable, a connection reset, a mint failure — get 3 attempts with exponential backoff, because a runner pod restart must not fail the run. SCRIPT-level outcomes — a thrown error, invalid output JSON, a timeout, a permission denial — are terminal via `nonRetryableErrorTypes`: they are surfaced to the agent (and the canvas) for revision, not silently retried.
...
- **Code pane (~60% width).** TypeScript code editor. *Implementation note:* Milestone E shipped CodeMirror as a stand-in because Monaco was not installed; the pane now mounts Monaco (`@monaco-editor/react`) with TS highlighting, line-anchored gutter markers, debounced onChange, and programmatic cursor positioning. Monaco's own type checker is off — publish-time `deno check` is the source of truth for type errors. **D-13:** Monaco is bundled from `node_modules` via `loader.config({ monaco })` in `monaco-loader.ts`, NOT fetched from `cdn.jsdelivr.net` as the React wrapper does by default — the editor must work behind an egress proxy and on an air-gapped deploy. The bring-up is bounded (15s to reach `onMount`); on failure the pane says so and Publish refuses, because publishing a script the author was never shown is worse than not publishing. Boilerplate prefilled in create mode:
