...
### 5.1.2 Benchmark OCR cache replay (optional)
...
- **`__benchmarkOcrCache`**: `{ ocrResponse: OCRResponse }` — full Azure poll response cached from a prior benchmark run.

When present, the graph runner merges this into `azureOcr.submit`, `azureOcr.poll`, and `azureOcr.extract` activity parameters so submit/poll skip the network and extract reuses the cached response. See [OCR_IMPROVEMENT_PIPELINE.md](../OCR_IMPROVEMENT_PIPELINE.md) § Benchmark OCR cache.
