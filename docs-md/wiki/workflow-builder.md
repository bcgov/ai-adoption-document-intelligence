...
The workflow builder UI is a drag-and-drop React Flow canvas: nodes are placed from a palette, wired port-to-port, grouped, and configured through a settings panel beside the canvas. Authoring guidance lives in `docs-md/workflows/WORKFLOW_BUILDER_GUIDE.md`.

## Source Map

- Authoring guides and design context live under `docs-md/workflows/`.
- Shared node and UI extension patterns live in `docs-md/architecture/PATTERNS_NODE_AND_UI.md`.
- The routed editor lives in `apps/frontend/src/features/workflow-builder/WorkflowEditorV2Page.tsx`; the list view lives in `apps/frontend/src/pages/WorkflowListPage.tsx`.
- The canvas, settings panel, preview, run surfaces and node palette all live under `apps/frontend/src/features/workflow-builder/`.
...
- Canvas capabilities can be described inconsistently between the README and the builder guides.
