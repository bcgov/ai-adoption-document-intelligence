...
The workflow builder UI edits workflows through a structured form editor (the default) or raw JSON (CodeMirror), and shows a read-only React Flow visualization (pan/zoom only, no canvas drag-and-drop). Full drag-and-drop canvas authoring is design-target material in `docs-md/workflows/WORKFLOW_BUILDER_GUIDE.md`.
