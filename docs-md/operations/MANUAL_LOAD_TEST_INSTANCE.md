...
For OCR-heavy scenarios use **`DOCUMENT_INTELLIGENCE_MODE=mock`** on the backend and **`MOCK_AZURE_OCR=true`** on the worker ([LOAD_TESTING.md](../benchmarking/LOAD_TESTING.md)). You can set these in **`dev.env`** / `<instance>.env` or pass:
