...
The internal tokens that dynamic-node scripts and the chat agent use for
platform self-calls are **not** in this list and need no rotation: they are
minted per invocation, stored only as SHA-256 hashes in the `internal_token`
table, and expire on their own (minutes-scale TTLs). There is no long-lived
platform credential to rotate.
