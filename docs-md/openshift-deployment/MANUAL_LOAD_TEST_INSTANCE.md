...
Be aware the namespace **`storage-quota`** caps total PVC requests across the namespace (e.g. **64 Gi** in **`fd34fb-dev`**, with shared PLG and PG-backup PVCs already consuming roughly **40 Gi**). Check **`oc get resourcequota storage-quota`** before patching; the expand will be rejected with **`exceeded quota`** if not enough headroom is free. The CI/local deploy scripts (`scripts/lib/wait-for-rollouts.sh`) also check quota utilization before rollout and fail with diagnostics when pods stay Pending due to `FailedScheduling`. Recommended default for sustained load testing is **`5–10 Gi`** per instance — a real fix in source would be a sibling **`tools/load-testing/cleanup-blobs.sh`** that prunes the load-test prefixes for a group, freeing actual disk space without changing the PVC request.
