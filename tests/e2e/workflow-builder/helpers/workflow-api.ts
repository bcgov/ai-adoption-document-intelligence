...
/**
 * POST /api/workflows/:id/tries — start a Temporal run with `initialCtx`.
 *
 * Tries, not runs: the per-run editor surfaces these suites assert against
 * (node-statuses, the activity-output cache) are scoped to editor Try runs.
 * A `/runs` start is a production run — never cached, and refused by the
 * per-run endpoints.
 */
export async function startRun(
  ...
  const res = await request.post(
    `${BACKEND_URL}/api/workflows/${workflowId}/tries`,
    { headers, data: { initialCtx } },
  );
  expect(
    res.ok(),
    `start try failed: ${res.status()} ${await res.text()}`,
  ).toBeTruthy();
