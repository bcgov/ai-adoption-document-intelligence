...
| Tier | Files | Needs | In default local run? |
|------|-------|-------|----------------|
...
These suites run **locally only**: no CI job runs Playwright (a compose-based
CI job for the hermetic tiers is a planned follow-up). "✅ yes" above means the
default local `npm run test:e2e` includes the tier, not that CI runs it.
...
- **Supersedes the manual walkthroughs.** The ad-hoc `feature-docs/**/walkthrough.mjs`
  ...
  as committed specs in the default local run.
