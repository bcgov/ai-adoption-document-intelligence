...
**On this benchmark**, the strongest single engine is the custom pipeline (Azure Document Intelligence `prebuilt-layout` OCR + GPT-5.2, sending both the form image *and* the OCR text to the model, referenced as "E08" in this report): F1.mean 0.973, ~95% field accuracy, ~$0.046/page cold-cache. It beats Microsoft's managed Content Understanding service on both accuracy (+2.0 pp F1.mean) and per-page cost (~37% lower), and beats the V1 trained-template approach (E00) by ~5.7 pp F1.mean.

These are encouraging results for the architecture — image + OCR text + a direct prompt to a generative model — and they warrant further exploration as a candidate production path for SDPR. They are **not** by themselves sufficient to commit to E08 as the production extraction path. Several caveats:

1. **The dataset is small (40 samples) and skewed toward edge cases.** The absolute accuracy and cost numbers will shift on a representative production sample. The engine rankings should hold; the deltas almost certainly won't. Differences smaller than ~2 pp F1.mean between engines are close to statistical-noise size on this dataset — see [Statistical significance and confidence intervals](#statistical-significance-and-confidence-intervals).
2. **The result is workload-specific.** E08 is suited to image-based, key-value-shaped forms going into a fixed schema. For digital-native PDFs with selectable text, free-form documents requiring summarisation or Q&A, multi-page tabular reports, or one-off extraction tasks, the trade-offs likely favour a managed product or a different tool entirely.
3. **Operating cost depends on the Azure OpenAI deployment mode chosen.** The benchmark ran in `eastus2` on Global Standard pricing (~$0.046/page cold-cache). Production for BC government data may require Canada residency, which is satisfied by either (a) Global Standard at the same token rate but with requests possibly executing outside Canada, (b) gpt-5.2 PTU Regional in Canada (~$15,750/month minimum reservation — much higher effective per-page cost at SDPR volume), or (c) a different model (e.g. gpt-4o token-based Regional in Canada East at ~21% per-token uplift, with a 3.7 pp F1 drop from gpt-5.2 to gpt-4o). The architectural choice (E08 vs CU vs Neural) is independent of the residency choice; the cost numbers are not. See [Production deployment — Canada data residency](#production-deployment--canada-data-residency).
4. **Generative engines hallucinate, including E08.** Both E08 and CU produce *confidently wrong* values on a small fraction of samples — the model returns a plausible-looking name or income figure with no internal signal that it's wrong (E03 returning `Junal` / `Aku De` / `April Lune` for the real names `Trung` / `AKM De` / `April Luna` on `HR0081 (6)` are documented examples). E08 reduces this class compared to CU but doesn't eliminate it (FP.mean 1.58 vs 1.85). For income-eligibility decisions, where a confidently-wrong income figure could change a benefit determination, the safety net has to be **a comparison source** — full or majority human review of high-stakes fields, ensemble disagreement-flagging, or downstream consistency checks — not a confidence-gated sample, because confidence thresholds cannot reliably separate fabrications from correct answers.
5. **A custom pipeline is not turnkey.** When the form changes, the prompt and field descriptions need to be updated. This is materially less work than re-labelling and re-training a custom template (E00 / E01), but it is not zero. Microsoft's managed Content Understanding (E03) is a viable alternative if this maintenance cost outweighs the accuracy and cost advantage.

Concrete next steps if exploring E08 further as a candidate production path:

1. **Re-measure on a larger and more representative production sample** to firm up the accuracy and cost numbers before any go/no-go decision.
2. **Resolve the deployment-mode question with the BC data-residency policy owner** — whether Global Standard routing (requests may execute outside Canada) meets the residency requirement, or whether PTU Regional / a Canada-pinned alternate model is required. This shifts the cost picture more than the architectural choice does.
3. **Design a human-in-the-loop review strategy proportionate to the risk profile of each field category**, not a fixed sampling percentage. High-stakes fields (income amounts, names that drive eligibility) need a comparison source; lower-stakes fields (signatures, free-form comments) can be confidence-gated.

The rest of this document is the full evidence base — per-engine accuracy, failure modes, cost build-up, and the architectural reasons E08 outperforms the managed alternative on this dataset.
