...
- [x] **Scenario 1**: NodeType union extended in all three apps
...
- [x] **Scenario 2**: TransformNode interface has required format fields
...
- [x] **Scenario 3**: TransformNode interface has the fieldMapping field
...
- [x] **Scenario 4**: TransformNode interface has optional xmlEnvelope field
    - **Given** the `TransformNode` interface definition
    - **When** inspecting the interface
    - **Then** it contains `xmlEnvelope?: string` as an optional field for the XML envelope template

- [x] **Scenario 5**: GraphNode discriminated union includes TransformNode
