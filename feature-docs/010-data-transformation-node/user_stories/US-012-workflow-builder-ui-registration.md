...
- [x] **Scenario 1**: Transform node appears in the node type dropdown
...
- [x] **Scenario 2**: defaultNodeForType returns a valid TransformNode structure
...
- [x] **Scenario 3**: Transform node has defined dimensions in GraphVisualization
...
- [x] **Scenario 4**: Transform node has a distinct colour in GraphVisualization
    - **Given** a workflow config containing a transform node
    - **When** the graph visualization renders
    - **Then** `NODE_COLORS` includes an entry for `"transform"` with a colour distinct from all other node types

- [x] **Scenario 5**: Transform node has an icon in GraphVisualization
    - **Given** a workflow config containing a transform node
    - **When** the graph visualization renders
