...
- [x] **Scenario 1**: Object → rows on mount
    - **Given** `value: { "0": "O", "l": "1" }`
...
- [x] **Scenario 2**: Rows → object on change
...
- [x] **Scenario 3**: Duplicate `from` keys surface a per-row warning
...
- [x] **Scenario 4**: Empty `from` rows are skipped on `onChange`
