...
## Acceptance Criteria
- [x] **Scenario 1**: Context provides required values
...
- [x] **Scenario 2**: Auto-selects first group on initial load
...
- [x] **Scenario 3**: Restores persisted group from localStorage
...
- [x] **Scenario 4**: Falls back to first group when persisted ID is stale
    - **Given** `localStorage` contains an `activeGroupId` that no longer appears in `availableGroups`
    - **When** `GroupContext` initialises
    - **Then** `activeGroup` is set to the first entry in `availableGroups`

- [x] **Scenario 5**: `activeGroup` is null when user has no memberships
    - **Given** a user with an empty `availableGroups` list
    - **When** `GroupContext` initialises
    - **Then** `activeGroup` is `null`

- [x] **Scenario 6**: Active group change is persisted
...
- [x] **Scenario 7**: `useGroup` throws outside of provider
