...
- [ ] **Scenario 1**: Library workflow → schema derived from `metadata.inputs[]`
...
- [ ] **Scenario 2**: Regular workflow → schema derived from ctx with `isInput: true`
...
- [ ] **Scenario 4**: `type: "array"` and `type: "object"` map without restriction
...
- [ ] **Scenario 5**: Function lives in the workflow service module and is pure
...
- [ ] **Scenario 6**: Library + ctx-isInput both present → library wins
