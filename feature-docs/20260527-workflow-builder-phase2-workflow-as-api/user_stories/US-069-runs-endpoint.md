...
- [x] **Scenario 1**: Endpoint exists at `POST /api/workflows/:id/runs`
...
- [x] **Scenario 2**: Optional `workflowVersionId` in the body
...
- [x] **Scenario 3**: `workflowVersionId` from a different lineage → 400
...
- [x] **Scenario 4**: Unknown lineage id → 404
    - **Given** a `:id` that does not resolve to a `WorkflowLineage`
    - **When** the endpoint is called
    - **Then** the response is 404

- [x] **Scenario 5**: Lineage with no published version → 409
...
- [x] **Scenario 6**: Input validation against the derived schema
...
- [x] **Scenario 7**: Auth required
    - **Given** no `x-api-key`
    - **When** the endpoint is called
    - **Then** a 401 is returned

- [x] **Scenario 8**: Calls the refactored temporal client correctly
...
- [x] **Scenario 10**: Full Swagger DTOs
...
- [x] **Scenario 11**: Vitest + supertest coverage
