...
- **L28. Publish-time validation pipeline.** Synchronous inside `POST` / `PUT`. Order: (1) JSDoc parse → if failure, return 400 with `[{ stage: "jsdoc-parse", line, column, message, tag? }]`. (2) Signature semantics — every declared kind exists in registry, `@name` matches `/^[a-z][a-z0-9-]*$/` max 64, `@parameters` shape coerces to JSON Schema 7. (3) TS check — POST to `deno-runner` `/check` endpoint with the script body; parse the runner's structured response into `{ stage: "ts-check", line, column, message }[]`. (4) Allowlist intersection — every host in `@allowNet` must be in `DYNAMIC_NODE_ALLOW_NET` env. Otherwise `{ stage: "allowlist", rejectedHost, message }`. (5) Persist. No script execution at any stage. Backend NEVER spawns Deno directly — all Deno invocations go through the `deno-runner` HTTP service per L49.
...
- **L32. `dyn.run` invocation flow via `deno-runner`.** Inside `dyn.run`: (1) Worker-side module cache lookup `Map<versionId, { signature, allowNet, deterministic, script }>`. Cache miss → SELECT from `dynamic_node_version`, populate cache. LRU cap 256 entries. (2) Compute the intersected `allowNet`: `(globalAllowlist) ∩ (signature.allowNet) ∪ {API_BASE_URL host}`. (3) Compose the runner request `{ script, inputCtx, parameters, allowNet, ambientEnv: { AI_DI_API_BASE_URL, AI_DI_API_KEY, AI_DI_GROUP_ID, AI_DI_WORKFLOW_RUN_ID }, timeoutMs: signature.timeoutMs ?? 60_000, maxMemoryMB: signature.maxMemoryMB ?? 256 }`. (4) POST to `${DENO_RUNNER_URL}/execute`. (5) Worker enforces an HTTP-client-level timeout slightly higher than `timeoutMs` (e.g. `timeoutMs + 5000`) so the runner's own timeout fires first. (6) Parse the runner response. On `timedOut: true` → throw `DynamicNodeTimeoutError`. On `exitCode != 0` → throw `DynamicNodeRuntimeError { exitCode, stderrTail: last 2 KB }`. On stdout-not-JSON → `DynamicNodeOutputInvalidJsonError`. On stdout > 5 MB (the runner caps + signals) → `DynamicNodeStdoutTooLargeError`. (7) Structural output check (every declared output port key present + not `undefined`) → `DynamicNodeOutputShapeError` on miss. (8) Return the parsed object as activity output. Worker NEVER spawns Deno directly — every dynamic-node execution goes through the runner.
...
- **L48. Milestone slicing — A through G + new prerequisite Milestone B0.** Seven feature milestones (A through G) preceded by a new Milestone B0 (`deno-runner` infrastructure), each landing as one commit matching Phase 4 / Phase 8 cadence.
- **L49. `deno-runner` HTTP sidecar service** (per DYNAMIC_NODES_DESIGN.md §1.5). New container image at `apps/deno-runner/` built from `denoland/deno:alpine-latest` exposing `POST /execute`, `POST /check`, `GET /health` on port 9090. Deployed both via `deployments/local/docker-compose.deno.yml` and OpenShift kustomize at `deployments/openshift/kustomize/base/deno-runner/`. Authenticationless on the cluster network, gated by NetworkPolicy permitting ingress only from backend-services + temporal-worker pods. Container resources: 512Mi memory / 500m CPU. Stateless across requests; the worker-side `versionCache` (L32) sends the script body in every `/execute` call.
- **L50. `DENO_RUNNER_URL` env var.** Both `apps/backend-services` and `apps/temporal` read `DENO_RUNNER_URL` (e.g. `http://deno-runner:9090` in compose / cluster; `http://localhost:9090` in local dev when running outside compose). All calls to the runner go through a small shared HTTP client (`packages/deno-runner-client/`) or a per-app client class — to be decided in US-186; recommend an `apps/`-local client to avoid premature sharing.
