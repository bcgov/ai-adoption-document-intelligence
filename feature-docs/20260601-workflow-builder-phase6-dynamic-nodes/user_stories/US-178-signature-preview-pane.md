...
- [ ] **Scenario 1**: New `SignaturePreviewPane.tsx` consumes the live parse result
...
- [ ] **Scenario 2**: Card header — name + description + DYN pill + deterministic flag
...
- [ ] **Scenario 3**: Inputs + outputs tables with Phase 3 kind dots
...
- [ ] **Scenario 4**: Parameters block uses the existing `JsonSchemaForm` in read-only mode
...
- [ ] **Scenario 6**: Tests cover empty + populated states
