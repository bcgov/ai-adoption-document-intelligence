...
- [ ] **Scenario 1**: `DynamicNode` model declared per the requirements
...
- [ ] **Scenario 2**: `DynamicNodeVersion` model declared per the requirements
    - **Given** the same file
    - **When** read
...
- [ ] **Scenario 3**: Bidirectional relations declared between the two models
...
- [ ] **Scenario 4**: Migration generated + applied via `npm run db:generate`
...
- [ ] **Scenario 5**: Backend `npx tsc --noEmit` + `npm test` green after migration
