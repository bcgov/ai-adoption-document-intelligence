...
- [x] **Scenario 1**: `DynamicNodeRepository` class declared with the five methods
...
- [x] **Scenario 2**: `createWithFirstVersion` atomically creates lineage + v1
...
- [x] **Scenario 3**: `publishNewVersion` atomically appends + moves head
...
- [x] **Scenario 4**: `findBySlugForGroup` + `listForGroup` honor soft-delete
...
- [x] **Scenario 5**: `softDelete` sets `deletedAt` idempotently
...
- [x] **Scenario 6**: Unit tests run against a real Prisma client (per CLAUDE.md)
