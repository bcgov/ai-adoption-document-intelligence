...
- [ ] **Scenario 1**: A switch node with no cases lights up the validation drawer
...
- [ ] **Scenario 2**: A join node whose `sourceMapNodeId` points to a non-map (or missing) node lights up the validation drawer
...
- [ ] **Scenario 3**: A PollUntil with an invalid `interval` lights up the validation drawer
