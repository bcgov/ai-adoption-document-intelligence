...
- [x] **Scenario 1**: `signal.name` is a required `TextInput`
...
- [x] **Scenario 2**: `signal.payloadSchema` shows a read-only JSON preview with an "advanced" hint
...
- [x] **Scenario 3**: `timeout` is a required `TextInput` validated as a Temporal duration
...
- [x] **Scenario 4**: `onTimeout` is a SegmentedControl with `fail` / `continue` / `fallback`
...
- [x] **Scenario 5**: `fallbackEdgeId` appears only when `onTimeout === "fallback"`
