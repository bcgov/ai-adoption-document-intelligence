...
- [x] **Scenario 1**: `activityType` is a Select populated from `ACTIVITY_CATALOG`
...
- [x] **Scenario 2**: When `activityType` is chosen, `JsonSchemaForm` renders for `parameters`
...
- [x] **Scenario 3**: `condition` uses `ConditionExpressionEditor`
...
- [x] **Scenario 4**: `interval` is a `TextInput` validated as a Temporal duration
...
- [x] **Scenario 5**: Optional fields `maxAttempts`, `initialDelay`, `timeout`
