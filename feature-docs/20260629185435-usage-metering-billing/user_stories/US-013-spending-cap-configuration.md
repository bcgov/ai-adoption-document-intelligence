...
- [x] **Scenario 1**: Platform admin can set a monthly spending cap on a group
...
- [x] **Scenario 2**: Platform admin can remove a spending cap
...
- [x] **Scenario 3**: Non-admin users cannot set spending caps
    - **Given** a request to set a spending cap made by a non-platform-admin user
    - **When** the request is processed
    - **Then** the request is rejected with HTTP 403

- [x] **Scenario 4**: Groups with no cap configured are treated as unlimited
