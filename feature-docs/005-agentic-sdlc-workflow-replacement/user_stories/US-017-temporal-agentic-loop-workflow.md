...
- [ ] **Scenario 2**: Activities for each major step
    - **Given** the workflow
    - **When** it runs
    - **Then** it uses activities (or child workflows) for: fetching aggregated HITL data (Feature 004), calling the AI recommendation pipeline (Feature 004), running the workflow modification utility and persisting the new version (Feature 004), starting the benchmark run (with workflow override), waiting for run completion, reading the baseline comparison from the run record, and performing replacement when appropriate (US-016)
...
- Feature 005 Step 2. Depends on Feature 004 (US-007 through US-013) and 005 US-015, US-016.
