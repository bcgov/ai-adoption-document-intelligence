...
- [x] **Scenario 1**: Compatible-first sort + divider for incompatibles
...
- [x] **Scenario 2**: Incompatible rows are dimmed and carry a hover tooltip naming the reason
...
- [x] **Scenario 3**: Picker on a port WITHOUT a declared kind shows no dimming
...
- [x] **Scenario 4**: Producer kind unknown → variable treated as compatible
...
- [x] **Scenario 5**: Picker re-sorts when the target port's `kind` changes
