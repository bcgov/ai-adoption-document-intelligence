...
## Acceptance Criteria
- [x] **Scenario 1**: DatabaseService is deleted
...
- [x] **Scenario 2**: Sub-db-service files are deleted from the database module
...
- [x] **Scenario 3**: database.types.ts is deleted
...
- [x] **Scenario 4**: DatabaseModule retains only PrismaService
...
- [x] **Scenario 5**: Only AppModule imports DatabaseModule
...
- [x] **Scenario 6**: Application builds and starts without errors
    - **Given** the cleaned-up codebase
    - **When** running `npm run build`
    - **Then** the build completes with no TypeScript or NestJS errors

- [x] **Scenario 7**: All tests pass after cleanup
    - **Given** the cleaned-up codebase
