# Feature: Unified Authorization via @Authorized Decorator

## Overview

Extend `IdentityGuard` to support a unified `@Authorized` decorator that:
...
@Authorized({ requireSystemAdmin: true })
@Get('admin/stats')
getAdminStats() { ... }

// Any group member — group_id in request body
@Authorized({ groupIdFrom: { body: 'group_id' } })
@Post()
createDocument(@Body() dto: CreateDocumentDto) { ... }

// Group ADMIN required — group_id as route param; also allow API keys
@Authorized({ groupIdFrom: { param: 'groupId' }, minimumRole: GroupRole.ADMIN, allowApiKey: true })
@Delete(':groupId')
...
When `@Authorized` metadata **is present**, the guard:

1. Resolves the identity as today (API key → `groupId`, JWT → `userId`).
...
When `@Authorized` metadata **is absent**, the guard behaves exactly as today (no DB queries, no enforcement — pass-through after identity resolution).

---

### `resolvedIdentity` Shape Extension

The existing `ResolvedIdentity` type gains two optional fields (only populated when `@Authorized` is present):

```typescript
interface ResolvedIdentity {
  userId?: string;
  groupId?: string;   // API key path only
  // New — populated by @Authorized enrichment:
...
`@Authorized` applies the same Swagger security metadata currently applied by `@ApiKeyAuth()` and `@KeycloakSSOAuth()`:
...
All existing usages of `@ApiKeyAuth()` and `@KeycloakSSOAuth()` are replaced with `@Authorized(...)` accordingly.
...
| `@ApiKeyAuth()` decorator | Delete — replaced by `@Authorized` |
| `@KeycloakSSOAuth()` decorator | Delete — replaced by `@Authorized` |

---

## Migration of Existing Endpoints

- **Resource-by-ID endpoints** (e.g. `GET /documents/:documentId`): These fetch the resource first to get its `group_id`, then call `identityCanAccessGroup`. After this feature, they instead check `request.resolvedIdentity.groupRoles[document.group_id]` directly — no helper needed, but the controller still does the resource fetch. `@Authorized` is still applied (e.g. with just `allowApiKey: true` for Swagger replacement), and manual group checks use the enriched `resolvedIdentity.groupRoles` map.
...
- **Creation endpoints with `group_id` in body** (e.g. `POST /upload`): Replaced fully by `@Authorized({ groupIdFrom: { body: 'group_id' }, allowApiKey: true })`.
- **System-admin endpoints** (group management, etc.): Replaced by `@Authorized({ requireSystemAdmin: true })`.
...
- **Group membership requests** endpoints that check `groupId` from a route param can use `@Authorized({ groupIdFrom: { param: 'groupId' } })`.
...
- The guard must not add DB queries to endpoints without `@Authorized`.
