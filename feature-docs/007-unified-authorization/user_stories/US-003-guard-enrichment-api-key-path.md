...
## Acceptance Criteria
- [x] **Scenario 1**: `isSystemAdmin` is set to `false` for API key requests
...
- [x] **Scenario 2**: `groupRoles` is set with the API key's scoped group as `MEMBER`
    - **Given** an API key scoped to `groupId = 'group-123'`
    - **When** the request is processed
    - **Then** `resolvedIdentity.groupRoles` equals `{ 'group-123': GroupRole.MEMBER }`

- [x] **Scenario 3**: No DB queries are made for API key enrichment
