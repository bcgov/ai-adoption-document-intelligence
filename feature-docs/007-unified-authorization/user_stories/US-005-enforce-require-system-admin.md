...
## Acceptance Criteria
- [x] **Scenario 1**: System admin is granted access
...
- [x] **Scenario 2**: Non-system-admin JWT user is rejected
    - **Given** `@Identity({ requireSystemAdmin: true })` is on the handler
    - **When** a JWT user with `isSystemAdmin = false` makes the request
    - **Then** the guard throws `ForbiddenException` (403)

- [x] **Scenario 3**: API key request is always rejected for `requireSystemAdmin`
    - **Given** `@Identity({ requireSystemAdmin: true })` is on the handler
...
- [x] **Scenario 4**: System admin bypasses all subsequent group checks
