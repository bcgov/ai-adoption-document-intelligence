...
**Parent:** [Feature 004 README](./README.md)  
...
- **Run candidate:** When the user has a candidate (from the pipeline), "Run candidate" starts a benchmark run with workflow override using the existing start-run API (`workflowConfigOverride` / `candidateWorkflowId`). Existing run list and run detail pages are used for status and baseline comparison.
...
- The UI SHALL use the existing start-run API with `workflowConfigOverride` and `candidateWorkflowId` when starting a candidate run (no new endpoint for starting runs).
...
- Extend the existing start-run payload (e.g. in `useRuns` or call site) to support optional `workflowConfigOverride` and `candidateWorkflowId` when running a candidate.
...
- Start-run API: `POST /api/benchmark/projects/:projectId/definitions/:definitionId/runs` — see `CreateRunDto` and benchmark controller for optional candidate / override fields used when re-running a candidate workflow
