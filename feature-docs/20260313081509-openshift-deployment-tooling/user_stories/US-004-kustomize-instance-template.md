...
- [x] **Scenario 1**: Instance template generates prefixed resources
...
- [x] **Scenario 2**: Instance label applied to all resources
...
- [x] **Scenario 3**: Each instance gets its own complete stack
...
- [x] **Scenario 4**: Instance routes include instance name in hostname
    - **Given** a generated instance overlay
    - **When** routes are created
    - **Then** each route hostname includes the sanitized instance name as part of the hostname

- [x] **Scenario 5**: Existing overlays remain untouched
