...
- [x] **Scenario 1**: High HTTP error rate rule fires when threshold is exceeded
...
- [x] **Scenario 2**: Slow HTTP response rule fires when p95 latency exceeds 5 seconds
    - **Given** the alert rules are loaded by Prometheus
...
- [x] **Scenario 3**: High Node.js heap usage rule fires when heap ratio exceeds 90%
    - **Given** the alert rules are loaded by Prometheus
