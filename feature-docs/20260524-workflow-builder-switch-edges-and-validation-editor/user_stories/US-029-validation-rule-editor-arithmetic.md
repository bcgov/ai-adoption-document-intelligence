...
- [x] **Scenario 1**: `ArithmeticRuleBody` renders the variant fields including a nested expression sub-form
...
- [x] **Scenario 2**: Editing the nested `operation` propagates correctly
...
- [x] **Scenario 3**: Adding / removing `expression.fields[]` rows works
    - **Given** the above rule
...
- [x] **Scenario 4**: Editing `expression.equals` propagates
    - **Given** the above rule
    - **When** the user types `"netTotal"` into the `equals` input
    - **Then** `onChange` fires with `expression.equals: "netTotal"`

- [x] **Scenario 5**: Default-shape arithmetic rule from the variant-switch
