...
- [ ] **Scenario 1**: An array schema carrying `x-widget: "validation-rule-editor"` is routed to `ValidationRuleEditor`
...
- [ ] **Scenario 2**: Edits inside the editor propagate through `JsonSchemaForm`'s `onChange`
...
- [ ] **Scenario 3**: Loading `multi-page-report-workflow.json` shows 4 rules
...
- [ ] **Scenario 4**: Without the widget hint, the array still falls back gracefully
