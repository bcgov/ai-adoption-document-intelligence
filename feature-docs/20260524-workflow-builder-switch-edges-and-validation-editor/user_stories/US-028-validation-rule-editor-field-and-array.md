...
- [x] **Scenario 1**: `FieldMatchRuleBody` renders all six variant fields
...
- [x] **Scenario 2**: `FieldMatchRuleBody` propagates edits via `onChange`
...
- [x] **Scenario 3**: Tolerance fields default to undefined and are stripped when blank
...
- [x] **Scenario 4**: `ArrayMatchRuleBody` renders the array variant correctly
...
- [x] **Scenario 5**: Adding/removing items in `primaryFields` mutates the right paths
...
- [x] **Scenario 6**: Required-field surfacing via Mantine `withAsterisk`
    - **Given** a `field-match` rule with `name: ""`
    - **When** `FieldMatchRuleBody` renders
