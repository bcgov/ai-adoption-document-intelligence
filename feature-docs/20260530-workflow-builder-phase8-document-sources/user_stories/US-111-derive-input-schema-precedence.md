...
- [ ] **Scenario 2**: library `metadata.inputs[]` wins when no `source.api`
...
- [ ] **Scenario 3**: `isInput`-flagged ctx wins when no source.api + no library
    - **Given** the same helper
...
- [ ] **Scenario 6**: full unit-test coverage at `build-run-spec.test.ts`
