...
- [x] **Scenario 1**: SourceNode with non-empty `inputs[]` rejected
...
- [x] **Scenario 2**: Unknown `sourceType` rejected
...
- [x] **Scenario 3**: `parameters` failing the entry's `parametersSchema` rejected
...
- [x] **Scenario 4**: Multi-`source.api` + multi-`source.upload` rejected (Phase 8.0 restriction)
    - **Given** the same validator
...
- [x] **Scenario 5**: Soft warning when both `source.api` and `isInput`-flagged ctx present
    - **Given** the same validator
...
- [x] **Scenario 6**: Existing validator tests still green
