...
The user vision: *"instruct an AI agent to build these workflows for you on the fly… work in a feedback loop where it sets up the pipeline and tests it"* ([NOTES.md §1.7](../../docs-md/workflow-builder/NOTES.md#17-ai-built-workflows--feedback-loop)). Designer confirmed this is the long-term primary creation path ([NOTES.md §2](../../docs-md/workflow-builder/NOTES.md#2-designer-conversation-outcomes)). Phase 6's structured `ParseError[]` (signature/line/column anchored) and Phase 4's preview-cache surfacing were specifically designed so that an LLM agent could read failures + outputs and revise — Phase 7 is the consumer that realises that design.
