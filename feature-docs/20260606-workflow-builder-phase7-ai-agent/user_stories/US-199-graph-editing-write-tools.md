...
- [x] **Scenario 1**: `addNode` tool inserts a node + persists via existing `WorkflowsService.update`
...
- [x] **Scenario 2**: `setNodeParameters` tool patches a single node's parameters
...
- [x] **Scenario 3**: `connectNodes` tool adds edges + optional input binding
    - **Given** the tool registered
...
- [x] **Scenario 4**: `deleteNode` tool cascades edges + bindings
    - **Given** the tool registered
...
- [x] **Scenario 5**: `setEntryNode` tool + unit tests for all five
