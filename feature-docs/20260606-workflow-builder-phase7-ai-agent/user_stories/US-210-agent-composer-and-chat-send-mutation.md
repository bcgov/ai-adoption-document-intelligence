...
- [ ] **Scenario 1**: `AgentComposer` mounts assistant-ui's `Composer`
...
- [ ] **Scenario 2**: `useAgentChatSend` opens an SSE stream against `/api/agent/chat`
...
- [ ] **Scenario 3**: Conversation + workflow scope flows from drawer state
...
- [ ] **Scenario 4**: Stream lifecycle + error handling
    - **Given** the hook
...
- [ ] **Scenario 5**: Mutation + composer tests
