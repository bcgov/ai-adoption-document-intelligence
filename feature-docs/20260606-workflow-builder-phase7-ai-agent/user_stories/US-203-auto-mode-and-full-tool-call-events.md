...
- [ ] **Scenario 1**: `permissionMode: 'bypassPermissions'` wired into `SDK.query()`
...
- [ ] **Scenario 2**: Translator decodes `tool-call-start` + `tool-call-complete` + `tool-call-error` events
...
- [ ] **Scenario 3**: Tool-call records persisted to `ChatMessage.content`
...
- [ ] **Scenario 4**: Context-compression env var read by SDK options
...
- [ ] **Scenario 5**: End-to-end smoke with a write tool
