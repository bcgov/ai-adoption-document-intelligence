...
- [x] **Scenario 1**: `createWorkflow` tool registered
...
- [x] **Scenario 2**: `updateWorkflowMetadata` tool registered
    - **Given** the same file
    - **When** read after the change
...
- [x] **Scenario 3**: Validation errors propagate as structured tool errors
...
- [x] **Scenario 4**: Conversation `workflowId` is bound on first `createWorkflow` success
...
- [x] **Scenario 5**: Unit tests for both handlers
