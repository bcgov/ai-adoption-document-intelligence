...
## Acceptance Criteria
- [x] **Scenario 1**: Group ID extracted from route param and member is allowed
...
- [x] **Scenario 2**: Group ID extracted from query param and member is allowed
...
- [x] **Scenario 3**: Group ID extracted from request body and member is allowed
    - **Given** `@Identity({ groupIdFrom: { body: 'group_id' } })` and the caller is a member
    - **When** the request arrives with `group_id` in the body
    - **Then** the guard passes

- [x] **Scenario 4**: Missing group ID results in 400 Bad Request
...
- [x] **Scenario 5**: Non-member is rejected with 403 Forbidden
...
- [x] **Scenario 6**: System admin bypasses membership check
