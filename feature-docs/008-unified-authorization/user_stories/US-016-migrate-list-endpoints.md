...
## Acceptance Criteria
- [x] **Scenario 1**: System admin sees resources from all groups
...
- [x] **Scenario 2**: Non-admin caller sees only resources from their groups
...
- [x] **Scenario 3**: `getIdentityGroupIds` is not called in the migrated controller
    - **Given** the migrated list endpoint
    - **When** the controller code is reviewed
    - **Then** no calls to `getIdentityGroupIds` remain

- [x] **Scenario 4**: `@Identity` decorator is present for enrichment
    - **Given** the migrated list endpoint
