...
- [x] **Scenario 1**: `cancelInFlightTriesForLineage` helper
...
- [x] **Scenario 2**: Helper is idempotent
    - **Given** the helper
...
- [x] **Scenario 3**: `POST /sources/:id/upload` extends response with `runId` + `workflowVersionId`
...
- [x] **Scenario 4**: Source-upload cache row written at workflow start
...
- [x] **Scenario 5**: DTO + Swagger updated
...
- [x] **Scenario 6**: Tests cover cancel + new response shape
