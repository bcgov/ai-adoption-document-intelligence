...
- [x] **Scenario 1**: Collect ctx values for each port binding
...
- [x] **Scenario 2**: Empty / absent `node.inputs` returns the empty-object hash
...
- [x] **Scenario 3**: Document / Segment ctx values are content-hashed via `hashArtifact`
...
- [x] **Scenario 4**: Primitive ctx values pass through `stableJson` directly
...
- [x] **Scenario 5**: Missing ctx key produces a stable sentinel
