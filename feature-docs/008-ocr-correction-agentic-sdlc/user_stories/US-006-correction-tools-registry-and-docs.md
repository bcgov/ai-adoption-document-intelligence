...
- Correction activity types include `ocr.spellcheck`, `ocr.characterConfusion`, `ocr.normalizeFields`, and `ocr.enrich` (enrich is registered for graphs but **not** listed in the AI recommender manifest). The **correction tool manifest** complements generic activity registration with parameter schemas for the three pipeline tools; **placement** for candidates is the fixed edge after `azureOcr.extract`, not manifest “slots.” User-facing pipeline and graph docs: [docs-md/OCR_IMPROVEMENT_PIPELINE.md](../../../docs-md/OCR_IMPROVEMENT_PIPELINE.md), [ADDING_GRAPH_NODES_AND_ACTIVITIES.md](../../../docs-md/graph-workflows/ADDING_GRAPH_NODES_AND_ACTIVITIES.md).
