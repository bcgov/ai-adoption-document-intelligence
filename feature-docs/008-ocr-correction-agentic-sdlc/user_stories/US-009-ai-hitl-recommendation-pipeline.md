...
- **Recommendation service:** `AiRecommendationService` (`apps/backend-services/src/benchmark/ai-recommendation.service.ts`) uses the manifest for **tool IDs and parameters** only. **`ocr.enrich`** is **not** recommendable. Enabled tools are applied in fixed order **`ocr.characterConfusion` → `ocr.normalizeFields` → `ocr.spellcheck`** on the **first normal edge after `azureOcr.extract`**. See [docs-md/OCR_IMPROVEMENT_PIPELINE.md](../../../docs-md/OCR_IMPROVEMENT_PIPELINE.md).
- **Enrichment:** If the base graph includes **`ocr.enrich`**, optional **`llmPromptAppend`** on that node appends to the enrichment LLM user prompt when **`enableLlmEnrichment`** is true (`enrichment-llm.ts` / `enrich-results.ts`). The AI recommender does not set enrich parameters.
