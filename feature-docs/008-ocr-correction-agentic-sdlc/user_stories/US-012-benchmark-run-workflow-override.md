...
- Step 4; requirements Section 6 (workflow versioning). The **OCR improvement pipeline** persists a **candidate workflow version** and starts a benchmark run with **`candidateWorkflowVersionId`** so execution loads config from that `workflow_version` row while the definition remains the comparison anchor. Orchestration: `OcrImprovementPipelineService` in `apps/backend-services/src/benchmark/ocr-improvement-pipeline.service.ts`. API, run `params` (e.g. `candidateWorkflowVersionId`, `workflowConfigHash`), and troubleshooting are documented in [docs-md/OCR_IMPROVEMENT_PIPELINE.md](../../../docs-md/OCR_IMPROVEMENT_PIPELINE.md).
