...
## Acceptance Criteria
- [x] **Scenario 1**: System admin creates a group successfully
...
- [x] **Scenario 2**: Returns `409` for a duplicate group name
...
- [x] **Scenario 3**: Returns `403` for non-system-admin callers
    - **Given** a caller who is not a system admin
    - **When** `POST /api/groups` is called
    - **Then** a `403 Forbidden` response is returned

- [x] **Scenario 4**: `name` is required
