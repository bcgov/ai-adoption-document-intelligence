...
## Acceptance Criteria
- [x] **Scenario 1**: Group admin successfully removes a member
...
- [x] **Scenario 2**: System admin successfully removes a member
...
- [x] **Scenario 3**: Returns `403` for a regular group member
    - **Given** a caller who is a member of the group but not a group admin or system admin
    - **When** `DELETE /api/groups/:groupId/members/:userId` is called
    - **Then** a `403 Forbidden` response is returned

- [x] **Scenario 4**: Returns `404` when target user is not a member
