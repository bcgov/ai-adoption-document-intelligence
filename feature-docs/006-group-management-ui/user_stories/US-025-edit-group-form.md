...
## Acceptance Criteria
- [x] **Scenario 1**: Edit button is visible per group row for system admins
...
- [x] **Scenario 2**: Edit button is not visible to non-admin users
    - **Given** a regular user on `/groups`
    - **When** the groups list renders
    - **Then** no `Edit` button is visible

- [x] **Scenario 3**: Clicking Edit opens a modal pre-populated with current values
...
- [x] **Scenario 4**: Submitting the form updates the group and refreshes the list
    - **Given** the form has valid updated values
    - **When** the form is submitted
    - **Then** `PATCH /api/groups/:groupId` is called, the modal closes, and the groups list refreshes with the updated values

- [x] **Scenario 5**: A descriptive error is shown on validation or conflict failures
