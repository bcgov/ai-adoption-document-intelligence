...
## Acceptance Criteria
- [ ] **Scenario 1**: Create Group button is visible to system admins on the groups listing page
    - **Given** a user with the `system-admin` role on `/groups`
    - **When** the page renders
    - **Then** a `Create Group` button is shown

- [ ] **Scenario 2**: Create Group button is not visible to non-admin users
    - **Given** a regular user on `/groups`
    - **When** the page renders
    - **Then** no `Create Group` button is visible

- [ ] **Scenario 3**: Clicking Create Group opens a modal with Name and Description fields
    - **Given** the admin clicks `Create Group`
    - **When** the modal opens
    - **Then** a form is shown with a required `Name` field and an optional `Description` field

- [ ] **Scenario 4**: Submitting the form creates the group and refreshes the list
...
- [ ] **Scenario 5**: A descriptive error is shown for duplicate names or validation failures
