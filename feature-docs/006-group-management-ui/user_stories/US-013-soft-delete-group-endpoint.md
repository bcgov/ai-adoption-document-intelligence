...
## Acceptance Criteria
- [x] **Scenario 1**: System admin soft-deletes a group successfully
...
- [x] **Scenario 2**: Soft-deleted group is excluded from subsequent listings
...
- [x] **Scenario 3**: No cascade deletes occur
...
- [x] **Scenario 4**: Returns `404` for a non-existent group
    - **Given** a `groupId` that does not exist
    - **When** `DELETE /api/groups/:groupId` is called
    - **Then** a `404 Not Found` response is returned

- [x] **Scenario 5**: Returns `403` for non-system-admin callers
